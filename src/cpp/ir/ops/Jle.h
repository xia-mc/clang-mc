...
#ifndef CLANG_MC_JLE_H
#define CLANG_MC_JLE_H
...
class Jle : public JmpLike, public CmpLike {
private:
...
public:
    explicit Jle(const i32 lineNumber, ValuePtr &&left, ValuePtr &&right, std::string label) :
            Op("jle", lineNumber), CallLike(std::move(label)), CmpLike(std::move(left), std::move(right)) {
    }
    ...
    [[nodiscard]] std::string toString() const noexcept override {
        return fmt::format("jle {}, {}, {}", left->toString(), right->toString(), label);
    }
