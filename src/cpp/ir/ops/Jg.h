...
#ifndef CLANG_MC_JG_H
#define CLANG_MC_JG_H
...
class Jg : public JmpLike, public CmpLike {
private:
...
public:
    explicit Jg(const i32 lineNumber, ValuePtr &&left, ValuePtr &&right, std::string label) :
            Op("jg", lineNumber), CallLike(std::move(label)), CmpLike(std::move(left), std::move(right)) {
    ...
    [[nodiscard]] std::string toString() const noexcept override {
        return fmt::format("jg {}, {}, {}", left->toString(), right->toString(), label);
    }
