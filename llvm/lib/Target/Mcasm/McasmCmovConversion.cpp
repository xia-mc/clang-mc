//====- McasmCmovConversion.cpp - Convert Cmov to Branch --------------------===//
//
...
//
/// \file
/// This file implements a pass that converts Mcasm cmov instructions into
...
//
//  External interfaces:
//      FunctionPass *llvm::createMcasmCmovConverterPass();
//      bool McasmCmovConverterPass::runOnMachineFunction(MachineFunction &MF);
//
//===----------------------------------------------------------------------===//
...
static cl::opt<bool>
    EnableCmovConverter("x86-cmov-converter",
                        cl::desc("Enable the Mcasm cmov-to-branch optimization."),
...
/// Converts Mcasm cmov instructions into branches when profitable.
class McasmCmovConversionImpl {
public:
  McasmCmovConversionImpl(MachineLoopInfo *MLI) : MLI(MLI) {}
...
class McasmCmovConversionLegacy : public MachineFunctionPass {
public:
  McasmCmovConversionLegacy() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override { return "Mcasm cmov Conversion"; }
...
char McasmCmovConversionLegacy::ID = 0;

void McasmCmovConversionLegacy::getAnalysisUsage(AnalysisUsage &AU) const {
  MachineFunctionPass::getAnalysisUsage(AU);
  AU.addRequired<MachineLoopInfoWrapperPass>();
}

bool McasmCmovConversionImpl::runOnMachineFunction(MachineFunction &MF) {
...
bool McasmCmovConversionImpl::collectCmovCandidates(
  ...
  //     3. have only operand registers (Mcasm::CMOVrr).
  //===--------------------------------------------------------------------===//
  // List of possible improvement (TODO's):
  // --------------------------------------
  //   TODO: Add support for Mcasm::CMOVrm instructions.
  //   TODO: Add support for Mcasm::SETcc instructions.
  ...
  for (auto *MBB : Blocks) {
    ...
    Mcasm::CondCode FirstCC = Mcasm::COND_INVALID, FirstOppCC = Mcasm::COND_INVALID,
                  MemOpCC = Mcasm::COND_INVALID;
    ...
    for (auto &I : *MBB) {
      ...
      Mcasm::CondCode CC = Mcasm::getCondFromCMov(I);
      // Check if we found a Mcasm::CMOVrr instruction. If it is marked as
      ...
      if (CC != Mcasm::COND_INVALID &&
        ...
        if (Group.empty()) {
          ...
          FirstCC = CC;
          FirstOppCC = Mcasm::GetOppositeBranchCondition(CC);
        ...
        if (!SkipGroup &&
            llvm::any_of(
                ...
                [&](MachineInstr &UseI) {
                  return UseI.getOpcode() == Mcasm::SUBREG_TO_REG;
                }))
      ...
      // We found a non Mcasm::CMOVrr instruction.
      ...
      if (I.definesRegister(Mcasm::EFLAGS, /*TRI=*/nullptr)) {
...
bool McasmCmovConversionImpl::checkForProfitableCmovCandidates(
    ArrayRef<MachineBasicBlock *> Blocks, CmovGroups &CmovInstGroups) {
  struct DepthInfo {
  ...
  for (auto &Group : TempGroups) {
    ...
    for (auto *MI : Group) {
      ...
      if (hasSingleElement(UIs)) {
        ...
        if (Op == Mcasm::MOV64rm || Op == Mcasm::MOV32rm) {
          WorthOpGroup = false;
          break;
        }
      }
...
static bool checkEFLAGSLive(MachineInstr *MI) {
  if (MI->killsRegister(Mcasm::EFLAGS, /*TRI=*/nullptr))
    return false;
  ...
  for (auto I = std::next(ItrMI), E = BB->end(); I != E; ++I) {
    if (I->readsRegister(Mcasm::EFLAGS, /*TRI=*/nullptr))
      return true;
    if (I->definesRegister(Mcasm::EFLAGS, /*TRI=*/nullptr))
      return false;
  }
...
static void packCmovGroup(MachineInstr *First, MachineInstr *Last) {
  assert(Mcasm::getCondFromCMov(*Last) != Mcasm::COND_INVALID &&
...
void McasmCmovConversionImpl::convertCmovInstsToBranches(
  ...
  Mcasm::CondCode CC = Mcasm::CondCode(Mcasm::getCondFromCMov(MI));
  Mcasm::CondCode OppCC = Mcasm::GetOppositeBranchCondition(CC);
  ...
  if (llvm::any_of(Group, [&](MachineInstr *I) {
        return I->mayLoad() && Mcasm::getCondFromCMov(*I) == CC;
      }))
    std::swap(CC, OppCC);
  ...
  if (checkEFLAGSLive(LastCMOV)) {
    FalseMBB->addLiveIn(Mcasm::EFLAGS);
    SinkMBB->addLiveIn(Mcasm::EFLAGS);
  }
  ...
  BuildMI(MBB, DL, TII->get(Mcasm::JCC_1)).addMBB(SinkMBB).addImm(CC);
  ...
  for (MachineBasicBlock::iterator MIIt = MIItBegin; MIIt != MIItEnd;) {
    ...
    if (!MI.mayLoad()) {
      ...
      Register FalseReg =
          MI.getOperand(Mcasm::getCondFromCMov(MI) == CC ? 1 : 2).getReg();
    ...
    assert(Mcasm::getCondFromCMov(MI) == OppCC &&
    ...
    assert(Mcasm::getCondFromCMov(*NewCMOV) == OppCC &&
  ...
  for (MachineBasicBlock::iterator MIIt = MIItBegin; MIIt != MIItEnd; ++MIIt) {
    ...
    // be generated.
    if (Mcasm::getCondFromCMov(*MIIt) == OppCC)
    ...
    //  ...
    MIB = BuildMI(*SinkMBB, SinkInsertionPoint, DL, TII->get(Mcasm::PHI), DestReg)
              .addReg(Op1Reg)
              .addMBB(FalseMBB)
              .addReg(Op2Reg)
              .addMBB(MBB);
...
INITIALIZE_PASS_BEGIN(McasmCmovConversionLegacy, DEBUG_TYPE,
                      "Mcasm cmov Conversion", false, false)
INITIALIZE_PASS_DEPENDENCY(MachineLoopInfoWrapperPass)
INITIALIZE_PASS_END(McasmCmovConversionLegacy, DEBUG_TYPE, "Mcasm cmov Conversion",
                    false, false)

FunctionPass *llvm::createMcasmCmovConversionLegacyPass() {
  return new McasmCmovConversionLegacy();
}

bool McasmCmovConversionLegacy::runOnMachineFunction(MachineFunction &MF) {
  if (skipFunction(MF.getFunction()))
    return false;
  MachineLoopInfo *MLI = &getAnalysis<MachineLoopInfoWrapperPass>().getLI();
  McasmCmovConversionImpl Impl(MLI);
  return Impl.runOnMachineFunction(MF);
}

PreservedAnalyses
McasmCmovConversionPass::run(MachineFunction &MF,
