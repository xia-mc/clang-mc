...
namespace llvm {
  ...
  enum NodeType : unsigned {
    ...
    /// Mcasm funnel/double shift i16 instructions. These correspond to
    /// Mcasm::SHLDW and Mcasm::SHRDW instructions which have different amt
    ...
    /// to Mcasm::ANDPS or Mcasm::ANDPD.
    FAND,

    /// Bitwise logical OR of floating point values. This corresponds
    /// to Mcasm::ORPS or Mcasm::ORPD.
    FOR,

    /// Bitwise logical XOR of floating point values. This corresponds
    /// to Mcasm::XORPS or Mcasm::XORPD.
    FXOR,

    ///  Bitwise logical ANDNOT of floating point values. This
    /// corresponds to Mcasm::ANDNPS or Mcasm::ANDNPD.
    FANDN,

    /// These operations represent an abstract Mcasm call
    ...
    /// Mcasm compare and logical compare instructions.
    CMP,
    FCMP,
    COMI,
    UCOMI,

    // Mcasm compare with Intrinsics similar to COMI.
    COMX,
    UCOMX,

    /// Mcasm bit-test instructions.
    BT,

    /// Mcasm SetCC. Operand 0 is condition code, and operand 1 is the EFLAGS
    ...
    /// Mcasm FP SETCC, implemented with CMP{cc}SS/CMP{cc}SD.
    /// Operands are two FP values to compare; result is a mask of
    /// 0s or 1s.  Generally DTRT for C/C++ with NaNs.
    FSETCC,

    /// Mcasm FP SETCC, similar to above, but with output as an i1 mask and
    /// and a version with SAE.
    FSETCCM,
    FSETCCM_SAE,

    /// Mcasm conditional moves. Operand 0 and operand 1 are the two values
    ...
    /// Repeat fill, corresponds to Mcasm::REP_STOSx.
    REP_STOS,

    /// Repeat move, corresponds to Mcasm::REP_MOVSx.
    REP_MOVS,
    ...
    /// Special wrapper used under Mcasm-64 PIC mode for RIP
    ...
    /// i32, corresponds to Mcasm::PEXTRB.
    PEXTRB,

    /// Extract a 16-bit value from a vector and zero extend it to
    /// i32, corresponds to Mcasm::PEXTRW.
    PEXTRW,
    ...
    /// corresponds to Mcasm::PINSRB.
    PINSRB,

    /// Insert the lower 16-bits of a 32-bit value to a vector,
    /// corresponds to Mcasm::PINSRW.
    PINSRW,
    ...
    /// Tail call return. See McasmTargetLowering::LowerCall for
    ...
    // Mcasm-specific multiply by immediate.
    MUL_IMM,
    ...
    // These nodes produce 3 results, index, mask, and flags. McasmISelDAGToDAG
    ...
    /// Mcasm strict FP compare instructions.
    ...
    /// to the Mcasm::FIST*m instructions and the rounding mode change stuff. It
    ...
    /// Mcasm::FILD*m instructions. It has two inputs (token chain and address)
    ...
    /// This corresponds to the Mcasm::FLD32m / Mcasm::FLD64m. It takes a chain
    ...
    /// slots. This corresponds to the Mcasm::FST32m / Mcasm::FST64m. It takes a
    /// chain operand, value to store, address, and glue. The memory VT
    /// specifies the type to store as.
    FST,
    ...
    // Mcasm specific gather and scatter
    MGATHER,
    MSCATTER,
  ...
  namespace Mcasm {
    ...
    bool mayFoldLoad(SDValue Op, const McasmSubtarget &Subtarget,
    ...
    /// functions.
    bool isExtendedSwiftAsyncFrameSupported(const McasmSubtarget &Subtarget,
                                            const MachineFunction &MF);

    /// Convert LLVM rounding mode to Mcasm rounding mode.
    int getRoundingModeMcasm(unsigned RM);

  } // end namespace Mcasm

  //===--------------------------------------------------------------------===//
  //  Mcasm Implementation of the TargetLowering interface
  class McasmTargetLowering final : public TargetLowering {
  ...
  public:
    ...
    /// for all types except for some special cases. For example, on Mcasm
    ...
    bool shouldConvertFpToSat(unsigned Op, EVT FPVT, EVT VT) const override {
      // Converting to sat variants holds little benefit on Mcasm as we will just
    ...
    bool isTargetCanonicalConstantNode(SDValue Op) const override {
      ...
      return Op.getOpcode() == McasmISD::VBROADCAST_LOAD ||
             Op.getOpcode() == McasmISD::SUBV_BROADCAST_LOAD ||
  ...
  private:
    /// Keep a reference to the McasmSubtarget around so that we can
    ...
    // Utility function to emit the low-level va_arg code for Mcasm-64.
    MachineBasicBlock *
    ...
    /// corresponding Mcasm condition code constant in McasmCC.
    SDValue emitFlagsForSetcc(SDValue Op0, SDValue Op1, ISD::CondCode CC,
                              const SDLoc &dl, SelectionDAG &DAG,
                              SDValue &McasmCC) const;
  ...
  public:
    ...
    static bool classof(const SDNode *N) {
      return N->getOpcode() == McasmISD::MGATHER ||
             N->getOpcode() == McasmISD::MSCATTER;
    }
  };

  class McasmMaskedGatherSDNode : public McasmMaskedGatherScatterSDNode {
  public:
  ...
  class McasmMaskedScatterSDNode : public McasmMaskedGatherScatterSDNode {
  public:
    const SDValue &getValue() const { return getOperand(1); }

    static bool classof(const SDNode *N) {
      return N->getOpcode() == McasmISD::MSCATTER;
    }
  };
...
#endif // LLVM_LIB_TARGET_Mcasm_McasmISELLOWERING_H
