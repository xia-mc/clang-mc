...
// Mcasm MachineCombiner patterns
enum McasmMachineCombinerPattern : unsigned {
  // Mcasm VNNI
...
std::pair<CondCode, bool> getMcasmConditionCode(CmpInst::Predicate Predicate);
...
/// have a condition code, return Mcasm::COND_INVALID.
...
/// memory reference, or -1 if it has none. Unlike McasmII::getMemoryOperandNo(),
...
inline static bool isGlobalStubReference(unsigned char TargetFlag) {
  switch (TargetFlag) {
  case McasmII::MO_DLLIMPORT:               // dllimport stub.
  case McasmII::MO_GOTPCREL:                // rip-relative GOT reference.
  case McasmII::MO_GOTPCREL_NORELAX:        // rip-relative GOT reference.
  case McasmII::MO_GOT:                     // normal GOT reference.
  case McasmII::MO_DARWIN_NONLAZY_PIC_BASE: // Normal $non_lazy_ptr ref.
  case McasmII::MO_DARWIN_NONLAZY:          // Normal $non_lazy_ptr ref.
  case McasmII::MO_COFFSTUB:                // COFF .refptr stub.
    return true;
  default:
    return false;
  }
...
/// reference is relative to a 32-bit PIC base (McasmISD::GlobalBaseReg).  If this
/// is true, the addressing mode has the PIC base register added in (e.g. EBX).
inline static bool isGlobalRelativeToPICBase(unsigned char TargetFlag) {
  switch (TargetFlag) {
  case McasmII::MO_GOTOFF:                  // isPICStyleGOT: local global.
  case McasmII::MO_GOT:                     // isPICStyleGOT: other global.
  case McasmII::MO_PIC_BASE_OFFSET:         // Darwin local global.
  case McasmII::MO_DARWIN_NONLAZY_PIC_BASE: // Darwin/32 external global.
  case McasmII::MO_TLVP:                    // ??? Pretty sure..
    return true;
  default:
    return false;
  }
...
inline static bool isLeaMem(const MachineInstr &MI, unsigned Op) {
  if (MI.getOperand(Op).isFI())
    return true;
  return Op + Mcasm::AddrSegmentReg <= MI.getNumOperands() &&
         MI.getOperand(Op + Mcasm::AddrBaseReg).isReg() &&
         isScale(MI.getOperand(Op + Mcasm::AddrScaleAmt)) &&
         MI.getOperand(Op + Mcasm::AddrIndexReg).isReg() &&
         (MI.getOperand(Op + Mcasm::AddrDisp).isImm() ||
          MI.getOperand(Op + Mcasm::AddrDisp).isGlobal() ||
          MI.getOperand(Op + Mcasm::AddrDisp).isCPI() ||
          MI.getOperand(Op + Mcasm::AddrDisp).isJTI());
}

inline static bool isMem(const MachineInstr &MI, unsigned Op) {
  if (MI.getOperand(Op).isFI())
    return true;
  return Op + Mcasm::AddrNumOperands <= MI.getNumOperands() &&
         MI.getOperand(Op + Mcasm::AddrSegmentReg).isReg() && isLeaMem(MI, Op);
}

inline static bool isAddMemInstrWithRelocation(const MachineInstr &MI) {
  unsigned Op = MI.getOpcode();
  if (Op == Mcasm::ADD64rm || Op == Mcasm::ADD64mr_ND || Op == Mcasm::ADD64rm_ND) {
...
inline static bool isMemInstrWithGOTPCREL(const MachineInstr &MI) {
  ...
  case Mcasm::XOR64rm: {
    int MemOpNo = McasmII::getMemoryOperandNo(MI.getDesc().TSFlags) +
                  McasmII::getOperandBias(MI.getDesc());
    const MachineOperand &MO = MI.getOperand(Mcasm::AddrDisp + MemOpNo);
    if (MO.getTargetFlags() == McasmII::MO_GOTPCREL)
      return true;
    break;
  }
  }
...
class McasmInstrInfo final : public McasmGenInstrInfo {
...
public:
  ...
  /// source to overlap the destination. e.g. Mcasm::MOVSX64rr32. If this returns
...
#include "McasmGenInstrInfo.inc"
  ...
  static bool hasLockPrefix(const MachineInstr &MI) {
    return MI.getDesc().TSFlags & McasmII::LOCK;
  }
...
protected:
  ...
  /// For Mcasm instruction (vpmaddwd + vpmaddwd) -> vpdpwssd, the vpmaddwd
