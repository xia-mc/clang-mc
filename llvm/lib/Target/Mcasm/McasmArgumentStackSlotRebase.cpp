//===---- McasmArgumentStackSlotRebase.cpp - rebase argument stack slot -----===//
//
...
class McasmArgumentStackSlotLegacy : public MachineFunctionPass {

public:
  static char ID; // Pass identification, replacement for typeid

  explicit McasmArgumentStackSlotLegacy() : MachineFunctionPass(ID) {}
...
char McasmArgumentStackSlotLegacy::ID = 0;

INITIALIZE_PASS(McasmArgumentStackSlotLegacy, DEBUG_TYPE, "Argument Stack Rebase",
                false, false)

FunctionPass *llvm::createMcasmArgumentStackSlotLegacyPass() {
  return new McasmArgumentStackSlotLegacy();
}
...
static Register getArgBaseReg(MachineFunction &MF) {
  ...
  case CallingConv::C:
    RC = STI.is64Bit() ? &Mcasm::GR64_ArgRefRegClass : &Mcasm::GR32_ArgRefRegClass;
    break;
  case CallingConv::Mcasm_RegCall:
    ...
    // pop       ebx
    // ret
    RC = STI.is64Bit() ? &Mcasm::GR64_ArgRefRegClass : nullptr;
    break;
...
static bool argumentStackSlotRebase(MachineFunction &MF) {
  ...
  const McasmSubtarget &STI = MF.getSubtarget<McasmSubtarget>();
  const McasmRegisterInfo *TRI = STI.getRegisterInfo();
  ...
  MachineInstr *LEA =
      BuildMI(MBB, MBBI, DL,
              TII->get(STI.is64Bit() ? Mcasm::PLEA64r : Mcasm::PLEA32r), ArgBaseReg)
          .addFrameIndex(FI)
          .addImm(1)
...
bool McasmArgumentStackSlotLegacy::runOnMachineFunction(MachineFunction &MF) {
  return argumentStackSlotRebase(MF);
}

PreservedAnalyses
McasmArgumentStackSlotPass::run(MachineFunction &MF,
