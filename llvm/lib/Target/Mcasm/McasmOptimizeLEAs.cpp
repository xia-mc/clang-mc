//===- McasmOptimizeLEAs.cpp - optimize usage of LEA instructions -----------===//
//
...
static cl::opt<bool>
    DisableMcasmLEAOpt("disable-x86-lea-opt", cl::Hidden,
                     cl::desc("Mcasm: Disable LEA optimizations."),
                     cl::init(false));
...
static inline MemOpKey getMemOpKey(const MachineInstr &MI, unsigned N) {
  ...
  return MemOpKey(&MI.getOperand(N + Mcasm::AddrBaseReg),
                  &MI.getOperand(N + Mcasm::AddrScaleAmt),
                  &MI.getOperand(N + Mcasm::AddrIndexReg),
                  &MI.getOperand(N + Mcasm::AddrSegmentReg),
                  &MI.getOperand(N + Mcasm::AddrDisp));
}
...
static inline bool isLEA(const MachineInstr &MI) {
  ...
  return Opcode == Mcasm::LEA16r || Opcode == Mcasm::LEA32r ||
         Opcode == Mcasm::LEA64r || Opcode == Mcasm::LEA64_32r;
}

namespace {

class McasmOptimizeLEAsImpl {
public:
...
class McasmOptimizeLEAsLegacy : public MachineFunctionPass {
public:
  McasmOptimizeLEAsLegacy() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override { return "Mcasm LEA Optimize"; }
...
char McasmOptimizeLEAsLegacy::ID = 0;

FunctionPass *llvm::createMcasmOptimizeLEAsLegacyPass() {
  return new McasmOptimizeLEAsLegacy();
}
INITIALIZE_PASS(McasmOptimizeLEAsLegacy, DEBUG_TYPE, "Mcasm optimize LEA pass",
                false, false)

int McasmOptimizeLEAsImpl::calcInstrDist(const MachineInstr &First,
...
//    possible.
bool McasmOptimizeLEAsImpl::chooseBestLEA(
    const SmallVectorImpl<MachineInstr *> &List, const MachineInstr &MI,
    MachineInstr *&BestLEA, int64_t &AddrDispShift, int &Dist) {
  const MCInstrDesc &Desc = MI.getDesc();
  int MemOpNo = McasmII::getMemoryOperandNo(Desc.TSFlags) +
                McasmII::getOperandBias(Desc);
  ...
  for (auto *DefMI : List) {
    ...
    if (TII->getRegClass(Desc, MemOpNo + Mcasm::AddrBaseReg) !=
...
int64_t McasmOptimizeLEAsImpl::getAddrDispShift(const MachineInstr &MI1,
                                              unsigned N1,
                                              const MachineInstr &MI2,
                                              unsigned N2) const {
  const MachineOperand &Op1 = MI1.getOperand(N1 + Mcasm::AddrDisp);
  const MachineOperand &Op2 = MI2.getOperand(N2 + Mcasm::AddrDisp);
...
bool McasmOptimizeLEAsImpl::isReplaceable(const MachineInstr &First,
                                        const MachineInstr &Last,
                                        int64_t &AddrDispShift) const {
...
void McasmOptimizeLEAsImpl::findLEAs(const MachineBasicBlock &MBB,
                                   MemOpMap &LEAs) {
  unsigned Pos = 0;
  for (auto &MI : MBB) {
...
// register.
bool McasmOptimizeLEAsImpl::removeRedundantAddrCalc(MemOpMap &LEAs) {
  bool Changed = false;
  ...
  for (MachineInstr &MI : llvm::make_early_inc_range(*MBB)) {
    ...
    MI.getOperand(MemOpNo + Mcasm::AddrBaseReg)
        .ChangeToRegister(DefMI->getOperand(0).getReg(), false);
    MI.getOperand(MemOpNo + Mcasm::AddrScaleAmt).ChangeToImmediate(1);
    MI.getOperand(MemOpNo + Mcasm::AddrIndexReg)
        .ChangeToRegister(Mcasm::NoRegister, false);
    MI.getOperand(MemOpNo + Mcasm::AddrDisp).ChangeToImmediate(AddrDispShift);
    MI.getOperand(MemOpNo + Mcasm::AddrSegmentReg)
        .ChangeToRegister(Mcasm::NoRegister, false);
...
MachineInstr *McasmOptimizeLEAsImpl::replaceDebugValue(MachineInstr &MI,
                                                     Register OldReg,
                                                     Register NewReg,
                                                     int64_t AddrDispShift) {
...
bool McasmOptimizeLEAsImpl::removeRedundantLEAs(MemOpMap &LEAs) {
  bool Changed = false;
    ...
    while (I1 != List.end()) {
      ...
      while (I2 != List.end()) {
        ...
        while (!MRI->use_empty(LastVReg)) {
          ...
          const MCInstrDesc &Desc = MI.getDesc();
          int MemOpNo =
              McasmII::getMemoryOperandNo(Desc.TSFlags) +
              McasmII::getOperandBias(Desc);
          ...
          // Update address disp.
          MachineOperand &Op = MI.getOperand(MemOpNo + Mcasm::AddrDisp);
          if (Op.isImm())
...
bool McasmOptimizeLEAsImpl::runOnMachineFunction(
    MachineFunction &MF, ProfileSummaryInfo *PSI,
    MachineBlockFrequencyInfo *MBFI) {
  bool Changed = false;

  if (DisableMcasmLEAOpt)
    return false;
...
bool McasmOptimizeLEAsLegacy::runOnMachineFunction(MachineFunction &MF) {
  ...
  MachineBlockFrequencyInfo *MBFI =
      (PSI && PSI->hasProfileSummary())
          ...
          : nullptr;
  McasmOptimizeLEAsImpl PassImpl;
  return PassImpl.runOnMachineFunction(MF, PSI, MBFI);
}

PreservedAnalyses
McasmOptimizeLEAsPass::run(MachineFunction &MF,
