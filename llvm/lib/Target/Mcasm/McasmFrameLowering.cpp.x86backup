//===-- McasmFrameLowering.cpp - Mcasm Frame Information ----------------------===//
//
...
bool McasmFrameLowering::hasReservedCallFrame(const MachineFunction &MF) const {
  return !MF.getFrameInfo().hasVarSizedObjects() &&
         !MF.getInfo<McasmMachineFunctionInfo>()->getHasPushSequences() &&
         !MF.getInfo<McasmMachineFunctionInfo>()->hasPreallocatedCall();
}
...
bool McasmFrameLowering::hasFPImpl(const MachineFunction &MF) const {
  ...
  return (MF.getTarget().Options.DisableFramePointerElim(MF) ||
          ...
          MF.getInfo<McasmMachineFunctionInfo>()->getForceFramePointer() ||
          MF.getInfo<McasmMachineFunctionInfo>()->hasPreallocatedCall() ||
...
static unsigned getSUBriOpcode(bool IsLP64) {
  return IsLP64 ? Mcasm::SUB64ri32 : Mcasm::SUB32ri;
}

static unsigned getADDriOpcode(bool IsLP64) {
  return IsLP64 ? Mcasm::ADD64ri32 : Mcasm::ADD32ri;
}

static unsigned getSUBrrOpcode(bool IsLP64) {
  return IsLP64 ? Mcasm::SUB64rr : Mcasm::SUB32rr;
}

static unsigned getADDrrOpcode(bool IsLP64) {
  return IsLP64 ? Mcasm::ADD64rr : Mcasm::ADD32rr;
}

static unsigned getANDriOpcode(bool IsLP64, int64_t Imm) {
  return IsLP64 ? Mcasm::AND64ri32 : Mcasm::AND32ri;
}

static unsigned getLEArOpcode(bool IsLP64) {
  return IsLP64 ? Mcasm::LEA64r : Mcasm::LEA32r;
}

static unsigned getMOVriOpcode(bool Use64BitReg, int64_t Imm) {
  if (Use64BitReg) {
    if (isUInt<32>(Imm))
      return Mcasm::MOV32ri64;
    if (isInt<32>(Imm))
      return Mcasm::MOV64ri32;
    return Mcasm::MOV64ri;
  }
  return Mcasm::MOV32ri;
}
...
static unsigned getPUSHOpcode(const McasmSubtarget &ST) {
  return ST.is64Bit() ? (ST.hasPPX() ? Mcasm::PUSHP64r : Mcasm::PUSH64r)
                      : Mcasm::PUSH32r;
}
static unsigned getPOPOpcode(const McasmSubtarget &ST) {
  return ST.is64Bit() ? (ST.hasPPX() ? Mcasm::POPP64r : Mcasm::POP64r)
                      : Mcasm::POP32r;
}
static unsigned getPUSH2Opcode(const McasmSubtarget &ST) {
  return ST.hasPPX() ? Mcasm::PUSH2P : Mcasm::PUSH2;
}
static unsigned getPOP2Opcode(const McasmSubtarget &ST) {
  return ST.hasPPX() ? Mcasm::POP2P : Mcasm::POP2;
}
...
static bool isEAXLiveIn(MachineBasicBlock &MBB) {
  for (MachineBasicBlock::RegisterMaskPair RegMask : MBB.liveins()) {
    ...
    if (Reg == Mcasm::RAX || Reg == Mcasm::EAX || Reg == Mcasm::AX ||
        Reg == Mcasm::AH || Reg == Mcasm::AL)
      return true;
  }
...
void McasmFrameLowering::emitSPUpdate(MachineBasicBlock &MBB,
  ...
  const McasmTargetLowering &TLI = *STI.getTargetLowering();
  ...
  if (EmitInlineStackProbe && !InEpilogue) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(Mcasm::STACKALLOC_W_PROBING)).addImm(Offset);
    return;
  } else if (Offset > MaxSPChunk) {
    // Rather than emit a long series of instructions for large offsets,
    // load the offset into a register and do one sub/add
    unsigned Reg = 0;
    unsigned Rax = (unsigned)(Uses64BitFramePtr ? Mcasm::RAX : Mcasm::EAX);

    if (isSub && !isEAXLiveIn(MBB))
      Reg = Rax;
    else
      Reg = getMcasmSubSuperRegister(TRI->findDeadCallerSavedReg(MBB, MBBI),
    ...
    if (Reg) {
      ...
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::PUSH64r))
      ...
      BuildMI(MBB, MBBI, DL, TII.get(getMOVriOpcode(Uses64BitFramePtr, Offset)),
          ...
          .addImm(Offset)
          .setMIFlag(Flag);
      MachineInstr *MI = BuildMI(MBB, MBBI, DL, TII.get(Mcasm::ADD64rr), Rax)
                             .addReg(Rax)
                             .addReg(StackPtr);
      MI->getOperand(3).setIsDead(); // The EFLAGS implicit def is dead.
      // Exchange the new SP in RAX with the top of the stack.
      addRegOffset(
          BuildMI(MBB, MBBI, DL, TII.get(Mcasm::XCHG64rm), Rax).addReg(Rax),
          StackPtr, false, 0);
      // Load new SP from the top of the stack into RSP.
      addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV64rm), StackPtr),
                   StackPtr, false, 0);
      return;
    }
  }
  ...
  while (Offset) {
    if (Offset == SlotSize) {
      ...
      unsigned Reg = isSub ? (unsigned)(Is64Bit ? Mcasm::RAX : Mcasm::EAX)
                           : TRI->findDeadCallerSavedReg(MBB, MBBI);
      if (Reg) {
        unsigned Opc = isSub ? (Is64Bit ? Mcasm::PUSH64r : Mcasm::PUSH32r)
                             : (Is64Bit ? Mcasm::POP64r : Mcasm::POP32r);
...
MachineInstrBuilder McasmFrameLowering::BuildStackAdjustment(
  ...
  if (!InEpilogue) {
    ...
    UseLEA = STI.useLeaForSP() || MBB.isLiveIn(Mcasm::EFLAGS);
  } else {
...
int64_t McasmFrameLowering::mergeSPUpdates(MachineBasicBlock &MBB,
                                         MachineBasicBlock::iterator &MBBI,
  ...
  */
  ...
  for (;;) {
    ...
    if ((Opc == Mcasm::ADD64ri32 || Opc == Mcasm::ADD32ri) &&
        PI->getOperand(0).getReg() == StackPtr) {
      assert(PI->getOperand(1).getReg() == StackPtr);
      Offset = PI->getOperand(2).getImm();
    } else if ((Opc == Mcasm::LEA32r || Opc == Mcasm::LEA64_32r) &&
               PI->getOperand(0).getReg() == StackPtr &&
               PI->getOperand(1).getReg() == StackPtr &&
               PI->getOperand(2).getImm() == 1 &&
               PI->getOperand(3).getReg() == Mcasm::NoRegister &&
               PI->getOperand(5).getReg() == Mcasm::NoRegister) {
      // For LEAs we have: def = lea SP, FI, noreg, Offset, noreg.
      Offset = PI->getOperand(4).getImm();
    } else if ((Opc == Mcasm::SUB64ri32 || Opc == Mcasm::SUB32ri) &&
               PI->getOperand(0).getReg() == StackPtr) {
      assert(PI->getOperand(1).getReg() == StackPtr);
      Offset = -PI->getOperand(2).getImm();
    } else
...
int64_t McasmFrameLowering::mergeSPAdd(MachineBasicBlock &MBB,
                                     MachineBasicBlock::iterator &MBBI,
                                     int64_t AddOffset,
                                     bool doMergeWithPrevious) const {
...
void McasmFrameLowering::BuildCFI(MachineBasicBlock &MBB,
                                MachineBasicBlock::iterator MBBI,
                                const DebugLoc &DL,
  ...
  if (CFIInst.getOperation() == MCCFIInstruction::OpAdjustCfaOffset)
    MF.getInfo<McasmMachineFunctionInfo>()->setHasCFIAdjustCfa(true);
...
void McasmFrameLowering::emitCalleeSavedFrameMovesFullCFA(
  ...
  const Register MachineFramePtr =
      STI.isTarget64BitILP32() ? Register(getMcasmSubSuperRegister(FramePtr, 64))
                               : FramePtr;
...
void McasmFrameLowering::emitCalleeSavedFrameMoves(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
      ...
      if (McasmFI->getStackPtrSaveMI()) {
        ...
        const Register MachineFramePtr =
            STI.isTarget64BitILP32()
                ? Register(getMcasmSubSuperRegister(FramePtr, 64))
                : FramePtr;
  ...
  if (auto *MI = McasmFI->getStackPtrSaveMI()) {
...
void McasmFrameLowering::emitZeroCallUsedRegs(BitVector RegsToZero,
  ...
  for (MCRegister Reg : RegsToZero.set_bits()) {
    if (!Mcasm::RFP80RegClass.contains(Reg))
      continue;

    unsigned NumFPRegs = ST.is64Bit() ? 8 : 7;
    for (unsigned i = 0; i != NumFPRegs; ++i)
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::LD_F0));

    for (unsigned i = 0; i != NumFPRegs; ++i)
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::ST_FPrr)).addReg(Mcasm::ST0);
    break;
  }
  ...
  for (MCRegister Reg : RegsToZero.set_bits())
    if (TRI->isGeneralPurposeRegister(MF, Reg)) {
      GPRsToZero.set(getMcasmSubSuperRegister(Reg, 32));
...
void McasmFrameLowering::emitStackProbe(
  ...
  if (STI.isTargetWindowsCoreCLR()) {
    if (InProlog) {
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::STACKALLOC_W_PROBING))
...
bool McasmFrameLowering::stackProbeFunctionModifiesSP() const {
  return STI.isOSWindows() && !STI.isTargetWin64();
}

void McasmFrameLowering::inlineStackProbe(MachineFunction &MF,
                                        MachineBasicBlock &PrologMBB) const {
  auto Where = llvm::find_if(PrologMBB, [](MachineInstr &MI) {
    return MI.getOpcode() == Mcasm::STACKALLOC_W_PROBING;
  });
...
void McasmFrameLowering::emitStackProbeInline(MachineFunction &MF,
...
void McasmFrameLowering::emitStackProbeInlineGeneric(
    MachineFunction &MF, MachineBasicBlock &MBB,
...
void McasmFrameLowering::emitStackProbeInlineGenericBlock(
    MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  const McasmSubtarget &STI = MF.getSubtarget<McasmSubtarget>();
  const McasmTargetLowering &TLI = *STI.getTargetLowering();
  const unsigned MovMIOpc = Is64Bit ? Mcasm::MOV64mi32 : Mcasm::MOV32mi;
  const uint64_t StackProbeSize = TLI.getStackProbeSize(MF);
  ...
  if (ChunkSize == SlotSize) {
    ...
    unsigned Reg = Is64Bit ? Mcasm::RAX : Mcasm::EAX;
    unsigned Opc = Is64Bit ? Mcasm::PUSH64r : Mcasm::PUSH32r;
    BuildMI(MBB, MBBI, DL, TII.get(Opc))
...
void McasmFrameLowering::emitStackProbeInlineGenericLoop(
    MachineFunction &MF, MachineBasicBlock &MBB,
    MachineBasicBlock::iterator MBBI, const DebugLoc &DL, uint64_t Offset,
    uint64_t AlignOffset) const {
  assert(Offset && "null offset");

  assert(MBB.computeRegisterLiveness(TRI, Mcasm::EFLAGS, MBBI) !=
  ...
  Register FinalStackProbed = Uses64BitFramePtr ? Mcasm::R11
                              : Is64Bit         ? Mcasm::R11D
                                                : Mcasm::EAX;
  ...
  // save loop bound
  {
    ...
    if (canUseSub) {
      ...
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV64ri), FinalStackProbed)
          .addImm(-BoundOffset)
          .setMIFlag(MachineInstr::FrameSetup);
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::ADD64rr), FinalStackProbed)
          .addReg(FinalStackProbed)
          .addReg(StackPtr)
          .setMIFlag(MachineInstr::FrameSetup);
    } else {
    ...
    if (!HasFP && NeedsDwarfCFI) {
      ...
      const Register DwarfFinalStackProbed =
          STI.isTarget64BitILP32()
              ? Register(getMcasmSubSuperRegister(FinalStackProbed, 64))
              : FinalStackProbed;
  ...
  BuildMI(testMBB, DL, TII.get(Uses64BitFramePtr ? Mcasm::CMP64rr : Mcasm::CMP32rr))
      .addReg(StackPtr)
      .addReg(FinalStackProbed)
      .setMIFlag(MachineInstr::FrameSetup);

  // jump
  BuildMI(testMBB, DL, TII.get(Mcasm::JCC_1))
      .addMBB(testMBB)
      .addImm(Mcasm::COND_NE)
      .setMIFlag(MachineInstr::FrameSetup);
  ...
  if (!HasFP && NeedsDwarfCFI) {
    ...
    const Register DwarfStackPtr =
        STI.isTarget64BitILP32()
            ? Register(getMcasmSubSuperRegister(StackPtr, 64))
            : Register(StackPtr);
...
void McasmFrameLowering::emitStackProbeInlineWindowsCoreCLR64(
    MachineFunction &MF, MachineBasicBlock &MBB,
    MachineBasicBlock::iterator MBBI, const DebugLoc &DL, bool InProlog) const {
  const McasmSubtarget &STI = MF.getSubtarget<McasmSubtarget>();
  ...
  const TargetRegisterClass *RegClass = &Mcasm::GR64RegClass;
  const Register
      SizeReg = InProlog ? Mcasm::RAX : MRI.createVirtualRegister(RegClass),
      ZeroReg = InProlog ? Mcasm::RCX : MRI.createVirtualRegister(RegClass),
      CopyReg = InProlog ? Mcasm::RDX : MRI.createVirtualRegister(RegClass),
      TestReg = InProlog ? Mcasm::RDX : MRI.createVirtualRegister(RegClass),
      FinalReg = InProlog ? Mcasm::RDX : MRI.createVirtualRegister(RegClass),
      RoundedReg = InProlog ? Mcasm::RDX : MRI.createVirtualRegister(RegClass),
      LimitReg = InProlog ? Mcasm::RCX : MRI.createVirtualRegister(RegClass),
      JoinReg = InProlog ? Mcasm::RCX : MRI.createVirtualRegister(RegClass),
      ProbeReg = InProlog ? Mcasm::RCX : MRI.createVirtualRegister(RegClass);
  ...
  if (InProlog) {
    ...
    McasmMachineFunctionInfo *McasmFI = MF.getInfo<McasmMachineFunctionInfo>();
    const int64_t CalleeSaveSize = McasmFI->getCalleeSavedFrameSize();
    const bool HasFP = hasFP(MF);
    ...
    const bool IsRCXLiveIn = MBB.isLiveIn(Mcasm::RCX);
    const bool IsRDXLiveIn = MBB.isLiveIn(Mcasm::RDX);
    ...
    if (IsRCXLiveIn)
      addRegOffset(BuildMI(&MBB, DL, TII.get(Mcasm::MOV64mr)), Mcasm::RSP, false,
                   RCXShadowSlot)
          .addReg(Mcasm::RCX);
    if (IsRDXLiveIn)
      addRegOffset(BuildMI(&MBB, DL, TII.get(Mcasm::MOV64mr)), Mcasm::RSP, false,
    ...
    BuildMI(&MBB, DL, TII.get(Mcasm::MOV64rr), SizeReg).addReg(Mcasm::RAX);
  }

  // Add code to MBB to check for overflow and set the new target stack pointer
  // to zero if so.
  BuildMI(&MBB, DL, TII.get(Mcasm::XOR64rr), ZeroReg)
      .addReg(ZeroReg, RegState::Undef)
      .addReg(ZeroReg, RegState::Undef);
  BuildMI(&MBB, DL, TII.get(Mcasm::MOV64rr), CopyReg).addReg(Mcasm::RSP);
  BuildMI(&MBB, DL, TII.get(Mcasm::SUB64rr), TestReg)
      .addReg(CopyReg)
      .addReg(SizeReg);
  BuildMI(&MBB, DL, TII.get(Mcasm::CMOV64rr), FinalReg)
      .addReg(TestReg)
      .addReg(ZeroReg)
      .addImm(Mcasm::COND_B);
  ...
  BuildMI(&MBB, DL, TII.get(Mcasm::MOV64rm), LimitReg)
      .addReg(0)
      .addImm(1)
      .addReg(0)
      .addImm(ThreadEnvironmentStackLimit)
      .addReg(Mcasm::GS);
  BuildMI(&MBB, DL, TII.get(Mcasm::CMP64rr)).addReg(FinalReg).addReg(LimitReg);
  // Jump if the desired stack pointer is at or above the stack limit.
  BuildMI(&MBB, DL, TII.get(Mcasm::JCC_1))
      .addMBB(ContinueMBB)
      .addImm(Mcasm::COND_AE);

  // Add code to roundMBB to round the final stack pointer to a page boundary.
  if (InProlog)
    RoundMBB->addLiveIn(FinalReg);
  BuildMI(RoundMBB, DL, TII.get(Mcasm::AND64ri32), RoundedReg)
      .addReg(FinalReg)
      .addImm(PageMask);
  BuildMI(RoundMBB, DL, TII.get(Mcasm::JMP_1)).addMBB(LoopMBB);
  ...
  if (!InProlog) {
    BuildMI(LoopMBB, DL, TII.get(Mcasm::PHI), JoinReg)
        .addReg(LimitReg)
        .addMBB(RoundMBB)
        .addReg(ProbeReg)
        .addMBB(LoopMBB);
  ...
  addRegOffset(BuildMI(LoopMBB, DL, TII.get(Mcasm::LEA64r), ProbeReg), JoinReg,
               false, -PageSize);

  // Probe by storing a byte onto the stack.
  BuildMI(LoopMBB, DL, TII.get(Mcasm::MOV8mi))
      .addReg(ProbeReg)
      .addImm(1)
      .addReg(0)
      .addImm(0)
  ...
  BuildMI(LoopMBB, DL, TII.get(Mcasm::CMP64rr))
      .addReg(RoundedReg)
      .addReg(ProbeReg);
  BuildMI(LoopMBB, DL, TII.get(Mcasm::JCC_1))
      .addMBB(LoopMBB)
      .addImm(Mcasm::COND_NE);
  ...
  if (InProlog) {
    if (RCXShadowSlot) // It means we spilled RCX in the prologue.
      addRegOffset(BuildMI(*ContinueMBB, ContinueMBBI, DL,
                           TII.get(Mcasm::MOV64rm), Mcasm::RCX),
                   Mcasm::RSP, false, RCXShadowSlot);
    if (RDXShadowSlot) // It means we spilled RDX in the prologue.
      addRegOffset(BuildMI(*ContinueMBB, ContinueMBBI, DL,
                           TII.get(Mcasm::MOV64rm), Mcasm::RDX),
                   Mcasm::RSP, false, RDXShadowSlot);
  }

  // Now that the probing is done, add code to continueMBB to update
  // the stack pointer for real.
  BuildMI(*ContinueMBB, ContinueMBBI, DL, TII.get(Mcasm::SUB64rr), Mcasm::RSP)
      .addReg(Mcasm::RSP)
      .addReg(SizeReg);
...
void McasmFrameLowering::emitStackProbeCall(
    MachineFunction &MF, MachineBasicBlock &MBB,
    MachineBasicBlock::iterator MBBI, const DebugLoc &DL, bool InProlog,
    std::optional<MachineFunction::DebugInstrOperandPair> InstrNum) const {
  ...
  unsigned CallOp;
  if (Is64Bit)
    CallOp = IsLargeCodeModel ? Mcasm::CALL64r : Mcasm::CALL64pcrel32;
  else
    CallOp = Mcasm::CALLpcrel32;
  ...
  if (Is64Bit && MF.getTarget().getCodeModel() == CodeModel::Large) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV64ri), Mcasm::R11)
        .addExternalSymbol(MF.createExternalSymbolName(Symbol));
    CI = BuildMI(MBB, MBBI, DL, TII.get(CallOp)).addReg(Mcasm::R11);
  } else {
    CI = BuildMI(MBB, MBBI, DL, TII.get(CallOp))
             .addExternalSymbol(MF.createExternalSymbolName(Symbol));
  }

  unsigned AX = Uses64BitFramePtr ? Mcasm::RAX : Mcasm::EAX;
  unsigned SP = Uses64BitFramePtr ? Mcasm::RSP : Mcasm::ESP;
  CI.addReg(AX, RegState::Implicit)
      .addReg(SP, RegState::Implicit)
      .addReg(AX, RegState::Define | RegState::Implicit)
      .addReg(SP, RegState::Define | RegState::Implicit)
      .addReg(Mcasm::EFLAGS, RegState::Define | RegState::Implicit);
...
uint64_t
McasmFrameLowering::calculateMaxStackAlign(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  ...
  if (!Is64Bit && MF.getFunction().getCallingConv() == CallingConv::Mcasm_INTR) {
    if (HasRealign)
...
void McasmFrameLowering::BuildStackAlignAND(MachineBasicBlock &MBB,
                                          MachineBasicBlock::iterator MBBI,
  ...
  if (Reg == StackPtr && EmitInlineStackProbe && MaxAlign >= StackProbeSize) {
    {
      ...
      // Setup entry block
      {
        ...
        BuildMI(entryMBB, DL,
                TII.get(Uses64BitFramePtr ? Mcasm::CMP64rr : Mcasm::CMP32rr))
            .addReg(FinalStackProbed)
            .addReg(StackPtr)
            .setMIFlag(MachineInstr::FrameSetup);
        BuildMI(entryMBB, DL, TII.get(Mcasm::JCC_1))
            .addMBB(&MBB)
            .addImm(Mcasm::COND_E)
            .setMIFlag(MachineInstr::FrameSetup);
...

      {
        ...
        BuildMI(headMBB, DL, TII.get(Mcasm::JCC_1))
            .addMBB(footMBB)
            .addImm(Mcasm::COND_B)
            .setMIFlag(MachineInstr::FrameSetup);
      ...
      // setup loop body
      {
        ...
        BuildMI(bodyMBB, DL, TII.get(Mcasm::JCC_1))
            .addMBB(bodyMBB)
            .addImm(Mcasm::COND_B)
            .setMIFlag(MachineInstr::FrameSetup);
...
bool McasmFrameLowering::has128ByteRedZone(const MachineFunction &MF) const {
  // x86-64 (non Win64) has a 128 byte red zone which is guaranteed not to be
  // clobbered by any interrupt handler.
  assert(&STI == &MF.getSubtarget<McasmSubtarget>() &&
...
/// directives).
bool McasmFrameLowering::isWin64Prologue(const MachineFunction &MF) const {
  return MF.getTarget().getMCAsmInfo()->usesWindowsCFI();
}

bool McasmFrameLowering::needsDwarfCFI(const MachineFunction &MF) const {
...
static bool isOpcodeRep(unsigned Opcode) {
  switch (Opcode) {
  case Mcasm::REPNE_PREFIX:
  case Mcasm::REP_MOVSB_32:
  case Mcasm::REP_MOVSB_64:
  case Mcasm::REP_MOVSD_32:
  case Mcasm::REP_MOVSD_64:
  case Mcasm::REP_MOVSQ_32:
  case Mcasm::REP_MOVSQ_64:
  case Mcasm::REP_MOVSW_32:
  case Mcasm::REP_MOVSW_64:
  case Mcasm::REP_PREFIX:
  case Mcasm::REP_STOSB_32:
  case Mcasm::REP_STOSB_64:
  case Mcasm::REP_STOSD_32:
  case Mcasm::REP_STOSD_64:
  case Mcasm::REP_STOSQ_32:
  case Mcasm::REP_STOSQ_64:
  case Mcasm::REP_STOSW_32:
  case Mcasm::REP_STOSW_64:
    return true;
  default:
    break;
  }
...
void McasmFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (auto *MI = McasmFI->getStackPtrSaveMI()) {
    // MI is lea instruction that created in McasmArgumentStackSlotPass.
    ...
    // andl    $-128, %esp
    // pushl   -4(%basereg)
    BuildMI(MBB, MBBI, DL, TII.get(Is64Bit ? Mcasm::LEA64r : Mcasm::LEA32r),
            ArgBaseReg)
        .addUse(StackPtr)
        .addImm(1)
        .addUse(Mcasm::NoRegister)
        .addImm(SlotSize)
        .addUse(Mcasm::NoRegister)
        .setMIFlag(MachineInstr::FrameSetup);
    if (NeedsDwarfCFI) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(Is64Bit ? Mcasm::PUSH64rmm : Mcasm::PUSH32rmm))
        .addReg(ArgBaseReg)
        .addImm(1)
  ...
  // tail call.
  unsigned TailCallArgReserveSize = -McasmFI->getTCReturnAddrDelta();
  ...
  if (HasFP && McasmFI->hasSwiftAsyncContext()) {
    ...
    case SwiftAsyncFramePointerMode::DeploymentBased:
      if (STI.swiftAsyncContextIsDynamicallySet()) {
        ...
        BuildMI(MBB, MBBI, DL, TII.get(Mcasm::OR64rm), MachineFramePtr)
    ...
    case SwiftAsyncFramePointerMode::Always:
      ...
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::BTS64ri8), MachineFramePtr)
          .addUse(MachineFramePtr)
          .addImm(60)
          .setMIFlag(MachineInstr::FrameSetup);
      break;
  ...
  // stack alignment.
  if (Fn.getCallingConv() == CallingConv::Mcasm_INTR && Is64Bit &&
      Fn.arg_size() == 2) {
    StackSize += 8;
    ...
    BuildMI(MBB, MBBI, DL, TII.get(Mcasm::PUSH64r))
        .addReg(Mcasm::RAX, RegState::Undef)
        .setMIFlag(MachineInstr::FrameSetup);
  }
  ...
  if (has128ByteRedZone(MF) && !TRI->hasStackRealignment(MF) &&
    ...
    uint64_t MinSize =
        McasmFI->getCalleeSavedFrameSize() - McasmFI->getTCReturnAddrDelta();
    if (HasFP)
      MinSize += SlotSize;
    McasmFI->setUsesRedZone(MinSize > 0 || StackSize > 0);
  ...
  if (IsClrFunclet)
    Establisher = Uses64BitFramePtr ? Mcasm::RCX : Mcasm::ECX;
  else if (IsFunclet)
    Establisher = Uses64BitFramePtr ? Mcasm::RDX : Mcasm::EDX;

  if (IsWin64Prologue && IsFunclet && !IsClrFunclet) {
    // Immediately spill establisher into the home slot.
    // The runtime cares about this.
    // MOV64mr %rdx, 16(%rsp)
    unsigned MOVmr = Uses64BitFramePtr ? Mcasm::MOV64mr : Mcasm::MOV32mr;
  ...
  if (HasFP) {
    ...
    NumBytes =
        FrameSize - (McasmFI->getCalleeSavedFrameSize() + TailCallArgReserveSize);
    ...
    BuildMI(MBB, MBBI, DL,
            TII.get(getPUSHOpcode(MF.getSubtarget<McasmSubtarget>())))
    ...
    if (NeedsWinCFI) {
      HasWinCFI = true;
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::SEH_PushReg))
          .addImm(FramePtr)
          .setMIFlag(MachineInstr::FrameSetup);
    }

    if (!IsFunclet) {
      if (McasmFI->hasSwiftAsyncContext()) {
        ...
        if (Attrs.hasAttrSomewhere(Attribute::SwiftAsync)) {
          ...
          // could be misused.
          BuildMI(MBB, MBBI, DL, TII.get(Mcasm::PUSH64i32))
              .addImm(0)
              .setMIFlag(MachineInstr::FrameSetup);
        }
        ...
        BuildMI(MBB, MBBI, DL, TII.get(Mcasm::LEA64r), FramePtr)
            .addUse(Mcasm::RSP)
            .addImm(1)
            .addUse(Mcasm::NoRegister)
            .addImm(8)
            .addUse(Mcasm::NoRegister)
            .setMIFlag(MachineInstr::FrameSetup);
        BuildMI(MBB, MBBI, DL, TII.get(Mcasm::SUB64ri32), Mcasm::RSP)
            .addUse(Mcasm::RSP)
            .addImm(8)
            .setMIFlag(MachineInstr::FrameSetup);
      }
      ...
      if (!IsWin64Prologue && !IsFunclet) {
        ...
        if (!McasmFI->hasSwiftAsyncContext())
          BuildMI(MBB, MBBI, DL,
                  TII.get(Uses64BitFramePtr ? Mcasm::MOV64rr : Mcasm::MOV32rr),
                  FramePtr)
              .addReg(StackPtr)
              .setMIFlag(MachineInstr::FrameSetup);
        ...
        if (NeedsWinFPO) {
          ...
          HasWinCFI = true;
          BuildMI(MBB, MBBI, DL, TII.get(Mcasm::SEH_SetFrame))
              .addImm(FramePtr)
              .addImm(0)
              .setMIFlag(MachineInstr::FrameSetup);
        }
    ...
    NumBytes =
        StackSize - (McasmFI->getCalleeSavedFrameSize() + TailCallArgReserveSize);
  }
  ...
  auto IsCSPush = [&](const MachineBasicBlock::iterator &MBBI) {
    ...
    return Opc == Mcasm::PUSH32r || Opc == Mcasm::PUSH64r || Opc == Mcasm::PUSHP64r ||
           Opc == Mcasm::PUSH2 || Opc == Mcasm::PUSH2P;
  };
  ...
  while (IsCSPush(MBBI)) {
    ...
    if (!HasFP && NeedsDwarfCFI) {
      ...
      // register).
      if (Opc == Mcasm::PUSH2 || Opc == Mcasm::PUSH2P)
  ...
  if (!IsWin64Prologue && !IsFunclet && TRI->hasStackRealignment(MF) &&
    ...
    if (NeedsWinCFI) {
      HasWinCFI = true;
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::SEH_StackAlign))
          .addImm(MaxAlign)
          .setMIFlag(MachineInstr::FrameSetup);
    }
  }
  ...
  if (AlignedNumBytes >= StackProbeSize && EmitStackProbeCall) {
    ...
    if (isEAXAlive) {
      if (Is64Bit) {
        // Save RAX
        BuildMI(MBB, MBBI, DL, TII.get(Mcasm::PUSH64r))
            .addReg(Mcasm::RAX, RegState::Kill)
            .setMIFlag(MachineInstr::FrameSetup);
      } else {
        // Save EAX
        BuildMI(MBB, MBBI, DL, TII.get(Mcasm::PUSH32r))
            .addReg(Mcasm::EAX, RegState::Kill)
            .setMIFlag(MachineInstr::FrameSetup);
      }
    }
    ...
    if (Is64Bit) {
      ...
      BuildMI(MBB, MBBI, DL, TII.get(getMOVriOpcode(Is64Bit, Alloc)), Mcasm::RAX)
          .addImm(Alloc)
          .setMIFlag(MachineInstr::FrameSetup);
    } else {
      // Allocate NumBytes-4 bytes on stack in case of isEAXAlive.
      // We'll also use 4 already allocated bytes for EAX.
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV32ri), Mcasm::EAX)
    ...
    if (isEAXAlive) {
      ...
      MachineInstr *MI;
      if (Is64Bit)
        MI = addRegOffset(BuildMI(MF, DL, TII.get(Mcasm::MOV64rm), Mcasm::RAX),
                          StackPtr, false, NumBytes - 8);
      else
        MI = addRegOffset(BuildMI(MF, DL, TII.get(Mcasm::MOV32rm), Mcasm::EAX),
  ...
  if (NeedsWinCFI && NumBytes) {
    HasWinCFI = true;
    BuildMI(MBB, MBBI, DL, TII.get(Mcasm::SEH_StackAlloc))
        .addImm(NumBytes)
        .setMIFlag(MachineInstr::FrameSetup);
  }
  ...
  if (IsFunclet) {
    if (IsClrFunclet) {
      ...
      MBB.addLiveIn(Establisher);
      addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV64rm), Establisher),
      ...
      addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV64mr)), StackPtr,
                   false, PSPSlotOffset)
          .addReg(Establisher)
          .addMemOperand(MF.getMachineMemOperand(
              NoInfo,
  ...
  if (IsWin64Prologue && HasFP) {
    ...
    if (SEHFrameOffset)
      addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(Mcasm::LEA64r), FramePtr),
                   SPOrEstablisher, false, SEHFrameOffset);
    else
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV64rr), FramePtr)
    ...
    if (!MBB.isCleanupFuncletEntry()) {
      ...
      addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV32mr)), FrameReg,
                   false, EHRegOffset)
          .addReg(Mcasm::ESP);
    }
  }
  ...
  while (MBBI != MBB.end() && MBBI->getFlag(MachineInstr::FrameSetup)) {
    ...
    if (NeedsWinCFI) {
      ...
      if (Register Reg = TII.isStoreToStackSlot(FrameInstr, FI)) {
        if (Mcasm::FR64RegClass.contains(Reg)) {
          int Offset;
          ...
          BuildMI(MBB, MBBI, DL, TII.get(Mcasm::SEH_SaveXMM))
              .addImm(Reg)
              .addImm(Offset)
              .setMIFlag(MachineInstr::FrameSetup);
        }
  ...
  if (NeedsWinCFI && HasWinCFI)
    BuildMI(MBB, MBBI, DL, TII.get(Mcasm::SEH_EndPrologue))
        .setMIFlag(MachineInstr::FrameSetup);
  ...
  if (FnHasClrFunclet && !IsFunclet) {
    ...
    addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV64mr)), StackPtr, false,
                 PSPSlotOffset)
        .addReg(StackPtr)
        .addMemOperand(MF.getMachineMemOperand(
  ...
  if (TRI->hasBasePointer(MF)) {
    ...
    unsigned Opc = Uses64BitFramePtr ? Mcasm::MOV64rr : Mcasm::MOV32rr;
    BuildMI(MBB, MBBI, DL, TII.get(Opc), BasePtr)
        .addReg(SPOrEstablisher)
        .setMIFlag(MachineInstr::FrameSetup);
    if (McasmFI->getRestoreBasePointer()) {
      // Stash value of base pointer.  Saving RSP instead of EBP shortens
      // dependence chain. Used by SjLj EH.
      unsigned Opm = Uses64BitFramePtr ? Mcasm::MOV64mr : Mcasm::MOV32mr;
      addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(Opm)), FramePtr, true,
                   McasmFI->getRestoreBasePointerOffset())
          .addReg(SPOrEstablisher)
          .setMIFlag(MachineInstr::FrameSetup);
    }

    if (McasmFI->getHasSEHFramePtrSave() && !IsFunclet) {
      ...
      Register UsedReg;
      int Offset =
          getFrameIndexReference(MF, McasmFI->getSEHFramePtrSaveIndex(), UsedReg)
              .getFixed();
  ...
  if (ArgBaseReg.isValid()) {
    ...
    auto *MI = McasmFI->getStackPtrSaveMI();
    int FI = MI->getOperand(1).getIndex();
    unsigned MOVmr = Is64Bit ? Mcasm::MOV64mr : Mcasm::MOV32mr;
  ...
  // Mcasm Interrupt handling function cannot assume anything about the direction
  ...
  if (Fn.getCallingConv() == CallingConv::Mcasm_INTR) {
    bool NeedsCLD = false;
    ...
    if (NeedsCLD) {
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::CLD))
          .setMIFlag(MachineInstr::FrameSetup);
    }
  }
...
bool McasmFrameLowering::canUseLEAForSPInEpilogue(
    const MachineFunction &MF) const {
...
unsigned
McasmFrameLowering::getPSPSlotOffsetFromSP(const MachineFunction &MF) const {
...
unsigned
McasmFrameLowering::getWinEHFuncletFrameSize(const MachineFunction &MF) const {
  const McasmMachineFunctionInfo *McasmFI = MF.getInfo<McasmMachineFunctionInfo>();
  // This is the size of the pushed CSRs.
  unsigned CSSize = McasmFI->getCalleeSavedFrameSize();
  // This is the size of callee saved XMMs.
  const auto &WinEHXMMSlotInfo = McasmFI->getWinEHXMMSlotInfo();
  unsigned XMMSize =
      WinEHXMMSlotInfo.size() * TRI->getSpillSize(Mcasm::VR128RegClass);
...
static bool isTailCallOpcode(unsigned Opc) {
  return Opc == Mcasm::TCRETURNri || Opc == Mcasm::TCRETURN_WIN64ri ||
         Opc == Mcasm::TCRETURN_HIPE32ri || Opc == Mcasm::TCRETURNdi ||
         Opc == Mcasm::TCRETURNmi || Opc == Mcasm::TCRETURNri64 ||
         Opc == Mcasm::TCRETURNri64_ImpCall || Opc == Mcasm::TCRETURNdi64 ||
         Opc == Mcasm::TCRETURNmi64 || Opc == Mcasm::TCRETURN_WINmi64;
}
...
void McasmFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  Register MachineFramePtr =
      Is64BitILP32 ? Register(getMcasmSubSuperRegister(FramePtr, 64)) : FramePtr;
  ...
  Register ArgBaseReg;
  if (auto *MI = McasmFI->getStackPtrSaveMI()) {
    unsigned Opc = Mcasm::LEA32r;
    Register StackReg = Mcasm::ESP;
    ArgBaseReg = MI->getOperand(0).getReg();
    if (STI.is64Bit()) {
      Opc = Mcasm::LEA64r;
      StackReg = Mcasm::RSP;
  ...
  if (HasFP) {
    ...
    // Pop EBP.
    BuildMI(MBB, MBBI, DL,
            TII.get(getPOPOpcode(MF.getSubtarget<McasmSubtarget>())),
            MachineFramePtr)
        .setMIFlag(MachineInstr::FrameDestroy);
    ...
    if (McasmFI->hasSwiftAsyncContext()) {
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::BTR64ri8), MachineFramePtr)
          .addUse(MachineFramePtr)
          .addImm(60)
          .setMIFlag(MachineInstr::FrameDestroy);
    }
    ...
    if (NeedsDwarfCFI) {
      if (!ArgBaseReg.isValid()) {
        unsigned DwarfStackPtr =
            TRI->getDwarfRegNum(Is64Bit ? Mcasm::RSP : Mcasm::ESP, true);
        BuildCFI(MBB, MBBI, DL,
                 MCCFIInstruction::cfiDefCfa(nullptr, DwarfStackPtr, SlotSize),
                 MachineInstr::FrameDestroy);
      }
  ...
  while (MBBI != MBB.begin()) {
    ...
    if (Opc != Mcasm::DBG_VALUE && !PI->isTerminator()) {
      if (!PI->getFlag(MachineInstr::FrameDestroy) ||
          (Opc != Mcasm::POP32r && Opc != Mcasm::POP64r && Opc != Mcasm::BTR64ri8 &&
           Opc != Mcasm::ADD64ri32 && Opc != Mcasm::POPP64r && Opc != Mcasm::POP2 &&
           Opc != Mcasm::POP2P && Opc != Mcasm::LEA64r))
        break;
      FirstCSPop = PI;
    }
  ...
  if (ArgBaseReg.isValid()) {
    ...
    auto *MI = McasmFI->getStackPtrSaveMI();
    int FI = MI->getOperand(1).getIndex();
    unsigned MOVrm = Is64Bit ? Mcasm::MOV64rm : Mcasm::MOV32rm;
  ...
  if (IsFunclet && Terminator->getOpcode() == Mcasm::CATCHRET)
  ...
  if (NeedsWin64CFI && MF.hasWinCFI())
    BuildMI(MBB, MBBI, DL, TII.get(Mcasm::SEH_BeginEpilogue));
  ...
  if (!HasFP && NeedsDwarfCFI) {
    ...
    while (MBBI != MBB.end()) {
      ...
      unsigned Opc = PI->getOpcode();
      ++MBBI;
      if (Opc == Mcasm::POP32r || Opc == Mcasm::POP64r || Opc == Mcasm::POPP64r ||
          Opc == Mcasm::POP2 || Opc == Mcasm::POP2P) {
        Offset += SlotSize;
        // Compared to pop, pop2 introduces more stack offset (one more
        // register).
        if (Opc == Mcasm::POP2 || Opc == Mcasm::POP2P)
          Offset += SlotSize;
        BuildCFI(MBB, MBBI, DL,
  ...
  if (Terminator == MBB.end() || !isTailCallOpcode(Terminator->getOpcode())) {
    ...
    int64_t Delta = McasmFI->getTCReturnAddrDelta();
  ...
  if (McasmFI->getAMXProgModel() == AMXProgModelEnum::ManagedRA)
    BuildMI(MBB, Terminator, DL, TII.get(Mcasm::TILERELEASE));

  if (NeedsWin64CFI && MF.hasWinCFI())
    BuildMI(MBB, Terminator, DL, TII.get(Mcasm::SEH_EndEpilogue));
}

StackOffset McasmFrameLowering::getFrameIndexReference(const MachineFunction &MF,
                                                     int FI,
  ...
  if (MF.getFunction().getCallingConv() == CallingConv::Mcasm_INTR &&
      Offset >= 0) {
  ...
  if (FrameReg == TRI->getFramePtr()) {
    ...
    int TailCallReturnAddrDelta = McasmFI->getTCReturnAddrDelta();
...
int McasmFrameLowering::getWin64EHFrameIndexRef(const MachineFunction &MF, int FI,
                                              Register &FrameReg) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const McasmMachineFunctionInfo *McasmFI = MF.getInfo<McasmMachineFunctionInfo>();
  const auto &WinEHXMMSlotInfo = McasmFI->getWinEHXMMSlotInfo();
...
StackOffset
McasmFrameLowering::getFrameIndexReferenceSP(const MachineFunction &MF, int FI,
                                           Register &FrameReg,
...
StackOffset
McasmFrameLowering::getFrameIndexReferencePreferSP(const MachineFunction &MF,
  ...
  assert(MF.getInfo<McasmMachineFunctionInfo>()->getTCReturnAddrDelta() >= 0 &&
...
bool McasmFrameLowering::assignCalleeSavedSpillSlots(
  ...
  auto &WinEHXMMSlotInfo = McasmFI->getWinEHXMMSlotInfo();
  int SpillSlotOffset = getOffsetOfLocalArea() + McasmFI->getTCReturnAddrDelta();

  int64_t TailCallReturnAddrDelta = McasmFI->getTCReturnAddrDelta();
  ...
  if (this->TRI->hasBasePointer(MF)) {
    ...
    if (MF.hasEHFunclets()) {
      ...
      McasmFI->setHasSEHFramePtrSave(true);
      McasmFI->setSEHFramePtrSaveIndex(FI);
    }
  }
  ...
  for (CalleeSavedInfo &I : llvm::reverse(CSI)) {
    ...
    if (McasmFI->getNumCandidatesForPush2Pop2() < NumRegsForPush2 &&
        (SpillSlotOffset % 16 == 0 ||
         McasmFI->getNumCandidatesForPush2Pop2() % 2))
      McasmFI->addCandidateForPush2Pop2(Reg);
  ...
  if (McasmFI->getRestoreBasePointer()) {
    ...
    McasmFI->setRestoreBasePointer(CalleeSavedFrameSize);
  }
  assert(McasmFI->getNumCandidatesForPush2Pop2() % 2 == 0 &&
         "Expect even candidates for push2/pop2");
  if (McasmFI->getNumCandidatesForPush2Pop2())
    ++NumFunctionUsingPush2Pop2;
  McasmFI->setCalleeSavedFrameSize(CalleeSavedFrameSize);
  ...
  for (CalleeSavedInfo &I : llvm::reverse(CSI)) {
    ...
    MVT VT = MVT::Other;
    if (Mcasm::VK16RegClass.contains(Reg))
    ...
    // ensure alignment
    assert(SpillSlotOffset < 0 && "SpillSlotOffset should always < 0 on Mcasm");
...
bool McasmFrameLowering::spillCalleeSavedRegisters(
  ...
  if (McasmFI->padForPush2Pop2()) {
    ...
    BuildMI(MBB, MI, DL, TII.get(Mcasm::PUSH64r))
        .addReg(Mcasm::RAX, RegState::Undef)
        .setMIFlag(MachineInstr::FrameSetup);
  }
  ...
  for (auto RI = CSI.rbegin(), RE = CSI.rend(); RI != RE; ++RI) {
    ...
    if (McasmFI->isCandidateForPush2Pop2(Reg)) {
  ...
  if (McasmFI->getRestoreBasePointer()) {
    unsigned Opc = STI.is64Bit() ? Mcasm::PUSH64r : Mcasm::PUSH32r;
  ...
  // Make XMM regs spilled. Mcasm does not have ability of push/pop XMM.
...
void McasmFrameLowering::emitCatchRetReturnValue(MachineBasicBlock &MBB,
                                               MachineBasicBlock::iterator MBBI,
  ...
  if (STI.is64Bit()) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(Mcasm::LEA64r), Mcasm::RAX)
        .addReg(Mcasm::RIP)
        .addImm(0)
        .addReg(0)
...
bool McasmFrameLowering::restoreCalleeSavedRegisters(
  ...
  if (MI != MBB.end() && isFuncletReturnInstr(*MI) && STI.isOSWindows()) {
    ...
    if (MI->getOpcode() == Mcasm::CATCHRET) {
  ...
  const McasmMachineFunctionInfo *McasmFI = MF.getInfo<McasmMachineFunctionInfo>();
  if (McasmFI->getRestoreBasePointer()) {
    unsigned Opc = STI.is64Bit() ? Mcasm::POP64r : Mcasm::POP32r;
    Register BaseReg = this->TRI->getBaseRegister();
...
void McasmFrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  if (TRI->hasBasePointer(MF)) {
    ...
    if (STI.isTarget64BitILP32())
      BasePtr = getMcasmSubSuperRegister(BasePtr, 64);
...
static unsigned GetScratchRegister(bool Is64Bit, bool IsLP64,
  ...
  if (CallingConvention == CallingConv::HiPE) {
    if (Is64Bit)
      return Primary ? Mcasm::R14 : Mcasm::R13;
    else
      return Primary ? Mcasm::EBX : Mcasm::EDI;
  }

  if (Is64Bit) {
    if (IsLP64)
      return Primary ? Mcasm::R11 : Mcasm::R12;
    else
      return Primary ? Mcasm::R11D : Mcasm::R12D;
  }

  bool IsNested = HasNestArgument(&MF);

  if (CallingConvention == CallingConv::Mcasm_FastCall ||
      CallingConvention == CallingConv::Fast ||
      CallingConvention == CallingConv::Tail) {
    if (IsNested)
      report_fatal_error("Segmented stacks does not support fastcall with "
                         "nested function.");
    return Primary ? Mcasm::EAX : Mcasm::ECX;
  }
  if (IsNested)
    return Primary ? Mcasm::EDX : Mcasm::EAX;
  return Primary ? Mcasm::ECX : Mcasm::EAX;
}
...
void McasmFrameLowering::adjustForSegmentedStacks(
  ...
  if (IsNested)
    allocMBB->addLiveIn(IsLP64 ? Mcasm::R10 : Mcasm::R10D);
  ...
  if (Is64Bit) {
    ...
    if (CompareStackPointer)
      ScratchReg = IsLP64 ? Mcasm::RSP : Mcasm::ESP;
    else
      BuildMI(checkMBB, DL, TII.get(IsLP64 ? Mcasm::LEA64r : Mcasm::LEA64_32r),
              ScratchReg)
          .addReg(Mcasm::RSP)
          .addImm(1)
          .addReg(0)
    ...
    BuildMI(checkMBB, DL, TII.get(IsLP64 ? Mcasm::CMP64rm : Mcasm::CMP32rm))
        .addReg(ScratchReg)
        .addReg(0)
        .addImm(1)
        .addReg(0)
    ...
    if (CompareStackPointer)
      ScratchReg = Mcasm::ESP;
    else
      BuildMI(checkMBB, DL, TII.get(Mcasm::LEA32r), ScratchReg)
          .addReg(Mcasm::ESP)
          .addImm(1)
          .addReg(0)
          .addImm(-StackSize)
    ...
    if (STI.isTargetLinux() || STI.isTargetWin32() || STI.isTargetWin64() ||
        STI.isTargetDragonFly()) {
      BuildMI(checkMBB, DL, TII.get(Mcasm::CMP32rm))
          .addReg(ScratchReg)
          .addReg(0)
          .addImm(0)
          .addReg(0)
      ...
      if (SaveScratch2)
        BuildMI(checkMBB, DL, TII.get(Mcasm::PUSH32r))
            .addReg(ScratchReg2, RegState::Kill);

      BuildMI(checkMBB, DL, TII.get(Mcasm::MOV32ri), ScratchReg2)
          .addImm(TlsOffset);
      BuildMI(checkMBB, DL, TII.get(Mcasm::CMP32rm))
          .addReg(ScratchReg)
          .addReg(ScratchReg2)
      ...
      if (SaveScratch2)
        BuildMI(checkMBB, DL, TII.get(Mcasm::POP32r), ScratchReg2);
    }
  }

  // This jump is taken if SP >= (Stacklet Limit + Stack Space required).
  // It jumps to normal execution of the function body.
  BuildMI(checkMBB, DL, TII.get(Mcasm::JCC_1))
      .addMBB(&PrologueMBB)
      .addImm(Mcasm::COND_A);
  ...
  if (Is64Bit) {
    ...
    const unsigned RegAX = IsLP64 ? Mcasm::RAX : Mcasm::EAX;
    const unsigned Reg10 = IsLP64 ? Mcasm::R10 : Mcasm::R10D;
    const unsigned Reg11 = IsLP64 ? Mcasm::R11 : Mcasm::R11D;
    const unsigned MOVrr = IsLP64 ? Mcasm::MOV64rr : Mcasm::MOV32rr;
    ...
    BuildMI(allocMBB, DL, TII.get(getMOVriOpcode(IsLP64, StackSize)), Reg10)
        .addImm(StackSize);
    BuildMI(allocMBB, DL,
            TII.get(getMOVriOpcode(IsLP64, McasmFI->getArgumentStackSize())),
            Reg11)
        .addImm(McasmFI->getArgumentStackSize());
  } else {
    BuildMI(allocMBB, DL, TII.get(Mcasm::PUSH32i))
        .addImm(McasmFI->getArgumentStackSize());
    BuildMI(allocMBB, DL, TII.get(Mcasm::PUSH32i)).addImm(StackSize);
  }
  ...
  if (Is64Bit && MF.getTarget().getCodeModel() == CodeModel::Large) {
    ...
    BuildMI(allocMBB, DL, TII.get(Mcasm::CALL64m))
        .addReg(Mcasm::RIP)
        .addImm(0)
        .addReg(0)
    ...
    if (Is64Bit)
      BuildMI(allocMBB, DL, TII.get(Mcasm::CALL64pcrel32))
          .addExternalSymbol("__morestack");
    else
      BuildMI(allocMBB, DL, TII.get(Mcasm::CALLpcrel32))
          .addExternalSymbol("__morestack");
  }

  if (IsNested)
    BuildMI(allocMBB, DL, TII.get(Mcasm::MORESTACK_RET_RESTORE_R10));
  else
    BuildMI(allocMBB, DL, TII.get(Mcasm::MORESTACK_RET));
...
///       temp0 = sp - MaxStack
///       if( temp0 < SP_LIMIT(P) ) goto IncStack else goto OldStart
void McasmFrameLowering::adjustForHiPEPrologue(
    MachineFunction &MF, MachineBasicBlock &PrologueMBB) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  DebugLoc DL;
  ...
  const unsigned HipeLeafWords = getHiPELiteral(
      HiPELiteralsMD, Is64Bit ? "AMD64_LEAF_WORDS" : "Mcasm_LEAF_WORDS");
  ...
  if (MaxStack > Guaranteed) {
    ...
    BuildMI(stackCheckMBB, DL, TII.get(Mcasm::JCC_1))
        .addMBB(&PrologueMBB)
        .addImm(Mcasm::COND_AE);
    ...
    addRegOffset(BuildMI(incStackMBB, DL, TII.get(CMPop)).addReg(ScratchReg),
                 PReg, false, SPLimitOffset);
    BuildMI(incStackMBB, DL, TII.get(Mcasm::JCC_1))
        .addMBB(incStackMBB)
        .addImm(Mcasm::COND_LE);
...
bool McasmFrameLowering::adjustStackWithPops(MachineBasicBlock &MBB,
                                           MachineBasicBlock::iterator MBBI,
                                           const DebugLoc &DL,
                                           int Offset) const {
  if (Offset <= 0)
  ...
  auto &RegClass =
      Is64Bit ? Mcasm::GR64_NOREX_NOSPRegClass : Mcasm::GR32_NOREX_NOSPRegClass;
  ...
  for (int i = 0; i < NumPops; ++i)
    BuildMI(MBB, MBBI, DL, TII.get(STI.is64Bit() ? Mcasm::POP64r : Mcasm::POP32r),
            Regs[i]);
...
bool McasmFrameLowering::canUseAsPrologue(const MachineBasicBlock &MBB) const {
  ...
  const McasmMachineFunctionInfo *McasmFI = MF.getInfo<McasmMachineFunctionInfo>();
  return !TRI->hasStackRealignment(MF) && !McasmFI->hasSwiftAsyncContext();
}

bool McasmFrameLowering::canUseAsEpilogue(const MachineBasicBlock &MBB) const {
  assert(MBB.getParent() && "Block is not attached to a function!");
  ...
  // EFLAGS.
  const MachineFunction &MF = *MBB.getParent();
  if (MF.getInfo<McasmMachineFunctionInfo>()->hasSwiftAsyncContext())
...
bool McasmFrameLowering::enableShrinkWrapping(const MachineFunction &MF) const {
...
MachineBasicBlock::iterator McasmFrameLowering::restoreWin32EHStackPointers(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
  ...
  if (RestoreSP) {
    ...
    addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV32rm), Mcasm::ESP),
                 Mcasm::EBP, true, -EHRegSize)
        .setMIFlag(MachineInstr::FrameSetup);
  }
  ...
  if (UsedReg == FramePtr) {
    ...
    // LEA offset(%ebp), %esi
    addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(Mcasm::LEA32r), BasePtr),
                 FramePtr, false, EndOffset)
        .setMIFlag(MachineInstr::FrameSetup);
    // MOV32rm SavedEBPOffset(%esi), %ebp
    assert(McasmFI->getHasSEHFramePtrSave());
    int Offset =
        getFrameIndexReference(MF, McasmFI->getSEHFramePtrSaveIndex(), UsedReg)
            .getFixed();
    assert(UsedReg == BasePtr);
    addRegOffset(BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV32rm), FramePtr),
                 UsedReg, true, Offset)
        .setMIFlag(MachineInstr::FrameSetup);
  } else {
...
int McasmFrameLowering::getInitialCFAOffset(const MachineFunction &MF) const {
  return TRI->getSlotSize();
}

Register
McasmFrameLowering::getInitialCFARegister(const MachineFunction &MF) const {
  return StackPtr;
}

TargetFrameLowering::DwarfFrameBase
McasmFrameLowering::getDwarfFrameBase(const MachineFunction &MF) const {
  const TargetRegisterInfo *RI = MF.getSubtarget().getRegisterInfo();
  Register FrameRegister = RI->getFrameRegister(MF);
  if (getInitialCFARegister(MF) == FrameRegister &&
      MF.getInfo<McasmMachineFunctionInfo>()->hasCFIAdjustCfa()) {
...
struct McasmFrameSortingObject {
...
struct McasmFrameSortingComparator {
  inline bool operator()(const McasmFrameSortingObject &A,
                         const McasmFrameSortingObject &B) const {
...
void McasmFrameLowering::orderFrameObjects(
  ...
  std::vector<McasmFrameSortingObject> SortingObjects(MFI.getObjectIndexEnd());
  ...
  // Sort the objects using McasmFrameSortingAlgorithm (see its comment for
  // info).
  llvm::stable_sort(SortingObjects, McasmFrameSortingComparator());
...
unsigned
McasmFrameLowering::getWinEHParentFrameOffset(const MachineFunction &MF) const {
  // RDX, the parent frame pointer, is homed into 16(%rsp) in the prologue.
  unsigned Offset = 16;
  // RBP is immediately pushed.
  Offset += SlotSize;
  // All callee-saved registers are then pushed.
  Offset += MF.getInfo<McasmMachineFunctionInfo>()->getCalleeSavedFrameSize();
  // Every funclet allocates enough stack space for the largest outgoing call.
  Offset += getWinEHFuncletFrameSize(MF);
  return Offset;
}

void McasmFrameLowering::processFunctionBeforeFrameFinalized(
...
void McasmFrameLowering::adjustFrameForMsvcCxxEh(MachineFunction &MF) const {
  ...
  addFrameReference(BuildMI(MBB, MBBI, DL, TII.get(Mcasm::MOV64mi32)),
                    UnwindHelpFI)
      .addImm(-2);
}

void McasmFrameLowering::processFunctionBeforeFrameIndicesReplaced(
    MachineFunction &MF, RegScavenger *RS) const {
  auto *McasmFI = MF.getInfo<McasmMachineFunctionInfo>();
  ...
  if (MachineInstr *MI = McasmFI->getStackPtrSaveMI()) {
    MI->eraseFromParent();
    McasmFI->setStackPtrSaveMI(nullptr);
  }
}

void McasmFrameLowering::restoreWinEHStackPointersInParent(
...
void McasmFrameLowering::spillFPBPUsingSP(MachineFunction &MF,
  ...
  if (FP.isValid() && needsDwarfCFI(MF)) {
    ...
    if (STI.isTarget64BitILP32())
      StackPtr = Register(getMcasmSubSuperRegister(StackPtr, 64));
...
void McasmFrameLowering::restoreFPBPUsingSP(MachineFunction &MF,
  ...
  if (BP.isValid()) {
    BuildMI(*MBB, Pos, DL,
            TII.get(getPOPOpcode(MF.getSubtarget<McasmSubtarget>())), BP);
  }

  // Restore FP.
  if (FP.isValid()) {
    BuildMI(*MBB, Pos, DL,
            TII.get(getPOPOpcode(MF.getSubtarget<McasmSubtarget>())), FP);
...
void McasmFrameLowering::saveAndRestoreFPBPUsingSP(
  ...
  if (SpillFP) {
    ...
    if (STI.isTarget64BitILP32())
      FP = Register(getMcasmSubSuperRegister(FP, 64));
  ...
  if (SpillBP) {
    ...
    if (STI.isTarget64BitILP32())
      BP = Register(getMcasmSubSuperRegister(BP, 64));
...
bool McasmFrameLowering::skipSpillFPBP(
    MachineFunction &MF, MachineBasicBlock::reverse_iterator &MI) const {
  if (MI->getOpcode() == Mcasm::LCMPXCHG16B_SAVE_RBX) {
    ...
    while (!(MI->getOpcode() == TargetOpcode::COPY &&
             MI->getOperand(1).getReg() == Mcasm::RBX) &&
           !((Reg = TII.isStoreToStackSlot(*MI, FI)) && Reg == Mcasm::RBX))
      ++MI;
    return true;
  }
  return false;
...
void McasmFrameLowering::checkInterferedAccess(
...
void McasmFrameLowering::spillFPBP(MachineFunction &MF) const {
  Register FP, BP;
  ...
  if (!MF.hasInlineAsm()) {
    McasmMachineFunctionInfo *McasmFI = MF.getInfo<McasmMachineFunctionInfo>();
    if (!McasmFI->getFPClobberedByCall())
      FP = 0;
    if (!McasmFI->getBPClobberedByCall())
      BP = 0;
  }
  if (!FP && !BP)
    return;
