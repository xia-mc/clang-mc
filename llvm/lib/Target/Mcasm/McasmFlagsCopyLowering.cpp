//====- McasmFlagsCopyLowering.cpp - Lowers COPY nodes of EFLAGS ------------===//
//
...
using CondRegArray = std::array<Register, Mcasm::LAST_VALID_COND + 1>;

class McasmFlagsCopyLoweringImpl {
public:
  McasmFlagsCopyLoweringImpl(MachineDominatorTree *MDT) : MDT(MDT) {}
...
private:
  ...
  Register promoteCondToReg(MachineBasicBlock &MBB,
                            ...
                            const DebugLoc &TestLoc, Mcasm::CondCode Cond);
  std::pair<Register, bool> getCondOrInverseInReg(
      MachineBasicBlock &TestMBB, MachineBasicBlock::iterator TestPos,
      const DebugLoc &TestLoc, Mcasm::CondCode Cond, CondRegArray &CondRegs);
...
class McasmFlagsCopyLoweringLegacy : public MachineFunctionPass {
public:
  McasmFlagsCopyLoweringLegacy() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override { return "Mcasm EFLAGS copy lowering"; }
...
INITIALIZE_PASS_BEGIN(McasmFlagsCopyLoweringLegacy, DEBUG_TYPE,
                      "Mcasm EFLAGS copy lowering", false, false)
INITIALIZE_PASS_END(McasmFlagsCopyLoweringLegacy, DEBUG_TYPE,
                    "Mcasm EFLAGS copy lowering", false, false)

FunctionPass *llvm::createMcasmFlagsCopyLoweringLegacyPass() {
  return new McasmFlagsCopyLoweringLegacy();
}

char McasmFlagsCopyLoweringLegacy::ID = 0;

void McasmFlagsCopyLoweringLegacy::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.addUsedIfAvailable<MachineDominatorTreeWrapperPass>();
  MachineFunctionPass::getAnalysisUsage(AU);
}

static bool isArithmeticOp(unsigned Opc) {
  return Mcasm::isADC(Opc) || Mcasm::isSBB(Opc) || Mcasm::isRCL(Opc) ||
         Mcasm::isRCR(Opc) || (Opc == Mcasm::SETB_C32r || Opc == Mcasm::SETB_C64r);
}
...
static MachineBasicBlock &splitBlock(MachineBasicBlock &MBB,
  ...
  assert(Mcasm::getCondFromBranch(SplitI) != Mcasm::COND_INVALID &&
  ...
  assert(Mcasm::getCondFromBranch(PrevI) != Mcasm::COND_INVALID &&
...
static EFLAGSClobber getClobberType(const MachineInstr &MI) {
  ...
  // support on Linux OS.
  bool IsWithReloc =
      McasmEnableAPXForRelocation ? false : isAddMemInstrWithRelocation(MI);

  if (FlagDef->isDead() && Mcasm::getNFVariant(MI.getOpcode()) && !IsWithReloc)
    return EvitableClobber;

  return InevitableClobber;
}

bool McasmFlagsCopyLoweringImpl::runOnMachineFunction(MachineFunction &MF) {
  ...
  PromoteRC = &Mcasm::GR8RegClass;

  if (MF.empty())
    // Nothing to do for a degenerate empty function...
    return false;

  if (none_of(MRI->def_instructions(Mcasm::EFLAGS), [](const MachineInstr &MI) {
  ...
  for (MachineBasicBlock *MBB : RPOT)
    for (MachineInstr &MI : *MBB)
      if (MI.getOpcode() == TargetOpcode::COPY &&
          MI.getOperand(0).getReg() == Mcasm::EFLAGS)
        Copies.insert(&MI);
  ...
  if (Subtarget->hasNF()) {
    ...
    while (CopyIIt != CopyIEnd) {
      ...
      for (auto *Clobber : EvitableClobbers) {
        unsigned NewOpc = Mcasm::getNFVariant(Clobber->getOpcode());
        assert(NewOpc && "evitable clobber must have a NF variant");
        Clobber->setDesc(TII->get(NewOpc));
        Clobber->removeOperand(
            Clobber->findRegisterDefOperand(Mcasm::EFLAGS, /*TRI=*/nullptr)
                ->getOperandNo());
        ++NumNFsConvertedTo;
      }
  ...
  for (MachineInstr *CopyI : Copies) {
    ...
    assert(DOp.getReg() == Mcasm::EFLAGS && "Unexpected copy def register!");
    if (DOp.isDead())
      continue;
    ...
    while (TestMBB->isLiveIn(Mcasm::EFLAGS) && !TestMBB->pred_empty() &&
    ...
    LLVM_DEBUG({
      auto DefIt = llvm::find_if(
          ...
          [&](MachineInstr &MI) {
            return MI.findRegisterDefOperand(Mcasm::EFLAGS, /*TRI=*/nullptr);
          });
    ...
    do {
      ...
      for (auto MII = (&UseMBB == &MBB && !VisitedBlocks.count(&UseMBB))
        ...
        MachineOperand *FlagUse =
            MI.findRegisterUseOperand(Mcasm::EFLAGS, /*TRI=*/nullptr);
        FlagsKilled = MI.modifiesRegister(Mcasm::EFLAGS, TRI);
        ...
        // tail calls, as those are not introduced into the Mcasm MI until post-RA
        ...
        if (Mcasm::getCondFromBranch(MI) != Mcasm::COND_INVALID) {
          ...
          do {
            ...
            ++JmpIt;
                   ...
                   Mcasm::getCondFromBranch(*JmpIt) != Mcasm::COND_INVALID);
          break;
        }
        ...
        if (Opc == TargetOpcode::COPY) {
          ...
          MI.eraseFromParent();
        } else if (Mcasm::isSETCC(Opc) || Mcasm::isSETZUCC(Opc)) {
      ...
      for (MachineBasicBlock *SuccMBB : UseMBB.successors())
        if (SuccMBB->isLiveIn(Mcasm::EFLAGS) &&
          ...
          SuccMBB->removeLiveIn(Mcasm::EFLAGS);
        }
    } while (!Blocks.empty());
  ...
  for (MachineBasicBlock &MBB : MF)
    for (MachineInstr &MI : MBB)
      if (MI.getOpcode() == TargetOpcode::COPY &&
          (MI.getOperand(0).getReg() == Mcasm::EFLAGS ||
           MI.getOperand(1).getReg() == Mcasm::EFLAGS)) {
...
CondRegArray McasmFlagsCopyLoweringImpl::collectCondsInRegs(
  ...
  for (MachineInstr &MI :
    ...
    Mcasm::CondCode Cond = Mcasm::getCondFromSETCC(MI);
    if (Cond != Mcasm::COND_INVALID && !MI.mayStore() &&
    ...
    if (MI.findRegisterDefOperand(Mcasm::EFLAGS, /*TRI=*/nullptr))
      break;
  }
  return CondRegs;
}

Register McasmFlagsCopyLoweringImpl::promoteCondToReg(
    MachineBasicBlock &TestMBB, MachineBasicBlock::iterator TestPos,
    const DebugLoc &TestLoc, Mcasm::CondCode Cond) {
...
std::pair<Register, bool> McasmFlagsCopyLoweringImpl::getCondOrInverseInReg(
    MachineBasicBlock &TestMBB, MachineBasicBlock::iterator TestPos,
    const DebugLoc &TestLoc, Mcasm::CondCode Cond, CondRegArray &CondRegs) {
  Register &CondReg = CondRegs[Cond];
  Register &InvCondReg = CondRegs[Mcasm::GetOppositeBranchCondition(Cond)];
...
void McasmFlagsCopyLoweringImpl::insertTest(MachineBasicBlock &MBB,
                                          MachineBasicBlock::iterator Pos,
                                          const DebugLoc &Loc, Register Reg) {
  auto TestI =
      BuildMI(MBB, Pos, Loc, TII->get(Mcasm::TEST8rr)).addReg(Reg).addReg(Reg);
  (void)TestI;
  LLVM_DEBUG(dbgs() << "    test cond: "; TestI->dump());
  ++NumTestsInserted;
}

void McasmFlagsCopyLoweringImpl::rewriteSetCC(MachineBasicBlock &MBB,
                                            MachineBasicBlock::iterator Pos,
                                            const DebugLoc &Loc,
                                            MachineInstr &MI,
                                            CondRegArray &CondRegs) {
...
void McasmFlagsCopyLoweringImpl::rewriteArithmetic(
  ...
  auto AddI =
      BuildMI(*MI.getParent(), MI.getIterator(), MI.getDebugLoc(),
              TII->get(Subtarget->hasNDD() ? Mcasm::ADD8ri_ND : Mcasm::ADD8ri))
  ...
  ++NumAddsInserted;
  MI.findRegisterUseOperand(Mcasm::EFLAGS, /*TRI=*/nullptr)->setIsKill(true);
}

static Mcasm::CondCode getImplicitCondFromMI(unsigned Opc) {
#define FROM_TO(A, B)                                                          \
  case Mcasm::CMOV##A##_Fp32:                                                    \
  case Mcasm::CMOV##A##_Fp64:                                                    \
  case Mcasm::CMOV##A##_Fp80:                                                    \
    return Mcasm::COND_##B;
...
static unsigned getOpcodeWithCC(unsigned Opc, Mcasm::CondCode CC) {
  assert((CC == Mcasm::COND_E || CC == Mcasm::COND_NE) && "Unexpected CC");
#define CASE(A)                                                                \
  case Mcasm::CMOVB_##A:                                                         \
  case Mcasm::CMOVE_##A:                                                         \
  case Mcasm::CMOVP_##A:                                                         \
  case Mcasm::CMOVBE_##A:                                                        \
  case Mcasm::CMOVNB_##A:                                                        \
  case Mcasm::CMOVNE_##A:                                                        \
  case Mcasm::CMOVNP_##A:                                                        \
  case Mcasm::CMOVNBE_##A:                                                       \
    return (CC == Mcasm::COND_E) ? Mcasm::CMOVE_##A : Mcasm::CMOVNE_##A;
  switch (Opc) {
  default:
...
void McasmFlagsCopyLoweringImpl::rewriteMI(MachineBasicBlock &MBB,
                                         MachineBasicBlock::iterator Pos,
                                         const DebugLoc &Loc, MachineInstr &MI,
                                         CondRegArray &CondRegs) {
  // First get the register containing this specific condition.
  bool IsImplicitCC = false;
  Mcasm::CondCode CC = Mcasm::getCondFromMI(MI);
  if (CC == Mcasm::COND_INVALID) {
    CC = getImplicitCondFromMI(MI.getOpcode());
    IsImplicitCC = true;
  }
  assert(CC != Mcasm::COND_INVALID && "Unknown EFLAG user!");
  Register CondReg;
  bool Inverted;
  ...
  Mcasm::CondCode NewCC = Inverted ? Mcasm::COND_E : Mcasm::COND_NE;
  if (IsImplicitCC)
...
bool McasmFlagsCopyLoweringLegacy::runOnMachineFunction(MachineFunction &MF) {
  auto *MDTWrapper = getAnalysisIfAvailable<MachineDominatorTreeWrapperPass>();
  MachineDominatorTree *MDT = MDTWrapper ? &MDTWrapper->getDomTree() : nullptr;
  return McasmFlagsCopyLoweringImpl(MDT).runOnMachineFunction(MF);
}

PreservedAnalyses
McasmFlagsCopyLoweringPass::run(MachineFunction &MF,
                              MachineFunctionAnalysisManager &MFAM) {
  MachineDominatorTree *MDT =
      MFAM.getCachedResult<MachineDominatorTreeAnalysis>(MF);
  bool Changed = McasmFlagsCopyLoweringImpl(MDT).runOnMachineFunction(MF);
