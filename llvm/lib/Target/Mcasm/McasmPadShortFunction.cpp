//===-------- McasmPadShortFunction.cpp - pad short functions -----------===//
//
...
namespace {
  ...
  struct PadShortFunc : public MachineFunctionPass {
    ...
    StringRef getPassName() const override {
      return "Mcasm Atom pad short functions";
    }
...
FunctionPass *llvm::createMcasmPadShortFunctions() {
...
bool PadShortFunc::runOnMachineFunction(MachineFunction &MF) {
  LLVM_DEBUG(dbgs() << "Start McasmPadShortFunctionPass\n";);
  ...
  if (!MF.getSubtarget<McasmSubtarget>().padShortFunctions())
    return false;
  ...
  LLVM_DEBUG(dbgs() << "End McasmPadShortFunctionPass\n";);
  return MadeChange;
}
...
void PadShortFunc::addPadding(MachineBasicBlock *MBB,
  ...
  for (unsigned i = 0, e = IssueWidth * NOOPsToAdd; i != e; ++i)
    BuildMI(*MBB, MBBI, DL, TSM.getInstrInfo()->get(Mcasm::NOOP));
}
