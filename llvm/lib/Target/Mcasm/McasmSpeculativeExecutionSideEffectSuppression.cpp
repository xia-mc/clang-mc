//===-- McasmSpeculativeExecutionSideEffectSuppression.cpp ------------------===//
//
...
/// \file
///
/// This file contains the Mcasm implementation of the speculative execution side
...
constexpr StringRef McasmSESESPassName =
    "Mcasm Speculative Execution Side Effect Suppression";

class McasmSpeculativeExecutionSideEffectSuppressionLegacy
    : public MachineFunctionPass {
public:
  McasmSpeculativeExecutionSideEffectSuppressionLegacy()
      : MachineFunctionPass(ID) {}

  static char ID;
  StringRef getPassName() const override { return McasmSESESPassName; }

  bool runOnMachineFunction(MachineFunction &MF) override;
};
} // namespace

char McasmSpeculativeExecutionSideEffectSuppressionLegacy::ID = 0;
...
static bool hasConstantAddressingMode(const MachineInstr &MI) {
  for (const MachineOperand &MO : MI.uses())
    if (MO.isReg() && Mcasm::RIP != MO.getReg())
      return false;
  return true;
}

static bool
runMcasmSpeculativeExecutionSideEffectSuppression(MachineFunction &MF) {
  ...
  LLVM_DEBUG(dbgs() << "********** " << McasmSESESPassName << " : "
                    << MF.getName() << " **********\n");
  bool Modified = false;
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    for (auto &MI : MBB) {
      ...
      if (MI.mayLoadOrStore() && !MI.isTerminator()) {
        if (!PrevInstIsLFENCE) {
          BuildMI(MBB, MI, DebugLoc(), TII->get(Mcasm::LFENCE));
          NumLFENCEsInserted++;
          Modified = true;
        }
      ...
      // McasmInstrInfo::analyzeBranch method assumes all terminators are grouped
      ...
      if (!PrevInstIsLFENCE) {
        ...
        BuildMI(MBB, FirstTerminator, DebugLoc(), TII->get(Mcasm::LFENCE));
        NumLFENCEsInserted++;
        Modified = true;
      }
      break;
...
bool McasmSpeculativeExecutionSideEffectSuppressionLegacy::runOnMachineFunction(
    MachineFunction &MF) {
  return runMcasmSpeculativeExecutionSideEffectSuppression(MF);
}

PreservedAnalyses McasmSpeculativeExecutionSideEffectSuppressionPass::run(
...
FunctionPass *
llvm::createMcasmSpeculativeExecutionSideEffectSuppressionLegacyPass() {
  return new McasmSpeculativeExecutionSideEffectSuppressionLegacy();
}

INITIALIZE_PASS(McasmSpeculativeExecutionSideEffectSuppressionLegacy, "x86-seses",
                "Mcasm Speculative Execution Side Effect Suppression", false,
                false)
