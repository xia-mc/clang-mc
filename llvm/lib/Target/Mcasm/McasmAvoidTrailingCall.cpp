//===----- McasmAvoidTrailingCall.cpp - Insert int3 after trailing calls ----===//
//
...
#define AVOIDCALL_DESC "Mcasm avoid trailing call pass"
...
namespace {
class McasmAvoidTrailingCallLegacyPass : public MachineFunctionPass {
public:
  McasmAvoidTrailingCallLegacyPass() : MachineFunctionPass(ID) {}
...
char McasmAvoidTrailingCallLegacyPass::ID = 0;

FunctionPass *llvm::createMcasmAvoidTrailingCallLegacyPass() {
  return new McasmAvoidTrailingCallLegacyPass();
}

INITIALIZE_PASS(McasmAvoidTrailingCallLegacyPass, AVOIDCALL_NAME, AVOIDCALL_DESC,
                false, false)
...
bool UpdatedOnMcasmAvoidTrailingCallPass(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    if (IsEmpty || IsCall) {
      ...
      BuildMI(MBB, MBBI, DL, TII.get(Mcasm::INT3));
      Changed = true;
    }
  }
...
bool McasmAvoidTrailingCallLegacyPass::runOnMachineFunction(MachineFunction &MF) {
  return UpdatedOnMcasmAvoidTrailingCallPass(MF);
}

PreservedAnalyses
McasmAvoidTrailingCallPass::run(MachineFunction &MF,
                              MachineFunctionAnalysisManager &MFAM) {
  bool Changed = UpdatedOnMcasmAvoidTrailingCallPass(MF);
  if (!Changed)
