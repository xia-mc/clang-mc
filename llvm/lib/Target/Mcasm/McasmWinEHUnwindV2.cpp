//===-- McasmWinEHUnwindV2.cpp - Win x64 Unwind v2 ----------------*- C++ -*-===//
//
...
class McasmWinEHUnwindV2 : public MachineFunctionPass {
public:
  static char ID;

  McasmWinEHUnwindV2() : MachineFunctionPass(ID) {
    initializeMcasmWinEHUnwindV2Pass(*PassRegistry::getPassRegistry());
  }
...
char McasmWinEHUnwindV2::ID = 0;

INITIALIZE_PASS(McasmWinEHUnwindV2, "x86-wineh-unwindv2",
                "Analyze and emit instructions for Win64 Unwind v2", false,
                false)

FunctionPass *llvm::createMcasmWinEHUnwindV2Pass() {
  return new McasmWinEHUnwindV2();
}
...
McasmWinEHUnwindV2::runAnalysisOnFuncOrFunclet(MachineFunction &MF,
                                             MachineFunction::iterator &Iter,
                                             WinX64EHUnwindV2Mode Mode) {
...
bool McasmWinEHUnwindV2::runOnMachineFunction(MachineFunction &MF) {
  WinX64EHUnwindV2Mode Mode =
  ...
  for (auto &FI : FrameInfos) {
    ...
    for (auto &Info : llvm::reverse(FI.EpilogInfos)) {
      ...
      BuildMI(MBB, Info.UnwindV2StartLocation, DL,
              TII->get(Mcasm::SEH_UnwindV2Start));
      ...
      if ((LastUnwindInfoEndPosition - Info.ApproximateInstructionPosition >=
        ...
        BuildMI(MBB, MBB.begin(), DL,
                TII->get(Mcasm::SEH_SplitChainedAtEndOfBlock));
  ...
  BuildMI(FirstMBB, FirstMBB.front(), findDebugLoc(FirstMBB),
          TII->get(Mcasm::SEH_UnwindVersion))
      .addImm(2);

  return true;
}

std::nullopt_t McasmWinEHUnwindV2::rejectCurrentFunctionInternalError(
    const MachineFunction &MF, WinX64EHUnwindV2Mode Mode, StringRef Reason) {
