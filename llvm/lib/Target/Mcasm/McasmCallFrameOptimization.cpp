//===----- McasmCallFrameOptimization.cpp - Optimize x86 call sequences -----===//
//
...
static cl::opt<bool>
    NoMcasmCFOpt("no-x86-call-frame-opt",
               cl::desc("Avoid optimizing x86 call frames for size"),
               cl::init(false), cl::Hidden);

namespace {

class McasmCallFrameOptimizationImpl {
public:
...
private:
  ...
  InstClassification classifyInstruction(MachineBasicBlock &MBB,
                                         ...
                                         const McasmRegisterInfo &RegInfo,
                                         const DenseSet<MCRegister> &UsedRegs);

  const McasmInstrInfo *TII = nullptr;
  const McasmFrameLowering *TFL = nullptr;
...
class McasmCallFrameOptimizationLegacy : public MachineFunctionPass {
public:
  McasmCallFrameOptimizationLegacy() : MachineFunctionPass(ID) {}
...
private:
  StringRef getPassName() const override { return "Mcasm Optimize Call Frame"; }
};

} // end anonymous namespace
char McasmCallFrameOptimizationLegacy::ID = 0;
INITIALIZE_PASS(McasmCallFrameOptimizationLegacy, DEBUG_TYPE,
                "Mcasm Call Frame Optimization", false, false)

// This checks whether the transformation is legal.
// Also returns false in cases where it's potentially legal, but
// we don't even want to try.
bool McasmCallFrameOptimizationImpl::isLegal(MachineFunction &MF) {
  if (NoMcasmCFOpt.getValue())
    return false;
...
bool McasmCallFrameOptimizationImpl::isProfitable(MachineFunction &MF,
...
bool McasmCallFrameOptimizationImpl::runOnMachineFunction(MachineFunction &MF) {
  STI = &MF.getSubtarget<McasmSubtarget>();
  TII = STI->getInstrInfo();
  TFL = STI->getFrameLowering();
  MRI = &MF.getRegInfo();

  const McasmRegisterInfo &RegInfo = *STI->getRegisterInfo();
...
McasmCallFrameOptimizationImpl::InstClassification
McasmCallFrameOptimizationImpl::classifyInstruction(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MI,
    const McasmRegisterInfo &RegInfo, const DenseSet<MCRegister> &UsedRegs) {
  if (MI == MBB.end())
    return Exit;
  ...
  switch (MI->getOpcode()) {
    case Mcasm::AND16mi:
    case Mcasm::AND32mi:
    case Mcasm::AND64mi32: {
      const MachineOperand &ImmOp = MI->getOperand(Mcasm::AddrNumOperands);
...
void McasmCallFrameOptimizationImpl::collectCallInfo(
  ...
  while (I->getOpcode() == Mcasm::LEA32r || I->isDebugInstr())
    ++I;
  ...
  for (InstClassification Classification = Skip; Classification != Exit; ++I) {
    ...
    // sequence.
    if (!I->getOperand(Mcasm::AddrBaseReg).isReg() ||
        (I->getOperand(Mcasm::AddrBaseReg).getReg() != StackPtr) ||
        !I->getOperand(Mcasm::AddrScaleAmt).isImm() ||
        (I->getOperand(Mcasm::AddrScaleAmt).getImm() != 1) ||
        (I->getOperand(Mcasm::AddrIndexReg).getReg() != Mcasm::NoRegister) ||
        (I->getOperand(Mcasm::AddrSegmentReg).getReg() != Mcasm::NoRegister) ||
        !I->getOperand(Mcasm::AddrDisp).isImm())
      return;

    int64_t StackDisp = I->getOperand(Mcasm::AddrDisp).getImm();
    assert(StackDisp >= 0 &&
...
void McasmCallFrameOptimizationImpl::adjustCallSequence(
  ...
  for (int Idx = (Context.ExpectedDist >> Log2SlotSize) - 1; Idx >= 0; --Idx) {
    ...
    const MachineOperand &PushOp = Store->getOperand(Mcasm::AddrNumOperands);
    ...
    case Mcasm::MOV32mi:
    case Mcasm::MOV64mi32:
      PushOpcode = Is64Bit ? Mcasm::PUSH64i32 : Mcasm::PUSH32i;
    ...
    case Mcasm::MOV64mr: {
      ...
      if (Is64Bit && Store->getOpcode() == Mcasm::MOV32mr) {
        Register UndefReg = MRI->createVirtualRegister(&Mcasm::GR64RegClass);
        Reg = MRI->createVirtualRegister(&Mcasm::GR64RegClass);
        BuildMI(MBB, Context.Call, DL, TII->get(Mcasm::IMPLICIT_DEF), UndefReg);
        BuildMI(MBB, Context.Call, DL, TII->get(Mcasm::INSERT_SUBREG), Reg)
            .addReg(UndefReg)
            .add(PushOp)
      ...
      if (!SlowPUSHrmm && (DefMov = canFoldIntoRegPush(FrameSetup, Reg))) {
        PushOpcode = Is64Bit ? Mcasm::PUSH64rmm : Mcasm::PUSH32rmm;
        Push = BuildMI(MBB, Context.Call, DL, TII->get(PushOpcode));

        unsigned NumOps = DefMov->getDesc().getNumOperands();
        for (unsigned i = NumOps - Mcasm::AddrNumOperands; i != NumOps; ++i)
          Push->addOperand(DefMov->getOperand(i));
        Push->cloneMergedMemRefs(MF, {DefMov, &*Store});
        DefMov->eraseFromParent();
      } else {
        PushOpcode = Is64Bit ? Mcasm::PUSH64r : Mcasm::PUSH32r;
        Push = BuildMI(MBB, Context.Call, DL, TII->get(PushOpcode))
                   .addReg(Reg)
                   .getInstr();
        Push->cloneMemRefs(MF, *Store);
...
MachineInstr *McasmCallFrameOptimizationImpl::canFoldIntoRegPush(
  ...
  if ((DefMI.getOpcode() != Mcasm::MOV32rm &&
       DefMI.getOpcode() != Mcasm::MOV64rm) ||
...
FunctionPass *llvm::createMcasmCallFrameOptimizationLegacyPass() {
  return new McasmCallFrameOptimizationLegacy();
}

bool McasmCallFrameOptimizationLegacy::runOnMachineFunction(MachineFunction &MF) {
  if (skipFunction(MF.getFunction()))
    return false;
  McasmCallFrameOptimizationImpl Impl;
  return Impl.runOnMachineFunction(MF);
}

PreservedAnalyses
McasmCallFrameOptimizationPass::run(MachineFunction &MF,
