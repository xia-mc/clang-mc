//===-- McasmWinEHState - Insert EH state updates for win32 exceptions ------===//
//
...
FunctionPass *llvm::createMcasmWinEHStatePass() { return new WinEHStatePass(); }
...
void WinEHStatePass::emitExceptionRegistrationRecord(Function *F) {
  assert(Personality == EHPersonality::MSVC_CXX ||
         Personality == EHPersonality::MSVC_McasmSEH);
  ...
  if (Personality == EHPersonality::MSVC_CXX) {
    ...
    cast<Function>(CxxLongjmpUnwind.getCallee()->stripPointerCasts())
        ->setCallingConv(CallingConv::Mcasm_StdCall);
  } else if (Personality == EHPersonality::MSVC_McasmSEH) {
...
void WinEHStatePass::linkExceptionRegistration(IRBuilder<> &Builder,
  ...
  // Next = [fs:00]
  Constant *FSZero = Constant::getNullValue(PointerType::get(C, McasmAS::FS));
...
void WinEHStatePass::addStateStores(Function &F, WinEHFuncInfo &FuncInfo) {
  ...
  for (BasicBlock *BB : RPOT) {
    ...
    LLVM_DEBUG(dbgs() << "McasmWinEHState: " << BB->getName()
