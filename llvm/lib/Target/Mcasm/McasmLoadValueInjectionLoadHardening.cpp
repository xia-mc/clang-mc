//==-- McasmLoadValueInjectionLoadHardening.cpp - LVI load hardening for x86 --=//
//
...
class McasmLoadValueInjectionLoadHardeningPass : public MachineFunctionPass {
public:
  McasmLoadValueInjectionLoadHardeningPass() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override {
    return "Mcasm Load Value Injection (LVI) Load Hardening";
  }
...
private:
  ...
  inline bool isFence(const MachineInstr *MI) const {
    return MI && (MI->getOpcode() == Mcasm::LFENCE ||
...
char McasmLoadValueInjectionLoadHardeningPass::ID = 0;

void McasmLoadValueInjectionLoadHardeningPass::getAnalysisUsage(
...
bool McasmLoadValueInjectionLoadHardeningPass::runOnMachineFunction(
    MachineFunction &MF) {
...
std::unique_ptr<MachineGadgetGraph>
McasmLoadValueInjectionLoadHardeningPass::getGadgetGraph(
...
int McasmLoadValueInjectionLoadHardeningPass::elimMitigatedEdgesAndNodes(
...
std::unique_ptr<MachineGadgetGraph>
McasmLoadValueInjectionLoadHardeningPass::trimMitigatedEdges(
    std::unique_ptr<MachineGadgetGraph> Graph) const {
...
int McasmLoadValueInjectionLoadHardeningPass::hardenLoadsWithPlugin(
...
int McasmLoadValueInjectionLoadHardeningPass::hardenLoadsWithHeuristic(
    MachineFunction &MF, std::unique_ptr<MachineGadgetGraph> Graph) const {
...
int McasmLoadValueInjectionLoadHardeningPass::insertFences(
    ...
    for (const Edge &E : N.edges()) {
      if (CutEdges.contains(E)) {
        ...
        if ((InsertionPt == MBB->end() || !isFence(&*InsertionPt)) &&
            (!Prev || !isFence(Prev))) {
          BuildMI(*MBB, InsertionPt, DebugLoc(), TII->get(Mcasm::LFENCE));
          ++FencesInserted;
        }
      }
    }
...
bool McasmLoadValueInjectionLoadHardeningPass::instrUsesRegToAccessMemory(
    const MachineInstr &MI, Register Reg) const {
  if (!MI.mayLoadOrStore() || MI.getOpcode() == Mcasm::MFENCE ||
      MI.getOpcode() == Mcasm::SFENCE || MI.getOpcode() == Mcasm::LFENCE)
    return false;
...
bool McasmLoadValueInjectionLoadHardeningPass::instrUsesRegToBranch(
    const MachineInstr &MI, Register Reg) const {
...
INITIALIZE_PASS_BEGIN(McasmLoadValueInjectionLoadHardeningPass, PASS_KEY,
                      "Mcasm LVI load hardening", false, false)
INITIALIZE_PASS_DEPENDENCY(MachineLoopInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(MachineDominatorTreeWrapperPass)
INITIALIZE_PASS_DEPENDENCY(MachineDominanceFrontier)
INITIALIZE_PASS_END(McasmLoadValueInjectionLoadHardeningPass, PASS_KEY,
                    "Mcasm LVI load hardening", false, false)

FunctionPass *llvm::createMcasmLoadValueInjectionLoadHardeningPass() {
  return new McasmLoadValueInjectionLoadHardeningPass();
}
