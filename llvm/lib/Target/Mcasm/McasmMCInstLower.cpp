//===-- McasmMCInstLower.cpp - Convert Mcasm MachineInstr to an MCInst --------===//
//
...
#include "MCTargetDesc/McasmEncodingOptimization.h"
#include "MCTargetDesc/McasmInstComments.h"
#include "MCTargetDesc/McasmMCAsmInfo.h"
#include "MCTargetDesc/McasmShuffleDecode.h"
...
#include "McasmShuffleDecodeConstantPool.h"
...
/// McasmMCInstLower - This class is used to lower an MachineInstr into an MCInst.
...
public:
  McasmMCInstLower(const MachineFunction &MF, McasmAsmPrinter &asmprinter);
...
static void emitMcasmNops(MCStreamer &OS, unsigned NumBytes,
                        const McasmSubtarget *Subtarget);

void McasmAsmPrinter::StackMapShadowTracker::count(const MCInst &Inst,
...
void McasmAsmPrinter::StackMapShadowTracker::emitShadowPadding(
    MCStreamer &OutStreamer, const MCSubtargetInfo &STI) {
  if (InShadow && CurrentShadowSize < RequiredShadowSize) {
    InShadow = false;
    emitMcasmNops(OutStreamer, RequiredShadowSize - CurrentShadowSize,
                &MF->getSubtarget<McasmSubtarget>());
  }
}

void McasmAsmPrinter::EmitAndCountInstruction(MCInst &Inst) {
  OutStreamer->emitInstruction(Inst, getSubtargetInfo());
  SMShadowTracker.count(Inst, getSubtargetInfo(), CodeEmitter.get());
}

McasmMCInstLower::McasmMCInstLower(const MachineFunction &mf,
                               McasmAsmPrinter &asmprinter)
    : Ctx(asmprinter.OutContext), MF(mf), TM(mf.getTarget()),
      MAI(*TM.getMCAsmInfo()), AsmPrinter(asmprinter) {}

MachineModuleInfoMachO &McasmMCInstLower::getMachOMMI() const {
...
MCOperand McasmMCInstLower::LowerSymbolOperand(const MachineOperand &MO,
  ...
  uint16_t Specifier = Mcasm::S_None;
  ...
  case McasmII::MO_TLVP:
    Specifier = Mcasm::S_TLVP;
    break;
  case McasmII::MO_TLVP_PIC_BASE:
    Expr = MCSymbolRefExpr::create(Sym, Mcasm::S_TLVP, Ctx);
    // Subtract the pic base.
    Expr = MCBinaryExpr::createSub(
        Expr, MCSymbolRefExpr::create(MF.getPICBaseSymbol(), Ctx), Ctx);
    break;
  case McasmII::MO_SECREL:
    Specifier = uint16_t(Mcasm::S_COFF_SECREL);
    break;
  case McasmII::MO_TLSGD:
    Specifier = Mcasm::S_TLSGD;
    break;
  case McasmII::MO_TLSLD:
    Specifier = Mcasm::S_TLSLD;
    break;
  case McasmII::MO_TLSLDM:
    Specifier = Mcasm::S_TLSLDM;
    break;
  case McasmII::MO_GOTTPOFF:
    Specifier = Mcasm::S_GOTTPOFF;
    break;
  case McasmII::MO_INDNTPOFF:
    Specifier = Mcasm::S_INDNTPOFF;
    break;
  case McasmII::MO_TPOFF:
    Specifier = Mcasm::S_TPOFF;
    break;
  case McasmII::MO_DTPOFF:
    Specifier = Mcasm::S_DTPOFF;
    break;
  case McasmII::MO_NTPOFF:
    Specifier = Mcasm::S_NTPOFF;
    break;
  case McasmII::MO_GOTNTPOFF:
    Specifier = Mcasm::S_GOTNTPOFF;
    break;
  case McasmII::MO_GOTPCREL:
    Specifier = Mcasm::S_GOTPCREL;
    break;
  case McasmII::MO_GOTPCREL_NORELAX:
    Specifier = Mcasm::S_GOTPCREL_NORELAX;
    break;
  case McasmII::MO_GOT:
    Specifier = Mcasm::S_GOT;
    break;
  case McasmII::MO_GOTOFF:
    Specifier = Mcasm::S_GOTOFF;
    break;
  case McasmII::MO_PLT:
    Specifier = Mcasm::S_PLT;
    break;
  case McasmII::MO_ABS8:
    Specifier = Mcasm::S_ABS8;
    break;
...
static unsigned getRetOpcode(const McasmSubtarget &Subtarget) {
  return Subtarget.is64Bit() ? Mcasm::RET64 : Mcasm::RET32;
}

MCOperand McasmMCInstLower::LowerMachineOperand(const MachineInstr *MI,
                                              const MachineOperand &MO) const {
  switch (MO.getType()) {
  default:
    MI->print(errs());
...
static unsigned convertTailJumpOpcode(unsigned Opcode) {
  ...
  case Mcasm::TAILJMPm:
    Opcode = Mcasm::JMP32m;
    break;
  case Mcasm::TAILJMPr64:
    Opcode = Mcasm::JMP64r;
    break;
  case Mcasm::TAILJMPm64:
    Opcode = Mcasm::JMP64m;
    break;
  case Mcasm::TAILJMPr64_REX:
    Opcode = Mcasm::JMP64r_REX;
    break;
  case Mcasm::TAILJMPm64_REX:
    Opcode = Mcasm::JMP64m_REX;
    break;
  case Mcasm::TAILJMPd:
  case Mcasm::TAILJMPd64:
    Opcode = Mcasm::JMP_1;
    break;
  case Mcasm::TAILJMPd_CC:
  case Mcasm::TAILJMPd64_CC:
    Opcode = Mcasm::JCC_1;
    break;
  }
...
void McasmMCInstLower::Lower(const MachineInstr *MI, MCInst &OutMI) const {
  ...
  if (Mcasm::optimizeInstFromVEX3ToVEX2(OutMI, MI->getDesc()) ||
      Mcasm::optimizeShiftRotateWithImmediateOne(OutMI) ||
      Mcasm::optimizeVPCMPWithImmediateOneOrSix(OutMI) ||
      Mcasm::optimizeMOVSX(OutMI) || Mcasm::optimizeINCDEC(OutMI, In64BitMode) ||
      Mcasm::optimizeMOV(OutMI, In64BitMode) ||
      Mcasm::optimizeToFixedRegisterOrShortImmediateForm(OutMI))
    return;
  ...
  case Mcasm::LEA32r:
    ...
    assert(OutMI.getNumOperands() == 1 + Mcasm::AddrNumOperands &&
           "Unexpected # of LEA operands");
    assert(OutMI.getOperand(1 + Mcasm::AddrSegmentReg).getReg() == 0 &&
           "LEA has segment specified!");
    break;
  case Mcasm::MULX32Hrr:
  case Mcasm::MULX32Hrm:
  case Mcasm::MULX64Hrr:
  case Mcasm::MULX64Hrm: {
    // Turn into regular MULX by duplicating the destination.
    unsigned NewOpc;
    switch (OutMI.getOpcode()) {
    default: llvm_unreachable("Invalid opcode");
    case Mcasm::MULX32Hrr: NewOpc = Mcasm::MULX32rr; break;
    case Mcasm::MULX32Hrm: NewOpc = Mcasm::MULX32rm; break;
    case Mcasm::MULX64Hrr: NewOpc = Mcasm::MULX64rr; break;
    case Mcasm::MULX64Hrm: NewOpc = Mcasm::MULX64rm; break;
    }
  ...
  case Mcasm::CATCHRET: {
    ...
    const McasmSubtarget &Subtarget = AsmPrinter.getSubtarget();
    unsigned ReturnReg = In64BitMode ? Mcasm::RAX : Mcasm::EAX;
    OutMI = MCInst();
  ...
  case Mcasm::TAILJMPm64:
  case Mcasm::TAILJMPm64_REX:
    assert(OutMI.getNumOperands() == Mcasm::AddrNumOperands &&
           "Unexpected number of operands!");
    OutMI.setOpcode(convertTailJumpOpcode(OutMI.getOpcode()));
    break;
  case Mcasm::MASKMOVDQU:
  case Mcasm::VMASKMOVDQU:
    if (In64BitMode)
      OutMI.setFlags(Mcasm::IP_HAS_AD_SIZE);
    break;
  case Mcasm::BSF16rm:
  case Mcasm::BSF16rr:
  case Mcasm::BSF32rm:
  ...
  case Mcasm::BSF64rr: {
    ...
    if (!MF.getFunction().hasOptSize() && FlagDef && FlagDef->isDead())
      OutMI.setFlags(Mcasm::IP_HAS_REPEAT);
    break;
  }
  default:
    break;
...
void McasmAsmPrinter::LowerTlsAddr(McasmMCInstLower &MCInstLowering,
  ...
  Mcasm::Specifier Specifier;
  switch (MI.getOpcode()) {
  case Mcasm::TLS_addr32:
  case Mcasm::TLS_addr64:
  case Mcasm::TLS_addrX32:
    Specifier = Mcasm::S_TLSGD;
    break;
  ...
  case Mcasm::TLS_base_addrX32:
    Specifier = Mcasm::S_TLSLD;
    break;
  case Mcasm::TLS_desc32:
  case Mcasm::TLS_desc64:
    Specifier = Mcasm::S_TLSDESC;
    break;
  default:
  ...
  // code sequence using R_Mcasm_64_GOTPCREL (instead of R_Mcasm_64_GOTPCRELX) is
  ...
  bool UseGot = MMI->getModule()->getRtLibUseGOT() &&
                Ctx.getTargetOptions()->McasmRelaxRelocations;

  if (Specifier == Mcasm::S_TLSDESC) {
    const MCSymbolRefExpr *Expr = MCSymbolRefExpr::create(
        MCInstLowering.GetSymbolFromOperand(MI.getOperand(3)), Mcasm::S_TLSCALL,
        Ctx);
    EmitAndCountInstruction(
        MCInstBuilder(Is64BitsLP64 ? Mcasm::LEA64r : Mcasm::LEA32r)
            .addReg(Is64BitsLP64 ? Mcasm::RAX : Mcasm::EAX)
            .addReg(Is64Bits ? Mcasm::RIP : Mcasm::EBX)
            .addImm(1)
            .addReg(0)
            .addExpr(Sym)
            .addReg(0));
    EmitAndCountInstruction(
        MCInstBuilder(Is64Bits ? Mcasm::CALL64m : Mcasm::CALL32m)
            .addReg(Is64BitsLP64 ? Mcasm::RAX : Mcasm::EAX)
            .addImm(1)
            .addReg(0)
            .addExpr(Expr)
    ...
    bool NeedsPadding = Specifier == Mcasm::S_TLSGD;
    if (NeedsPadding && Is64BitsLP64)
      EmitAndCountInstruction(MCInstBuilder(Mcasm::DATA16_PREFIX));
    EmitAndCountInstruction(MCInstBuilder(Mcasm::LEA64r)
                                .addReg(Mcasm::RDI)
                                .addReg(Mcasm::RIP)
                                .addImm(1)
                                .addReg(0)
    ...
    if (NeedsPadding) {
      if (!UseGot)
        EmitAndCountInstruction(MCInstBuilder(Mcasm::DATA16_PREFIX));
      EmitAndCountInstruction(MCInstBuilder(Mcasm::DATA16_PREFIX));
      EmitAndCountInstruction(MCInstBuilder(Mcasm::REX64_PREFIX));
    }
    if (UseGot) {
      const MCExpr *Expr =
          MCSymbolRefExpr::create(TlsGetAddr, Mcasm::S_GOTPCREL, Ctx);
      EmitAndCountInstruction(MCInstBuilder(Mcasm::CALL64m)
                                  .addReg(Mcasm::RIP)
                                  .addImm(1)
                                  .addReg(0)
                                  .addExpr(Expr)
      ...
      EmitAndCountInstruction(
          MCInstBuilder(Mcasm::CALL64pcrel32)
              .addExpr(MCSymbolRefExpr::create(TlsGetAddr, Mcasm::S_PLT, Ctx)));
    }
  } else {
    if (Specifier == Mcasm::S_TLSGD && !UseGot) {
      EmitAndCountInstruction(MCInstBuilder(Mcasm::LEA32r)
                                  .addReg(Mcasm::EAX)
                                  .addReg(0)
                                  .addImm(1)
                                  .addReg(Mcasm::EBX)
    ...
    if (UseGot) {
      const MCExpr *Expr = MCSymbolRefExpr::create(TlsGetAddr, Mcasm::S_GOT, Ctx);
      EmitAndCountInstruction(MCInstBuilder(Mcasm::CALL32m)
                                  .addReg(Mcasm::EBX)
                                  .addImm(1)
                                  .addReg(0)
                                  .addExpr(Expr)
      ...
      EmitAndCountInstruction(
          MCInstBuilder(Mcasm::CALLpcrel32)
              .addExpr(MCSymbolRefExpr::create(TlsGetAddr, Mcasm::S_PLT, Ctx)));
    }
  }
}
...
static unsigned emitNop(MCStreamer &OS, unsigned NumBytes,
  ...
  if (Subtarget->is64Bit()) {
    ...
    if (Subtarget->hasFeature(Mcasm::TuningFast7ByteNOP))
      MaxNopLength = 7;
    else if (Subtarget->hasFeature(Mcasm::TuningFast15ByteNOP))
      MaxNopLength = 15;
    else if (Subtarget->hasFeature(Mcasm::TuningFast11ByteNOP))
      MaxNopLength = 11;
    else
      MaxNopLength = 10;
  ...
  case Mcasm::NOOP:
    ...
    break;
  case Mcasm::XCHG16ar:
    OS.emitInstruction(MCInstBuilder(Opc).addReg(Mcasm::AX).addReg(Mcasm::AX),
                       *Subtarget);
    break;
  case Mcasm::NOOPL:
  case Mcasm::NOOPW:
...
/// Emit the optimal amount of multi-byte nops on Mcasm.
static void emitMcasmNops(MCStreamer &OS, unsigned NumBytes,
                        const McasmSubtarget *Subtarget) {
...
void McasmAsmPrinter::LowerSTATEPOINT(const MachineInstr &MI,
                                    McasmMCInstLower &MCIL) {
  assert(Subtarget->is64Bit() && "Statepoint currently only supports Mcasm-64");

  NoAutoPaddingScope NoPadScope(*OutStreamer);

  StatepointOpers SOpers(&MI);
  if (unsigned PatchBytes = SOpers.getNumPatchBytes()) {
    emitMcasmNops(*OutStreamer, PatchBytes, Subtarget);
  } else {
    ...
    case MachineOperand::MO_ExternalSymbol:
      ...
      CallOpcode = Mcasm::CALL64pcrel32;
...
void McasmAsmPrinter::LowerFAULTING_OP(const MachineInstr &FaultingMI,
                                     McasmMCInstLower &MCIL) {
  ...
  if (DefRegister != Mcasm::NoRegister)
...
void McasmAsmPrinter::LowerFENTRY_CALL(const MachineInstr &MI,
                                     McasmMCInstLower &MCIL) {
  ...
  EmitAndCountInstruction(
      MCInstBuilder(Is64Bits ? Mcasm::CALL64pcrel32 : Mcasm::CALLpcrel32)
          .addExpr(Op));
}

void McasmAsmPrinter::LowerKCFI_CHECK(const MachineInstr &MI) {
  assert(std::next(MI.getIterator())->isCall() &&
         "KCFI_CHECK not followed by a call instruction");

  // Adjust the offset for patchable-function-prefix. McasmInstrInfo::getNop()
  // returns a 1-byte Mcasm::NOOP, which means the offset is the same in
  ...
  unsigned TempReg = AddrReg == Mcasm::R10 ? Mcasm::R11D : Mcasm::R10D;
  EmitAndCountInstruction(
      MCInstBuilder(Mcasm::MOV32ri).addReg(TempReg).addImm(-MaskKCFIType(Type)));
  EmitAndCountInstruction(MCInstBuilder(Mcasm::ADD32rm)
  ...
  EmitAndCountInstruction(
      MCInstBuilder(Mcasm::JCC_1)
          .addExpr(MCSymbolRefExpr::create(Pass, OutContext))
          .addImm(Mcasm::COND_E));

  MCSymbol *Trap = OutContext.createTempSymbol();
  OutStreamer->emitLabel(Trap);
  EmitAndCountInstruction(MCInstBuilder(Mcasm::TRAP));
  emitKCFITrapEntry(MF, Trap);
  OutStreamer->emitLabel(Pass);
}

void McasmAsmPrinter::LowerASAN_CHECK_MEMACCESS(const MachineInstr &MI) {
...
void McasmAsmPrinter::LowerPATCHABLE_OP(const MachineInstr &MI,
                                      McasmMCInstLower &MCIL) {
  ...
  if (Code.size() < MinSize) {
    if (MinSize == 2 && Subtarget->is32Bit() &&
      ...
      OutStreamer->emitInstruction(
          MCInstBuilder(Mcasm::MOV32rr_REV).addReg(Mcasm::EDI).addReg(Mcasm::EDI),
          *Subtarget);
    } else {
...
// <id>, <shadowBytes>, ...
void McasmAsmPrinter::LowerSTACKMAP(const MachineInstr &MI) {
...
void McasmAsmPrinter::LowerPATCHPOINT(const MachineInstr &MI,
                                    McasmMCInstLower &MCIL) {
  assert(Subtarget->is64Bit() && "Patchpoint currently only supports Mcasm-64");
  ...
  if (!(CalleeMO.isImm() && !CalleeMO.getImm())) {
    ...
    if (McasmII::isMcasm_64ExtendedReg(ScratchReg))
      EncodedBytes = 13;
    else
      EncodedBytes = 12;

    EmitAndCountInstruction(
        MCInstBuilder(Mcasm::MOV64ri).addReg(ScratchReg).addOperand(CalleeMCOp));
    // FIXME: Add retpoline support and remove this.
    if (Subtarget->useIndirectThunkCalls())
      report_fatal_error(
          "Lowering patchpoint with thunks not yet implemented.");
    EmitAndCountInstruction(MCInstBuilder(Mcasm::CALL64r).addReg(ScratchReg));
  }
  ...
  emitMcasmNops(*OutStreamer, NumBytes - EncodedBytes, Subtarget);
}

void McasmAsmPrinter::LowerPATCHABLE_EVENT_CALL(const MachineInstr &MI,
                                              McasmMCInstLower &MCIL) {
  assert(Subtarget->is64Bit() && "XRay custom events only supports Mcasm-64");
  ...
  const Register DestRegs[] = {Mcasm::RDI, Mcasm::RSI};
  ...
  for (unsigned I = 0; I < MI.getNumOperands(); ++I)
    if (auto Op = MCIL.LowerMachineOperand(&MI, MI.getOperand(I));
      ...
      SrcRegs[I] = getMcasmSubSuperRegister(Op.getReg(), 64);
      assert(SrcRegs[I].isValid() && "Invalid operand");
      if (SrcRegs[I] != DestRegs[I]) {
        UsedMask[I] = true;
        EmitAndCountInstruction(
            MCInstBuilder(Mcasm::PUSH64r).addReg(DestRegs[I]));
      } else {
        emitMcasmNops(*OutStreamer, 4, Subtarget);
      }
    }
  ...
  for (unsigned I = 0; I < MI.getNumOperands(); ++I)
    if (SrcRegs[I] != DestRegs[I])
      EmitAndCountInstruction(
          MCInstBuilder(Mcasm::MOV64rr).addReg(DestRegs[I]).addReg(SrcRegs[I]));
  ...
  if (isPositionIndependent())
    TOp.setTargetFlags(McasmII::MO_PLT);

  // Emit the call instruction.
  EmitAndCountInstruction(MCInstBuilder(Mcasm::CALL64pcrel32)
                              .addOperand(MCIL.LowerSymbolOperand(TOp, TSym)));

  // Restore caller-saved and used registers.
  for (unsigned I = sizeof UsedMask; I-- > 0;)
    if (UsedMask[I])
      EmitAndCountInstruction(MCInstBuilder(Mcasm::POP64r).addReg(DestRegs[I]));
    else
      emitMcasmNops(*OutStreamer, 1, Subtarget);
...
void McasmAsmPrinter::LowerPATCHABLE_TYPED_EVENT_CALL(const MachineInstr &MI,
                                                    McasmMCInstLower &MCIL) {
  assert(Subtarget->is64Bit() && "XRay typed events only supports Mcasm-64");
  ...
  const Register DestRegs[] = {Mcasm::RDI, Mcasm::RSI, Mcasm::RDX};
...
void McasmAsmPrinter::LowerPATCHABLE_FUNCTION_ENTER(const MachineInstr &MI,
                                                  McasmMCInstLower &MCIL) {
  ...
  if (F.hasFnAttribute("patchable-function-entry")) {
    ...
    if (F.getFnAttribute("patchable-function-entry")
            ...
            .getAsInteger(10, Num))
      return;
    emitMcasmNops(*OutStreamer, Num, Subtarget);
    return;
  }
  ...
  emitMcasmNops(*OutStreamer, 9, Subtarget);
  recordSled(CurSled, MI, SledKind::FUNCTION_ENTER, 2);
}

void McasmAsmPrinter::LowerPATCHABLE_RET(const MachineInstr &MI,
                                       McasmMCInstLower &MCIL) {
  NoAutoPaddingScope NoPadScope(*OutStreamer);
    ...
    EmitToStreamer(
        *OutStreamer,
        MCInstBuilder(Mcasm::JCC_1)
            ...
            .addImm(Mcasm::GetOppositeBranchCondition(
                static_cast<Mcasm::CondCode>(MI.getOperand(2).getImm()))));
    TC.setOpcode(Mcasm::JMP_1);
...
static unsigned getSrcIdx(const MachineInstr* MI, unsigned SrcIdx) {
  if (McasmII::isKMasked(MI->getDesc().TSFlags)) {
    // Skip mask operand.
    ++SrcIdx;
    if (McasmII::isKMergeMasked(MI->getDesc().TSFlags)) {
...
static void printDstRegisterName(raw_ostream &CS, const MachineInstr *MI,
  ...
  CS << McasmATTInstPrinter::getRegisterName(DstOp.getReg());
  ...
  if (McasmII::isKMasked(MI->getDesc().TSFlags)) {
    ...
    StringRef Mask = McasmATTInstPrinter::getRegisterName(WriteMaskOp.getReg());
    CS << " {%" << Mask << "}";
    if (!McasmII::isKMergeMasked(MI->getDesc().TSFlags)) {
      CS << " {z}";
    }
...
static std::string getShuffleComment(const MachineInstr *MI, unsigned SrcOp1Idx,
  ...
  StringRef Src1Name = SrcOp1.isReg()
                           ? McasmATTInstPrinter::getRegisterName(SrcOp1.getReg())
                           : "mem";
  StringRef Src2Name = SrcOp2.isReg()
                           ? McasmATTInstPrinter::getRegisterName(SrcOp2.getReg())
                           : "mem";
...
static void printZeroUpperMove(const MachineInstr *MI, MCStreamer &OutStreamer,
  ...
  if (auto *C = Mcasm::getConstantFromPool(*MI, SrcIdx)) {
    CS << "[";
...
static bool printExtend(const MachineInstr *MI, MCStreamer &OutStreamer,
  ...
  unsigned SrcIdx = getSrcIdx(MI, 1);
  auto *C = Mcasm::getConstantFromPool(*MI, SrcIdx);
...
static void printZeroExtend(const MachineInstr *MI, MCStreamer &OutStreamer,
  ...
  unsigned Width = Mcasm::getVectorRegisterWidth(MI->getDesc().operands()[0]);
...
void McasmAsmPrinter::EmitSEHInstruction(const MachineInstr *MI) {
  ...
  case Mcasm::SEH_BeginEpilogue:
    OutStreamer->emitWinCFIBeginEpilogue();
    break;

  case Mcasm::SEH_EndEpilogue:
    OutStreamer->emitWinCFIEndEpilogue();
    break;

  case Mcasm::SEH_UnwindV2Start:
    OutStreamer->emitWinCFIUnwindV2Start();
    break;

  case Mcasm::SEH_UnwindVersion:
...
static void addConstantComments(const MachineInstr *MI,
  ...
  case Mcasm::PSHUFBrm:
  case Mcasm::VPSHUFBrm:
  case Mcasm::VPSHUFBYrm:
  case Mcasm::VPSHUFBZ128rm:
  case Mcasm::VPSHUFBZ128rmk:
  case Mcasm::VPSHUFBZ128rmkz:
  case Mcasm::VPSHUFBZ256rm:
  case Mcasm::VPSHUFBZ256rmk:
  case Mcasm::VPSHUFBZ256rmkz:
  case Mcasm::VPSHUFBZrm:
  case Mcasm::VPSHUFBZrmk:
  case Mcasm::VPSHUFBZrmkz: {
    unsigned SrcIdx = getSrcIdx(MI, 1);
    if (auto *C = Mcasm::getConstantFromPool(*MI, SrcIdx + 1)) {
      unsigned Width = Mcasm::getVectorRegisterWidth(MI->getDesc().operands()[0]);
  ...
  case Mcasm::VPERMILPSrm:
  case Mcasm::VPERMILPSYrm:
  case Mcasm::VPERMILPSZ128rm:
  case Mcasm::VPERMILPSZ128rmk:
  case Mcasm::VPERMILPSZ128rmkz:
  case Mcasm::VPERMILPSZ256rm:
  case Mcasm::VPERMILPSZ256rmk:
  case Mcasm::VPERMILPSZ256rmkz:
  case Mcasm::VPERMILPSZrm:
  case Mcasm::VPERMILPSZrmk:
  case Mcasm::VPERMILPSZrmkz: {
    unsigned SrcIdx = getSrcIdx(MI, 1);
    if (auto *C = Mcasm::getConstantFromPool(*MI, SrcIdx + 1)) {
      unsigned Width = Mcasm::getVectorRegisterWidth(MI->getDesc().operands()[0]);
  ...
  case Mcasm::VPERMILPDrm:
  case Mcasm::VPERMILPDYrm:
  case Mcasm::VPERMILPDZ128rm:
  case Mcasm::VPERMILPDZ128rmk:
  case Mcasm::VPERMILPDZ128rmkz:
  case Mcasm::VPERMILPDZ256rm:
  case Mcasm::VPERMILPDZ256rmk:
  case Mcasm::VPERMILPDZ256rmkz:
  case Mcasm::VPERMILPDZrm:
  case Mcasm::VPERMILPDZrmk:
  case Mcasm::VPERMILPDZrmkz: {
    unsigned SrcIdx = getSrcIdx(MI, 1);
    if (auto *C = Mcasm::getConstantFromPool(*MI, SrcIdx + 1)) {
      unsigned Width = Mcasm::getVectorRegisterWidth(MI->getDesc().operands()[0]);
      SmallVector<int, 16> Mask;
  ...
  case Mcasm::VPERMIL2PDrm:
  case Mcasm::VPERMIL2PSrm:
  case Mcasm::VPERMIL2PDYrm:
  case Mcasm::VPERMIL2PSYrm: {
    assert(MI->getNumOperands() >= (3 + Mcasm::AddrNumOperands + 1) &&
           "Unexpected number of operands!");
    ...
    switch (MI->getOpcode()) {
    default: llvm_unreachable("Invalid opcode");
    case Mcasm::VPERMIL2PSrm: case Mcasm::VPERMIL2PSYrm: ElSize = 32; break;
    case Mcasm::VPERMIL2PDrm: case Mcasm::VPERMIL2PDYrm: ElSize = 64; break;
    }

    if (auto *C = Mcasm::getConstantFromPool(*MI, 3)) {
      unsigned Width = Mcasm::getVectorRegisterWidth(MI->getDesc().operands()[0]);
      SmallVector<int, 16> Mask;
  ...
  case Mcasm::MMX_MOVQ64rm: {
    if (auto *C = Mcasm::getConstantFromPool(*MI, 1)) {
      std::string Comment;
      raw_string_ostream CS(Comment);
      const MachineOperand &DstOp = MI->getOperand(0);
      CS << McasmATTInstPrinter::getRegisterName(DstOp.getReg()) << " = ";
...
#define INSTR_CASE(Prefix, Instr, Suffix, Postfix)                             \
  case Mcasm::Prefix##Instr##Suffix##rm##Postfix:
    ...
    CASE_ARITH_RM(PMULHRSW) {
      ...
      if (auto *C = Mcasm::getConstantFromPool(*MI, SrcIdx + 1)) {
        ...
        unsigned VectorWidth =
            Mcasm::getVectorRegisterWidth(MI->getDesc().operands()[0]);
        CS << "[";
...
#define MASK_AVX512_CASE(Instr)                                                \
  ...
  case Mcasm::MOVSDrm:
  case Mcasm::VMOVSDrm:
  MASK_AVX512_CASE(Mcasm::VMOVSDZrm)
  case Mcasm::MOVSDrm_alt:
  case Mcasm::VMOVSDrm_alt:
  case Mcasm::VMOVSDZrm_alt:
  case Mcasm::MOVQI2PQIrm:
  case Mcasm::VMOVQI2PQIrm:
  case Mcasm::VMOVQI2PQIZrm:
    printZeroUpperMove(MI, OutStreamer, 64, 128, "mem[0],zero");
    break;

  MASK_AVX512_CASE(Mcasm::VMOVSHZrm)
  ...
  case Mcasm::MOVSSrm:
  case Mcasm::VMOVSSrm:
  MASK_AVX512_CASE(Mcasm::VMOVSSZrm)
  case Mcasm::MOVSSrm_alt:
  case Mcasm::VMOVSSrm_alt:
  case Mcasm::VMOVSSZrm_alt:
  case Mcasm::MOVDI2PDIrm:
  case Mcasm::VMOVDI2PDIrm:
  case Mcasm::VMOVDI2PDIZrm:
    printZeroUpperMove(MI, OutStreamer, 32, 128, "mem[0],zero,zero,zero");
    break;

#define MOV_CASE(Prefix, Suffix)                                               \
  case Mcasm::Prefix##MOVAPD##Suffix##rm:                                        \
  case Mcasm::Prefix##MOVAPS##Suffix##rm:                                        \
  case Mcasm::Prefix##MOVUPD##Suffix##rm:                                        \
  case Mcasm::Prefix##MOVUPS##Suffix##rm:                                        \
  case Mcasm::Prefix##MOVDQA##Suffix##rm:                                        \
  case Mcasm::Prefix##MOVDQU##Suffix##rm:

#define MOV_AVX512_CASE(Suffix, Postfix)                                       \
  case Mcasm::VMOVDQA64##Suffix##rm##Postfix:                                    \
  case Mcasm::VMOVDQA32##Suffix##rm##Postfix:                                    \
  case Mcasm::VMOVDQU64##Suffix##rm##Postfix:                                    \
  case Mcasm::VMOVDQU32##Suffix##rm##Postfix:                                    \
  case Mcasm::VMOVDQU16##Suffix##rm##Postfix:                                    \
  case Mcasm::VMOVDQU8##Suffix##rm##Postfix:                                     \
  case Mcasm::VMOVAPS##Suffix##rm##Postfix:                                      \
  case Mcasm::VMOVAPD##Suffix##rm##Postfix:                                      \
  case Mcasm::VMOVUPS##Suffix##rm##Postfix:                                      \
  case Mcasm::VMOVUPD##Suffix##rm##Postfix:
...
#define CASE_512_MOV_RM()                                                      \
  ...
  MOV_AVX512_CASE(Z, kz)                                                       \
    ...
    break;
  case Mcasm::VBROADCASTF128rm:
  case Mcasm::VBROADCASTI128rm:
  MASK_AVX512_CASE(Mcasm::VBROADCASTF32X4Z256rm)
  MASK_AVX512_CASE(Mcasm::VBROADCASTF64X2Z256rm)
  MASK_AVX512_CASE(Mcasm::VBROADCASTI32X4Z256rm)
  MASK_AVX512_CASE(Mcasm::VBROADCASTI64X2Z256rm)
    printBroadcast(MI, OutStreamer, 2, 128);
    break;
  MASK_AVX512_CASE(Mcasm::VBROADCASTF32X4Zrm)
  MASK_AVX512_CASE(Mcasm::VBROADCASTF64X2Zrm)
  MASK_AVX512_CASE(Mcasm::VBROADCASTI32X4Zrm)
  MASK_AVX512_CASE(Mcasm::VBROADCASTI64X2Zrm)
    printBroadcast(MI, OutStreamer, 4, 128);
    break;
  MASK_AVX512_CASE(Mcasm::VBROADCASTF32X8Zrm)
  MASK_AVX512_CASE(Mcasm::VBROADCASTF64X4Zrm)
  MASK_AVX512_CASE(Mcasm::VBROADCASTI32X8Zrm)
  MASK_AVX512_CASE(Mcasm::VBROADCASTI64X4Zrm)
  ...
  case Mcasm::MOVDDUPrm:
  case Mcasm::VMOVDDUPrm:
  MASK_AVX512_CASE(Mcasm::VMOVDDUPZ128rm)
  case Mcasm::VPBROADCASTQrm:
  MASK_AVX512_CASE(Mcasm::VPBROADCASTQZ128rm)
    printBroadcast(MI, OutStreamer, 2, 64);
    break;
  case Mcasm::VBROADCASTSDYrm:
  MASK_AVX512_CASE(Mcasm::VBROADCASTSDZ256rm)
  case Mcasm::VPBROADCASTQYrm:
  MASK_AVX512_CASE(Mcasm::VPBROADCASTQZ256rm)
    printBroadcast(MI, OutStreamer, 4, 64);
    break;
  MASK_AVX512_CASE(Mcasm::VBROADCASTSDZrm)
  MASK_AVX512_CASE(Mcasm::VPBROADCASTQZrm)
    printBroadcast(MI, OutStreamer, 8, 64);
    break;
  case Mcasm::VBROADCASTSSrm:
  MASK_AVX512_CASE(Mcasm::VBROADCASTSSZ128rm)
  case Mcasm::VPBROADCASTDrm:
  MASK_AVX512_CASE(Mcasm::VPBROADCASTDZ128rm)
    printBroadcast(MI, OutStreamer, 4, 32);
    break;
  case Mcasm::VBROADCASTSSYrm:
    MASK_AVX512_CASE(Mcasm::VBROADCASTSSZ256rm)
  case Mcasm::VPBROADCASTDYrm:
  MASK_AVX512_CASE(Mcasm::VPBROADCASTDZ256rm)
    printBroadcast(MI, OutStreamer, 8, 32);
    break;
  MASK_AVX512_CASE(Mcasm::VBROADCASTSSZrm)
  MASK_AVX512_CASE(Mcasm::VPBROADCASTDZrm)
    printBroadcast(MI, OutStreamer, 16, 32);
    break;
  case Mcasm::VPBROADCASTWrm:
  MASK_AVX512_CASE(Mcasm::VPBROADCASTWZ128rm)
    printBroadcast(MI, OutStreamer, 8, 16);
    break;
  case Mcasm::VPBROADCASTWYrm:
  MASK_AVX512_CASE(Mcasm::VPBROADCASTWZ256rm)
    printBroadcast(MI, OutStreamer, 16, 16);
    break;
  MASK_AVX512_CASE(Mcasm::VPBROADCASTWZrm)
    printBroadcast(MI, OutStreamer, 32, 16);
    break;
  case Mcasm::VPBROADCASTBrm:
  MASK_AVX512_CASE(Mcasm::VPBROADCASTBZ128rm)
    printBroadcast(MI, OutStreamer, 16, 8);
    break;
  case Mcasm::VPBROADCASTBYrm:
  MASK_AVX512_CASE(Mcasm::VPBROADCASTBZ256rm)
    printBroadcast(MI, OutStreamer, 32, 8);
    break;
  MASK_AVX512_CASE(Mcasm::VPBROADCASTBZrm)
    printBroadcast(MI, OutStreamer, 64, 8);
    break;

#define MOVX_CASE(Prefix, Ext, Type, Suffix, Postfix)                          \
  case Mcasm::Prefix##PMOV##Ext##Type##Suffix##rm##Postfix:
...
bool isImportedFunction(const MachineOperand &MO) {
  return MO.isGlobal() && (MO.getTargetFlags() == McasmII::MO_DLLIMPORT);
}

// Is the given instruction a call to a CFGuard function?
bool isCallToCFGuardFunction(const MachineInstr *MI) {
  assert(MI->getOpcode() == Mcasm::TAILJMPm64_REX ||
         MI->getOpcode() == Mcasm::CALL64m);
  const MachineOperand &MO = MI->getOperand(3);
  return MO.isGlobal() && (MO.getTargetFlags() == McasmII::MO_NO_FLAG) &&
...
void McasmAsmPrinter::emitInstruction(const MachineInstr *MI) {
  ...
  // Mcasm_MC::verifyInstructionPredicates(MI->getOpcode(),
  //                                     Subtarget->getFeatureBits());

  McasmMCInstLower MCInstLowering(*MF, *this);
  const McasmRegisterInfo *RI =
      MF->getSubtarget<McasmSubtarget>().getRegisterInfo();

  if (MI->getOpcode() == Mcasm::OR64rm) {
  ...
  if (TM.Options.MCOptions.ShowMCEncoding) {
    if (MI->getAsmPrinterFlags() & Mcasm::AC_EVEX_2_LEGACY)
      OutStreamer->AddComment("EVEX TO LEGACY Compression ", false);
    else if (MI->getAsmPrinterFlags() & Mcasm::AC_EVEX_2_VEX)
      OutStreamer->AddComment("EVEX TO VEX Compression ", false);
    else if (MI->getAsmPrinterFlags() & Mcasm::AC_EVEX_2_EVEX)
  ...
  case Mcasm::EH_RETURN64: {
    ...
    OutStreamer->AddComment(StringRef("eh_return, addr: %") +
                            McasmATTInstPrinter::getRegisterName(Reg));
    break;
  }
  case Mcasm::CLEANUPRET: {
    // Lower these as normal, but add some comments.
  ...
  case Mcasm::TAILJMPd64:
    if (IndCSPrefix && MI->hasRegisterImplicitUseOperand(Mcasm::R11))
      EmitAndCountInstruction(MCInstBuilder(Mcasm::CS_PREFIX));
  ...
  case Mcasm::TAILJMPr64_REX: {
    if (EnableImportCallOptimization) {
      assert(MI->getOperand(0).getReg() == Mcasm::RAX &&
  ...
  case Mcasm::MOVPC32r: {
    ...
    const McasmFrameLowering *FrameLowering =
        MF->getSubtarget<McasmSubtarget>().getFrameLowering();
    ...
    // popl $reg
    EmitAndCountInstruction(
        MCInstBuilder(Mcasm::POP32r).addReg(MI->getOperand(0).getReg()));
  ...
  case Mcasm::ADD32ri: {
    ...
    if (MI->getOperand(2).getTargetFlags() != McasmII::MO_GOT_ABSOLUTE_ADDRESS)
      break;
    ...
    EmitAndCountInstruction(MCInstBuilder(Mcasm::ADD32ri)
  ...
  case Mcasm::MORESTACK_RET:
    EmitAndCountInstruction(MCInstBuilder(getRetOpcode(*Subtarget)));
    return;

  case Mcasm::KCFI_CHECK:
    return LowerKCFI_CHECK(*MI);

  case Mcasm::ASAN_CHECK_MEMACCESS:
    return LowerASAN_CHECK_MEMACCESS(*MI);

  case Mcasm::MORESTACK_RET_RESTORE_R10:
    // Return, then restore R10.
    EmitAndCountInstruction(MCInstBuilder(getRetOpcode(*Subtarget)));
    EmitAndCountInstruction(
        MCInstBuilder(Mcasm::MOV64rr).addReg(Mcasm::R10).addReg(Mcasm::RAX));
    return;
  ...
  case Mcasm::SEH_SplitChainedAtEndOfBlock:
  ...
  case Mcasm::UBSAN_UD1:
    EmitAndCountInstruction(MCInstBuilder(Mcasm::UD1Lm)
                                .addReg(Mcasm::EAX)
                                .addReg(Mcasm::EAX)
                                .addImm(1)
                                .addReg(Mcasm::NoRegister)
  ...
  case Mcasm::JCC_1:
    ...
    if (getSubtarget().hasBranchHint() && EnableBranchHint) {
      ...
      if (EdgeProb > Threshold)
        EmitAndCountInstruction(MCInstBuilder(Mcasm::DS_PREFIX));
    }
    break;
  }
...
void McasmAsmPrinter::emitInlineAsmEnd(const MCSubtargetInfo &StartInfo,
...
void McasmAsmPrinter::emitCallInstruction(const llvm::MCInst &MCI) {
...
void McasmAsmPrinter::maybeEmitNopAfterCallForWindowsEH(const MachineInstr *MI) {
  ...
  for (;;) {
    ...
    for (; MBBI != MBBE; ++MBBI) {
      ...
      if (NextMI.isEHLabel()) {
        if (HasEHPersonality) {
          EmitAndCountInstruction(MCInstBuilder(Mcasm::NOOP));
          return;
        }
      ...
      // behavior as MSVC.
      if (NextMI.getOpcode() == Mcasm::SEH_BeginEpilogue) {
        EmitAndCountInstruction(MCInstBuilder(Mcasm::NOOP));
        return;
      }
    ...
    if (MFI == MFE) {
      ...
      if (HasEHPersonality) {
        ...
        if (MI->getParent()->succ_empty())
          EmitAndCountInstruction(MCInstBuilder(Mcasm::INT3));
        else
          EmitAndCountInstruction(MCInstBuilder(Mcasm::NOOP));
      }
      return;
...
void McasmAsmPrinter::emitLabelAndRecordForImportCallOptimization(
    ImportCallKind Kind) {
