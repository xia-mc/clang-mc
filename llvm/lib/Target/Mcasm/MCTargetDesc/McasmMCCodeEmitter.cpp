//===-- McasmMCCodeEmitter.cpp - Convert Mcasm code to machine code -----------===//
//
...
public:
  ...
  void setX(const MCInst &MI, unsigned OpNum, unsigned Shift = 3) {
    ...
    if (Shift != 3 && McasmII::isApxExtendedReg(Reg))
      return;
  ...
  void setXX2(const MCInst &MI, unsigned OpNum) {
    ...
    if (Kind <= REX2 || McasmII::isApxExtendedReg(Reg))
      setX2(Encoding);
  }
  ...
  void setV2(const MCInst &MI, unsigned OpNum, bool HasVEX_4V) {
    ...
    if (HasVEX_4V)
      return;
    MCRegister Reg = MI.getOperand(OpNum).getReg();
    if (McasmII::isApxExtendedReg(Reg))
      return;
  ...
  McasmOpcodePrefixHelper(const MCRegisterInfo &MRI)
...
public:
  McasmMCCodeEmitter(const MCInstrInfo &mcii, MCContext &ctx)
      : MCII(mcii), Ctx(ctx) {}
  McasmMCCodeEmitter(const McasmMCCodeEmitter &) = delete;
  McasmMCCodeEmitter &operator=(const McasmMCCodeEmitter &) = delete;
...
private:
  unsigned getMcasmRegNum(const MCOperand &MO) const;

  unsigned getMcasmRegEncoding(const MCInst &MI, unsigned OpNum) const;
...
static bool isDispOrCDisp8(uint64_t TSFlags, int Value, int &ImmOffset) {
  bool HasEVEX = (TSFlags & McasmII::EncodingMask) == McasmII::EVEX;

  unsigned CD8_Scale =
      (TSFlags & McasmII::CD8_Scale_Mask) >> McasmII::CD8_Scale_Shift;
...
static MCFixupKind getImmFixupKind(uint64_t TSFlags) {
  unsigned Size = McasmII::getSizeOfImm(TSFlags);
  if (McasmII::isImmSigned(TSFlags)) {
    switch (Size) {
    default:
      llvm_unreachable("Unsupported signed fixup size!");
    case 4:
      return Mcasm::reloc_signed_4byte;
    }
  }
  switch (Size) {
  default:
...
/// binary expression.
///
/// TODO: Move this to McasmAsmBackend.cpp at relocation decision phase so that we
...
static bool hasSecRelSymbolRef(const MCExpr *Expr) {
  if (Expr->getKind() == MCExpr::SymbolRef) {
    ...
    return Ref->getSpecifier() == Mcasm::S_COFF_SECREL;
  }
  return false;
}
...
static bool isPCRel32Branch(const MCInst &MI, const MCInstrInfo &MCII) {
  ...
  if ((Opcode != Mcasm::CALL64pcrel32 && Opcode != Mcasm::JMP_4 &&
       Opcode != Mcasm::JCC_4) ||
      !(getImmFixupKind(Desc.TSFlags) == FK_Data_4 &&
        McasmII::isImmPCRel(Desc.TSFlags)))
    return false;

  unsigned CurOp = McasmII::getOperandBias(Desc);
  const MCOperand &Op = MI.getOperand(CurOp);
  if (!Op.isExpr())
    return false;

  auto *Ref = dyn_cast<MCSymbolRefExpr>(Op.getExpr());
  return Ref && Ref->getSpecifier() == Mcasm::S_None;
}

unsigned McasmMCCodeEmitter::getMcasmRegNum(const MCOperand &MO) const {
  return Ctx.getRegisterInfo()->getEncodingValue(MO.getReg()) & 0x7;
}

unsigned McasmMCCodeEmitter::getMcasmRegEncoding(const MCInst &MI,
                                             unsigned OpNum) const {
  return Ctx.getRegisterInfo()->getEncodingValue(MI.getOperand(OpNum).getReg());
}

void McasmMCCodeEmitter::emitImmediate(const MCOperand &DispOp, SMLoc Loc,
  ...
  if ((FixupKind == FK_Data_4 || FixupKind == FK_Data_8 ||
       FixupKind == Mcasm::reloc_signed_4byte)) {
    GlobalOffsetTableExprKind Kind = startsWithGlobalOffsetTable(Expr);
    if (Kind != GOT_None) {
      assert(ImmOffset == 0);

      if (Size == 8) {
        FixupKind = FirstLiteralRelocationKind + ELF::R_Mcasm_64_GOTPC64;
      } else {
        assert(Size == 4);
        FixupKind = Mcasm::reloc_global_offset_table;
      }
...
void McasmMCCodeEmitter::emitRegModRMByte(const MCOperand &ModRMReg,
                                        unsigned RegOpcodeFld,
                                        SmallVectorImpl<char> &CB) const {
  emitByte(modRMByte(3, RegOpcodeFld, getMcasmRegNum(ModRMReg)), CB);
}

void McasmMCCodeEmitter::emitSIBByte(unsigned SS, unsigned Index, unsigned Base,
                                   SmallVectorImpl<char> &CB) const {
  // SIB byte is in the same format as the modRMByte.
  emitByte(modRMByte(SS, Index, Base), CB);
}

void McasmMCCodeEmitter::emitMemModRMByte(
  ...
  if (BaseReg == Mcasm::RIP ||
      BaseReg == Mcasm::EIP) { // [disp32+rIP] in Mcasm-64 mode
    assert(STI.hasFeature(Mcasm::Is64Bit) &&
    ...
    unsigned FixupKind = [&]() {
      ...
      if (!(Disp.isExpr() && isa<MCSymbolRefExpr>(Disp.getExpr())))
        return Mcasm::reloc_riprel_4byte;
      ...
      case Mcasm::MOV64rm:
        ...
        // flexible R_Mcasm_64_REX_GOTPCRELX/R_Mcasm_64_CODE_4_GOTPCRELX relaxation.
        return Kind == REX2 ? Mcasm::reloc_riprel_4byte_movq_load_rex2
                            : Mcasm::reloc_riprel_4byte_movq_load;
      case Mcasm::ADC32rm:
      case Mcasm::ADD32rm:
      case Mcasm::AND32rm:
      case Mcasm::CMP32rm:
      ...
      case Mcasm::XOR64rm:
      case Mcasm::LEA64r:
        return Kind == REX2  ? Mcasm::reloc_riprel_4byte_relax_rex2
               : Kind == REX ? Mcasm::reloc_riprel_4byte_relax_rex
                             : Mcasm::reloc_riprel_4byte_relax;
      case Mcasm::ADD64rm_NF:
      case Mcasm::ADD64rm_ND:
      case Mcasm::ADD64mr_ND:
      case Mcasm::ADD64mr_NF_ND:
      case Mcasm::ADD64rm_NF_ND:
        return Mcasm::reloc_riprel_4byte_relax_evex;
      }
    }();
    ...
    int ImmSize = !Disp.isImm() && McasmII::hasImm(TSFlags)
                      ? McasmII::getSizeOfImm(TSFlags)
                      : 0;
  ...
  unsigned BaseRegNo = BaseReg ? getMcasmRegNum(Base) : -1U;

  bool IsAdSize16 = STI.hasFeature(Mcasm::Is32Bit) &&
                    (TSFlags & McasmII::AdSizeMask) == McasmII::AdSize16;

  // 16-bit addressing forms of the ModR/M byte have a different encoding for
  // the R/M field and are far more limited in which registers can be used.
  if (IsAdSize16 || Mcasm_MC::is16BitMemOperand(MI, Op, STI)) {
    if (BaseReg) {
      // For 32-bit addressing, the row and column values in Table 2-2 are
      // basically the same. It's AX/CX/DX/BX/SP/BP/SI/DI in that order, with
      // some special cases. And getMcasmRegNum reflects that numbering.
      ...
      if (IndexReg.getReg()) {
        unsigned IndexReg16 = R16Table[getMcasmRegNum(IndexReg)];
  ...
  bool UseDisp8 = MI.getFlags() & Mcasm::IP_USE_DISP8;
  bool UseDisp32 = MI.getFlags() & Mcasm::IP_USE_DISP32;
  ...
  if (!ForceSIB && !McasmII::needSIB(BaseReg, IndexReg.getReg(),
                                   STI.hasFeature(Mcasm::Is64Bit))) {
    if (!BaseReg) { // [disp32]     in Mcasm-32 mode
      emitByte(modRMByte(0, RegOpcodeField, 5), CB);
    ...
    if (BaseRegNo != N86::EBP) {
      ...
      if (Disp.isExpr()) {
        ...
        if (Sym && Sym->getSpecifier() == Mcasm::S_TLSCALL) {
          // This is exclusively used by call *a@tlscall(base). The relocation
          // (R_386_TLSCALL or R_Mcasm_64_TLSCALL) applies to the beginning.
    ...
    unsigned Opcode = MI.getOpcode();
    unsigned FixupKind = Opcode == Mcasm::MOV32rm ? Mcasm::reloc_signed_4byte_relax
                                                : Mcasm::reloc_signed_4byte;
  ...
  assert(IndexReg.getReg() != Mcasm::ESP && IndexReg.getReg() != Mcasm::RSP &&
  ...
  unsigned IndexRegNo = IndexReg.getReg() ? getMcasmRegNum(IndexReg) : 4;
  ...
  if (ForceDisp8)
    ...
    emitImmediate(Disp, MI.getLoc(), Mcasm::reloc_signed_4byte, false, StartByte,
                  CB, Fixups);
}
...
PrefixKind McasmMCCodeEmitter::emitPrefixImpl(unsigned &CurOp, const MCInst &MI,
                                            const MCSubtargetInfo &STI,
                                            SmallVectorImpl<char> &CB) const {
  ...
  if (MemoryOperand != -1) {
    MemoryOperand += CurOp;
    emitSegmentOverridePrefix(MemoryOperand + Mcasm::AddrSegmentReg, MI, CB);
  }

  // Emit the repeat opcode prefix as needed.
  unsigned Flags = MI.getFlags();
  if (TSFlags & McasmII::REP || Flags & Mcasm::IP_HAS_REPEAT)
    emitByte(0xF3, CB);
  if (Flags & Mcasm::IP_HAS_REPEAT_NE)
    emitByte(0xF2, CB);

  // Emit the address size opcode prefix as needed.
  if (Mcasm_MC::needsAddressSizeOverride(MI, STI, MemoryOperand, TSFlags) ||
      Flags & Mcasm::IP_HAS_AD_SIZE)
    emitByte(0x67, CB);
  ...
  case McasmII::RawFrmDstSrc: {
    ...
    if (MI.getOperand(2).getReg() != Mcasm::DS)
  ...
  case McasmII::RawFrmSrc: {
    // Emit segment override opcode prefix as needed (not for %ds).
    if (MI.getOperand(1).getReg() != Mcasm::DS)
  ...
  return (TSFlags & McasmII::EncodingMask)
...
PrefixKind
McasmMCCodeEmitter::emitVEXOpcodePrefix(int MemOperand, const MCInst &MI,
                                      const MCSubtargetInfo &STI,
                                      SmallVectorImpl<char> &CB) const {
  const MCInstrDesc &Desc = MCII.get(MI.getOpcode());
  uint64_t TSFlags = Desc.TSFlags;

  assert(!(TSFlags & McasmII::LOCK) && "Can't have LOCK VEX.");

#ifndef NDEBUG
  unsigned NumOps = MI.getNumOperands();
  for (unsigned I = NumOps ? McasmII::getOperandBias(Desc) : 0; I != NumOps;
       ++I) {
    const MCOperand &MO = MI.getOperand(I);
    if (!MO.isReg())
      continue;
    MCRegister Reg = MO.getReg();
    if (Reg == Mcasm::AH || Reg == Mcasm::BH || Reg == Mcasm::CH || Reg == Mcasm::DH)
      report_fatal_error(
          "Cannot encode high byte register in VEX/EVEX-prefixed instruction");
  }
#endif

  McasmOpcodePrefixHelper Prefix(*Ctx.getRegisterInfo());
  switch (TSFlags & McasmII::EncodingMask) {
  default:
    break;
  case McasmII::XOP:
  ...
  case McasmII::VEX:
    ...
    Prefix.setLowerBound((MI.getFlags() & Mcasm::IP_USE_VEX3) ? VEX3 : VEX2);
    break;
  case McasmII::EVEX:
  ...
  Prefix.setW(TSFlags & McasmII::REX_W);
  Prefix.setNF(TSFlags & McasmII::EVEX_NF);

  bool HasEVEX_K = TSFlags & McasmII::EVEX_K;
  bool HasVEX_4V = TSFlags & McasmII::VEX_4V;
  bool IsND = McasmII::hasNewDataDest(TSFlags); // IsND implies HasVEX_4V
  bool HasEVEX_RC = TSFlags & McasmII::EVEX_RC;

  switch (TSFlags & McasmII::OpMapMask) {
  default:
  ...
  Prefix.setL(TSFlags & McasmII::VEX_L);
  Prefix.setL2(TSFlags & McasmII::EVEX_L2);
  switch (TSFlags & McasmII::OpPrefixMask) {
  case McasmII::PD:
    Prefix.setPP(0x1); // 66
    break;
  case McasmII::XS:
  ...
  Prefix.setZ(HasEVEX_K && (TSFlags & McasmII::EVEX_Z));
  Prefix.setEVEX_b(TSFlags & McasmII::EVEX_B);
  Prefix.setEVEX_U(TSFlags & McasmII::EVEX_U);

  bool EncodeRC = false;
  uint8_t EVEX_rc = 0;

  unsigned CurOp = McasmII::getOperandBias(Desc);
  bool HasTwoConditionalOps = TSFlags & McasmII::TwoConditionalOps;

  switch (TSFlags & McasmII::FormMask) {
  default:
    llvm_unreachable("Unexpected form in emitVEXOpcodePrefix!");
  case McasmII::MRMDestMem4VOp3CC: {
    //  src1(ModR/M), MemAddr, src2(VEX_4V)
    Prefix.setRR2(MI, CurOp++);
    Prefix.setBB2(MI, MemOperand + Mcasm::AddrBaseReg);
    Prefix.setXX2(MI, MemOperand + Mcasm::AddrIndexReg);
    CurOp += Mcasm::AddrNumOperands;
  ...
  case McasmII::RawFrm:
    break;
  case McasmII::MRMDestMemCC:
  case McasmII::MRMDestMemFSIB:
  case McasmII::MRMDestMem: {
    ...
    Prefix.setBB2(MI, MemOperand + Mcasm::AddrBaseReg);
    Prefix.setXX2(MI, MemOperand + Mcasm::AddrIndexReg);
    Prefix.setV2(MI, MemOperand + Mcasm::AddrIndexReg, HasVEX_4V);
  ...
  case McasmII::MRMSrcMemCC:
  case McasmII::MRMSrcMemFSIB:
  case McasmII::MRMSrcMem: {
  ...
  case McasmII::MRMSrcMem4VOp3: {
    // Instruction format for 4VOp3:
    //   src1(ModR/M), MemAddr, src3(VEX_4V)
    Prefix.setRR2(MI, CurOp++);
    Prefix.setBB2(MI, MemOperand + Mcasm::AddrBaseReg);
    Prefix.setXX2(MI, MemOperand + Mcasm::AddrIndexReg);
    Prefix.set4VV2(MI, CurOp + Mcasm::AddrNumOperands);
    break;
  }
  case McasmII::MRMSrcMemOp4: {
  ...
  case McasmII::MRM7m: {
    ...
    Prefix.setXX2(MI, MemOperand + Mcasm::AddrIndexReg);
    Prefix.setV2(MI, MemOperand + Mcasm::AddrIndexReg, HasVEX_4V);
    CurOp += Mcasm::AddrNumOperands + 1; // Skip first imm.
    if (HasTwoConditionalOps) {
      Prefix.set4V(MI, CurOp++, /*IsImm=*/true);
      Prefix.setSC(MI, CurOp++);
    }
  ...
  case McasmII::MRMSrcRegCC:
  case McasmII::MRMSrcReg: {
  ...
  case McasmII::MRMDestRegCC:
  case McasmII::MRMDestReg: {
...
///   3) use of Mcasm-64 extended registers.
///
/// \returns the used prefix (REX or None).
PrefixKind McasmMCCodeEmitter::emitREXPrefix(int MemOperand, const MCInst &MI,
                                           const MCSubtargetInfo &STI,
                                           SmallVectorImpl<char> &CB) const {
  ...
  bool HasRegOp = false;
#endif
  unsigned CurOp = NumOps ? McasmII::getOperandBias(Desc) : 0;
  for (unsigned i = CurOp; i != NumOps; ++i) {
    ...
    if (MO.isReg()) {
      ...
      if (McasmII::isMcasm_64NonExtLowByteReg(Reg))
      ...
      if (Ref && (Ref->getSpecifier() == Mcasm::S_GOTTPOFF ||
                  Ref->getSpecifier() == Mcasm::S_TLSDESC)) {
        Prefix.setLowerBound(REX);
      }
    }
  }
  if (MI.getFlags() & Mcasm::IP_USE_REX)
    Prefix.setLowerBound(REX);
  if ((TSFlags & McasmII::ExplicitOpPrefixMask) == McasmII::ExplicitREX2Prefix ||
      MI.getFlags() & Mcasm::IP_USE_REX2)
  ...
  Prefix.setM((TSFlags & McasmII::OpMapMask) == McasmII::TB);
  PrefixKind Kind = Prefix.determineOptimalKind();
...
/// Emit segment override opcode prefix as needed.
void McasmMCCodeEmitter::emitSegmentOverridePrefix(
  ...
  if (MCRegister Reg = MI.getOperand(SegOperand).getReg())
    emitByte(Mcasm::getSegmentOverridePrefixForReg(Reg), CB);
}
...
///
/// \returns the used prefix (REX or None).
PrefixKind McasmMCCodeEmitter::emitOpcodePrefix(int MemOperand, const MCInst &MI,
                                              const MCSubtargetInfo &STI,
                                              SmallVectorImpl<char> &CB) const {
  const MCInstrDesc &Desc = MCII.get(MI.getOpcode());
  uint64_t TSFlags = Desc.TSFlags;
  ...
  if ((TSFlags & McasmII::OpSizeMask) ==
      (STI.hasFeature(Mcasm::Is16Bit) ? McasmII::OpSize32 : McasmII::OpSize16))
    emitByte(0x66, CB);

  // Emit the LOCK opcode prefix.
  if (TSFlags & McasmII::LOCK || MI.getFlags() & Mcasm::IP_HAS_LOCK)
    emitByte(0xF0, CB);

  // Emit the NOTRACK opcode prefix.
  if (TSFlags & McasmII::NOTRACK || MI.getFlags() & Mcasm::IP_HAS_NOTRACK)
    emitByte(0x3E, CB);
  ...
  // Handle REX prefix.
  assert((STI.hasFeature(Mcasm::Is64Bit) || !(TSFlags & McasmII::REX_W)) &&
         "REX.W requires 64bit mode.");
  PrefixKind Kind = emitREXPrefix(MemOperand, MI, STI, CB);

  // 0x0F escape code must be emitted just before the opcode.
  switch (TSFlags & McasmII::OpMapMask) {
  case McasmII::TB:        // Two-byte opcode map
    // Encoded by M bit in REX2
    if (Kind == REX2)
      break;
    [[fallthrough]];
  case McasmII::T8:        // 0F 38
  case McasmII::TA:        // 0F 3A
  case McasmII::ThreeDNow: // 0F 0F, second 0F emitted by caller.
    emitByte(0x0F, CB);
    break;
  }
...
void McasmMCCodeEmitter::emitPrefix(const MCInst &MI, SmallVectorImpl<char> &CB,
                                  const MCSubtargetInfo &STI) const {
  unsigned Opcode = MI.getOpcode();
  const MCInstrDesc &Desc = MCII.get(Opcode);
  uint64_t TSFlags = Desc.TSFlags;
...
void Mcasm_MC::emitPrefix(MCCodeEmitter &MCE, const MCInst &MI,
                        SmallVectorImpl<char> &CB, const MCSubtargetInfo &STI) {
  static_cast<McasmMCCodeEmitter &>(MCE).emitPrefix(MI, CB, STI);
}

void McasmMCCodeEmitter::encodeInstruction(const MCInst &MI,
                                         SmallVectorImpl<char> &CB,
                                         SmallVectorImpl<MCFixup> &Fixups,
                                         const MCSubtargetInfo &STI) const {
  unsigned Opcode = MI.getOpcode();
  ...
  bool HasVEX_4V = TSFlags & McasmII::VEX_4V;
  bool HasVEX_I8Reg = (TSFlags & McasmII::ImmMask) == McasmII::Imm8Reg;
  ...
  uint8_t BaseOpcode = McasmII::getBaseOpcodeFor(TSFlags);

  if ((TSFlags & McasmII::OpMapMask) == McasmII::ThreeDNow)
    BaseOpcode = 0x0F; // Weird 3DNow! encoding.

  unsigned OpcodeOffset = 0;

  bool IsND = McasmII::hasNewDataDest(TSFlags);
  bool HasTwoConditionalOps = TSFlags & McasmII::TwoConditionalOps;
  ...
  default:
    errs() << "FORM: " << Form << "\n";
    llvm_unreachable("Unknown FormMask value in McasmMCCodeEmitter!");
  case McasmII::Pseudo:
    llvm_unreachable("Pseudo instruction shouldn't be emitted");
  case McasmII::RawFrmDstSrc:
  case McasmII::RawFrmSrc:
  case McasmII::RawFrmDst:
  case McasmII::PrefixByte:
    emitByte(BaseOpcode, CB);
    break;
  case McasmII::AddCCFrm: {
  ...
  case McasmII::RawFrm:
    ...
    if (!STI.hasFeature(Mcasm::Is64Bit) || !isPCRel32Branch(MI, MCII))
      break;

    const MCOperand &Op = MI.getOperand(CurOp++);
    emitImmediate(Op, MI.getLoc(), Mcasm::reloc_branch_4byte_pcrel, true,
                  StartByte, CB, Fixups);
    break;
  }
  case McasmII::RawFrmMemOffs:
    ...
    emitImmediate(MI.getOperand(CurOp++), MI.getLoc(), getImmFixupKind(TSFlags),
                  McasmII::isImmPCRel(TSFlags), StartByte, CB, Fixups);
    ++CurOp; // skip segment operand
    break;
  case McasmII::RawFrmImm8:
    emitByte(BaseOpcode, CB);
    emitImmediate(MI.getOperand(CurOp++), MI.getLoc(), getImmFixupKind(TSFlags),
                  McasmII::isImmPCRel(TSFlags), StartByte, CB, Fixups);
  ...
  case McasmII::AddRegFrm:
    emitByte(BaseOpcode + getMcasmRegNum(MI.getOperand(CurOp++)), CB);
    break;
  ...
  case McasmII::MRMDestReg: {
    ...
    emitRegModRMByte(MI.getOperand(CurOp),
                     getMcasmRegNum(MI.getOperand(SrcRegNum)), CB);
    CurOp = SrcRegNum + 1;
    break;
  }
  case McasmII::MRMDestRegCC: {
    ...
    emitRegModRMByte(MI.getOperand(FirstOp),
                     getMcasmRegNum(MI.getOperand(SecondOp)), CB);
    break;
  }
  case McasmII::MRMDestMem4VOp3CC: {
    unsigned CC = MI.getOperand(8).getImm();
    emitByte(BaseOpcode + CC, CB);
    unsigned SrcRegNum = CurOp + Mcasm::AddrNumOperands;
    emitMemModRMByte(MI, CurOp + 1, getMcasmRegNum(MI.getOperand(0)), TSFlags,
  ...
  case McasmII::MRMDestMem: {
    ...
    bool ForceSIB = (Form == McasmII::MRMDestMemFSIB);
    emitMemModRMByte(MI, CurOp, getMcasmRegNum(MI.getOperand(SrcRegNum)), TSFlags,
  ...
  case McasmII::MRMDestMemCC: {
    ...
    CurOp = MemOp + Mcasm::AddrNumOperands;
    unsigned RegOp = CurOp++;
    unsigned CC = MI.getOperand(CurOp++).getImm();
    emitByte(BaseOpcode + CC, CB);
    emitMemModRMByte(MI, MemOp, getMcasmRegNum(MI.getOperand(RegOp)), TSFlags,
  ...
  case McasmII::MRMSrcReg: {
    ...
    emitRegModRMByte(MI.getOperand(SrcRegNum),
                     getMcasmRegNum(MI.getOperand(CurOp)), CB);
    CurOp = SrcRegNum + 1;
    if (HasVEX_I8Reg)
      I8RegNum = getMcasmRegEncoding(MI, CurOp++);
  ...
  case McasmII::MRMSrcRegOp4: {
    ...
    I8RegNum = getMcasmRegEncoding(MI, SrcRegNum++);
  ...
  case McasmII::MRMSrcRegCC: {
    ...
    emitRegModRMByte(MI.getOperand(SecondOp),
                     getMcasmRegNum(MI.getOperand(FirstOp)), CB);
    break;
  }
  case McasmII::MRMSrcMemFSIB:
  case McasmII::MRMSrcMem: {
    ...
    bool ForceSIB = (Form == McasmII::MRMSrcMemFSIB);
    emitMemModRMByte(MI, FirstMemOp, getMcasmRegNum(MI.getOperand(CurOp)),
                     TSFlags, Kind, StartByte, CB, Fixups, STI, ForceSIB);
    CurOp = FirstMemOp + Mcasm::AddrNumOperands;
    if (HasVEX_I8Reg)
      I8RegNum = getMcasmRegEncoding(MI, CurOp++);
    break;
  }
  ...
  case McasmII::MRMSrcMemOp4: {
    ...
    assert(HasVEX_I8Reg && "MRMSrcRegOp4 should imply VEX_I8Reg");
    I8RegNum = getMcasmRegEncoding(MI, FirstMemOp++);
  ...
  case McasmII::MRMSrcMemCC: {
    ...
    emitMemModRMByte(MI, FirstMemOp, getMcasmRegNum(MI.getOperand(RegOp)),
                     TSFlags, Kind, StartByte, CB, Fixups, STI);
    break;
  }
  ...
  case McasmII::MRM7r:
    ...
    emitByte(BaseOpcode, CB);
    emitRegModRMByte(MI.getOperand(CurOp++),
                     (Form == McasmII::MRMXr) ? 0 : Form - McasmII::MRM0r, CB);
    break;
  case McasmII::MRMr0:
    emitByte(BaseOpcode, CB);
    emitByte(modRMByte(3, getMcasmRegNum(MI.getOperand(CurOp++)), 0), CB);
    break;
  ...
  case McasmII::MRM7m:
    ...
    emitMemModRMByte(MI, CurOp,
                     (Form == McasmII::MRMXm) ? 0 : Form - McasmII::MRM0m, TSFlags,
                     Kind, StartByte, CB, Fixups, STI);
    CurOp += Mcasm::AddrNumOperands;
    break;
  ...
  case McasmII::MRM7X:
    emitByte(BaseOpcode, CB);
    emitByte(0xC0 + ((Form - McasmII::MRM0X) << 3), CB);
    break;
  ...
  case McasmII::MRM_FF:
    emitByte(BaseOpcode, CB);
    emitByte(0xC0 + Form - McasmII::MRM_C0, CB);
    break;
  }
  ...
  if (HasVEX_I8Reg) {
    ...
    while (RemainingOps) {
      emitImmediate(MI.getOperand(CurOp++), MI.getLoc(),
                    ...
                    McasmII::isImmPCRel(Desc.TSFlags), StartByte, CB, Fixups);
      --RemainingOps;
    }
  ...
  if ((TSFlags & McasmII::OpMapMask) == McasmII::ThreeDNow)
    emitByte(McasmII::getBaseOpcodeFor(TSFlags), CB);
