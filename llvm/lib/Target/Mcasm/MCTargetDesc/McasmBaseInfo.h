//===-- McasmBaseInfo.h - Top level definitions for Mcasm -------- --*- C++ -*-===//
//
...
// the Mcasm target useful for the compiler back-end and the MC libraries.
// As such, it deliberately does not include references to LLVM core
// code gen types, passes, etc..
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_Mcasm_MCTARGETDESC_McasmBASEINFO_H
#define LLVM_LIB_TARGET_Mcasm_MCTARGETDESC_McasmBASEINFO_H
...
// Mcasm specific condition code. These correspond to Mcasm_*_COND in
// McasmInstrInfo.td. They must be kept in synch.
enum CondCode {
  COND_O = 0,
  COND_NO = 1,
  COND_B = 2,
...
classifySecondCondCodeInMacroFusion(Mcasm::CondCode CC) {
  if (CC == Mcasm::COND_INVALID)
    return SecondMacroFusionInstKind::Invalid;
  switch (CC) {
  default:
    return SecondMacroFusionInstKind::Invalid;
  case Mcasm::COND_E:  // JE,JZ
  case Mcasm::COND_NE: // JNE,JNZ
  case Mcasm::COND_L:  // JL,JNGE
  case Mcasm::COND_LE: // JLE,JNG
  case Mcasm::COND_G:  // JG,JNLE
  case Mcasm::COND_GE: // JGE,JNL
    return SecondMacroFusionInstKind::ELG;
  case Mcasm::COND_B:  // JB,JC
  case Mcasm::COND_BE: // JNA,JBE
  case Mcasm::COND_A:  // JA,JNBE
  case Mcasm::COND_AE: // JAE,JNC,JNB
    return SecondMacroFusionInstKind::AB;
  case Mcasm::COND_S:  // JS
  case Mcasm::COND_NS: // JNS
  case Mcasm::COND_P:  // JP,JPE
  case Mcasm::COND_NP: // JNP,JPO
  case Mcasm::COND_O:  // JO
  case Mcasm::COND_NO: // JNO
...
inline bool isMacroFused(FirstMacroFusionInstKind FirstKind,
  ...
  switch (FirstKind) {
  case Mcasm::FirstMacroFusionInstKind::Test:
  case Mcasm::FirstMacroFusionInstKind::And:
    return true;
  case Mcasm::FirstMacroFusionInstKind::Cmp:
  case Mcasm::FirstMacroFusionInstKind::AddSub:
    return SecondKind == Mcasm::SecondMacroFusionInstKind::AB ||
           SecondKind == Mcasm::SecondMacroFusionInstKind::ELG;
  case Mcasm::FirstMacroFusionInstKind::IncDec:
    return SecondKind == Mcasm::SecondMacroFusionInstKind::ELG;
  case Mcasm::FirstMacroFusionInstKind::Invalid:
    return false;
  }
...
/// McasmII - This namespace holds all of the target specific flags that
...
enum TOF {
  //===------------------------------------------------------------------===//
  // Mcasm Specific MachineOperand flags.
  //
  ...
  /// See the Mcasm-64 ELF ABI supplement for more details.
  ///    SYMBOL_LABEL @GOT
  MO_GOT,
  ...
  MO_GOTPCREL,
  /// MO_GOTPCREL_NORELAX - Same as MO_GOTPCREL except that R_Mcasm_64_GOTPCREL
  /// relocations are guaranteed to be emitted by the integrated assembler
  /// instead of the relaxable R_Mcasm_64[_REX]_GOTPCRELX relocations.
  MO_GOTPCREL_NORELAX,
...
enum : uint64_t {
  //===------------------------------------------------------------------===//
  // Instruction encodings.  These are the standard/most common forms for Mcasm
  // instructions.
  //
...
inline bool isPrefix(uint64_t TSFlags) {
  return (TSFlags & McasmII::FormMask) == PrefixByte;
}

/// \returns true if the instruction with given opcode is a pseudo.
inline bool isPseudo(uint64_t TSFlags) {
  return (TSFlags & McasmII::FormMask) == Pseudo;
}

/// \returns the "base" Mcasm opcode for the specified machine
/// instruction.
inline uint8_t getBaseOpcodeFor(uint64_t TSFlags) {
  return TSFlags >> McasmII::OpcodeShift;
}

inline bool hasImm(uint64_t TSFlags) { return (TSFlags & McasmII::ImmMask) != 0; }

/// Decode the "size of immediate" field from the TSFlags field of the
/// specified instruction.
inline unsigned getSizeOfImm(uint64_t TSFlags) {
  switch (TSFlags & McasmII::ImmMask) {
  default:
    llvm_unreachable("Unknown immediate size");
  case McasmII::Imm8:
  case McasmII::Imm8PCRel:
  case McasmII::Imm8Reg:
    return 1;
  case McasmII::Imm16:
  case McasmII::Imm16PCRel:
    return 2;
  case McasmII::Imm32:
  case McasmII::Imm32S:
  case McasmII::Imm32PCRel:
    return 4;
  case McasmII::Imm64:
    return 8;
  }
...
inline bool hasNewDataDest(uint64_t TSFlags) {
  return (TSFlags & McasmII::OpMapMask) == McasmII::T_MAP4 &&
         (TSFlags & McasmII::EVEX_B) && (TSFlags & McasmII::VEX_4V);
}
