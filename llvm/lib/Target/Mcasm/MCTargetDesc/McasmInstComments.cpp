//===-- McasmInstComments.cpp - Generate verbose-asm comments for instrs ----===//
//
...
#define CASE_AVX_INS_COMMON(Inst, Suffix, src)    \
  case Mcasm::V##Inst##Suffix##src:

#define CASE_MASK_INS_COMMON(Inst, Suffix, src)   \
  case Mcasm::V##Inst##Suffix##src##k:

#define CASE_MASKZ_INS_COMMON(Inst, Suffix, src)  \
  case Mcasm::V##Inst##Suffix##src##kz:
...
#define CASE_MASK_INS_COMMON_INT(Inst, Suffix, src) \
  case Mcasm::V##Inst##Suffix##src##k_Int:

#define CASE_MASKZ_INS_COMMON_INT(Inst, Suffix, src) \
  case Mcasm::V##Inst##Suffix##src##kz_Int:
...
static unsigned getVectorRegSize(MCRegister Reg) {
  if (McasmII::isZMMReg(Reg))
    return 512;
  if (McasmII::isYMMReg(Reg))
    return 256;
  if (McasmII::isXMMReg(Reg))
    return 128;
  if (Reg >= Mcasm::MM0 && Reg <= Mcasm::MM7)
    return 64;
...
static const char *getRegName(MCRegister Reg) {
  return McasmATTInstPrinter::getRegisterName(Reg);
}
...
static void printMasking(raw_ostream &OS, const MCInst *MI,
  ...
  if (!(TSFlags & McasmII::EVEX_K))
    return;

  bool MaskWithZero = (TSFlags & McasmII::EVEX_Z);
...
/// EmitAnyMcasmInstComments - This function decodes x86 instructions and prints
/// newline terminated strings to the specified string if desired.  This
/// information is shown in disassembly dumps when verbose assembly is enabled.
bool llvm::EmitAnyMcasmInstComments(const MCInst *MI, raw_ostream &OS,
                                  const MCInstrInfo &MCII) {
  ...
  case Mcasm::INSERTPSrmi:
  case Mcasm::VINSERTPSrmi:
  case Mcasm::VINSERTPSZrmi:
    DestName = getRegName(MI->getOperand(0).getReg());
    Src1Name = getRegName(MI->getOperand(1).getReg());
    if (MI->getOperand(NumOperands - 1).isImm())
      DecodeINSERTPSMask(MI->getOperand(NumOperands - 1).getImm(), ShuffleMask,
  ...
  case Mcasm::MOVHPDrm:
  case Mcasm::VMOVHPDrm:
  case Mcasm::VMOVHPDZ128rm:
    Src1Name = getRegName(MI->getOperand(1).getReg());
    DestName = getRegName(MI->getOperand(0).getReg());
  ...
  case Mcasm::MOVHPSrm:
  case Mcasm::VMOVHPSrm:
  case Mcasm::VMOVHPSZ128rm:
    Src1Name = getRegName(MI->getOperand(1).getReg());
    DestName = getRegName(MI->getOperand(0).getReg());
  ...
  case Mcasm::MOVLPDrm:
  case Mcasm::VMOVLPDrm:
  case Mcasm::VMOVLPDZ128rm:
    Src1Name = getRegName(MI->getOperand(1).getReg());
    DestName = getRegName(MI->getOperand(0).getReg());
  ...
  case Mcasm::MOVLPSrm:
  case Mcasm::VMOVLPSrm:
  case Mcasm::VMOVLPSZ128rm:
    Src1Name = getRegName(MI->getOperand(1).getReg());
    DestName = getRegName(MI->getOperand(0).getReg());
  ...
  case Mcasm::PSLLDQri:
  case Mcasm::VPSLLDQri:
  case Mcasm::VPSLLDQYri:
  case Mcasm::VPSLLDQZ128ri:
  case Mcasm::VPSLLDQZ256ri:
  case Mcasm::VPSLLDQZri:
    Src1Name = getRegName(MI->getOperand(1).getReg());
    [[fallthrough]];
  case Mcasm::VPSLLDQZ128mi:
  case Mcasm::VPSLLDQZ256mi:
  case Mcasm::VPSLLDQZmi:
    DestName = getRegName(MI->getOperand(0).getReg());
    if (MI->getOperand(NumOperands - 1).isImm())
  ...
  case Mcasm::PSRLDQri:
  case Mcasm::VPSRLDQri:
  case Mcasm::VPSRLDQYri:
  case Mcasm::VPSRLDQZ128ri:
  case Mcasm::VPSRLDQZ256ri:
  case Mcasm::VPSRLDQZri:
    Src1Name = getRegName(MI->getOperand(1).getReg());
    [[fallthrough]];
  case Mcasm::VPSRLDQZ128mi:
  case Mcasm::VPSRLDQZ256mi:
  case Mcasm::VPSRLDQZmi:
    DestName = getRegName(MI->getOperand(0).getReg());
    if (MI->getOperand(NumOperands - 1).isImm())
  ...
  case Mcasm::MMX_PSHUFWri:
    Src1Name = getRegName(MI->getOperand(1).getReg());
    [[fallthrough]];

  case Mcasm::MMX_PSHUFWmi:
    DestName = getRegName(MI->getOperand(0).getReg());
    if (MI->getOperand(NumOperands - 1).isImm())
  ...
  case Mcasm::VPERM2F128rmi:
  case Mcasm::VPERM2I128rmi:
  ...
  case Mcasm::MOVPQI2QIrr:
  case Mcasm::MOVZPQILo2PQIrr:
  case Mcasm::VMOVPQI2QIrr:
  case Mcasm::VMOVPQI2QIZrr:
  case Mcasm::VMOVZPQILo2PQIrr:
  case Mcasm::VMOVZPQILo2PQIZrr:
    Src1Name = getRegName(MI->getOperand(1).getReg());
