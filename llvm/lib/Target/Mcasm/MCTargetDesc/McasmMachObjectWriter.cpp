//===-- McasmMachObjectWriter.cpp - Mcasm Mach-O Writer -----------------------===//
//
...
namespace {
class McasmMachObjectWriter : public MCMachObjectTargetWriter {
  ...
  void RecordMcasmRelocation(MachObjectWriter *Writer,
                              const MCAssembler &Asm,
                              const MCFragment *Fragment,
                              const MCFixup &Fixup,
                              MCValue Target,
                              uint64_t &FixedValue);
  void RecordMcasm_64Relocation(MachObjectWriter *Writer, MCAssembler &Asm,
                              const MCFragment *Fragment, const MCFixup &Fixup,
                              MCValue Target, uint64_t &FixedValue);

public:
  McasmMachObjectWriter(bool Is64Bit, uint32_t CPUType, uint32_t CPUSubtype)
  ...
  void recordRelocation(MachObjectWriter *Writer, MCAssembler &Asm,
    ...
    if (Writer->is64Bit())
      RecordMcasm_64Relocation(Writer, Asm, Fragment, Fixup, Target, FixedValue);
    else
      RecordMcasmRelocation(Writer, Asm, Fragment, Fixup, Target, FixedValue);
  }
};
} // namespace

static bool isFixupKindRIPRel(unsigned Kind) {
  return Kind == Mcasm::reloc_riprel_4byte ||
         Kind == Mcasm::reloc_riprel_4byte_movq_load ||
         Kind == Mcasm::reloc_riprel_4byte_movq_load_rex2 ||
         Kind == Mcasm::reloc_riprel_4byte_relax ||
         Kind == Mcasm::reloc_riprel_4byte_relax_rex ||
         Kind == Mcasm::reloc_riprel_4byte_relax_rex2 ||
         Kind == Mcasm::reloc_riprel_4byte_relax_evex;
}
...
void McasmMachObjectWriter::RecordMcasm_64Relocation(
  ...
  if (Target.isAbsolute()) { // constant
    ...
    Type = MachO::Mcasm_64_RELOC_UNSIGNED;
    ...
    if (IsPCRel) {
      IsExtern = 1;
      Type = MachO::Mcasm_64_RELOC_BRANCH;
    }
    ...
    Type = MachO::Mcasm_64_RELOC_SUBTRACTOR;
  } else {
    ...
    if (IsPCRel) {
      if (IsRIPRel) {
        if (Specifier == Mcasm::S_GOTPCREL) {
          // x86_64 distinguishes movq foo@GOTPCREL so that the linker can
          // rewrite the movq to an leaq at link time if the symbol ends up in
          // the same linkage unit.
          if (Fixup.getKind() == Mcasm::reloc_riprel_4byte_movq_load)
            Type = MachO::Mcasm_64_RELOC_GOT_LOAD;
          else
            Type = MachO::Mcasm_64_RELOC_GOT;
        } else if (Specifier == Mcasm::S_TLVP) {
          Type = MachO::Mcasm_64_RELOC_TLV;
        } else if (Specifier) {
          reportError(Fixup.getLoc(),
                      "unsupported symbol modifier in relocation");
          return;
        } else {
          Type = MachO::Mcasm_64_RELOC_SIGNED;
          ...
          case 1: Type = MachO::Mcasm_64_RELOC_SIGNED_1; break;
          case 2: Type = MachO::Mcasm_64_RELOC_SIGNED_2; break;
          case 4: Type = MachO::Mcasm_64_RELOC_SIGNED_4; break;
          }
        }
      } else {
        if (Specifier) {
      ...
      if (Specifier == Mcasm::S_GOT) {
        Type = MachO::Mcasm_64_RELOC_GOT;
      } else if (Specifier == Mcasm::S_GOTPCREL) {
        ...
        Type = MachO::Mcasm_64_RELOC_UNSIGNED;
        if (Fixup.getKind() == Mcasm::reloc_signed_4byte) {
          reportError(
              Fixup.getLoc(),
...
bool McasmMachObjectWriter::recordScatteredRelocation(MachObjectWriter *Writer,
                                                    const MCAssembler &Asm,
                                                    const MCFragment *Fragment,
                                                    const MCFixup &Fixup,
                                                    MCValue Target,
...
void McasmMachObjectWriter::recordTLVPRelocation(MachObjectWriter *Writer,
                                               const MCAssembler &Asm,
                                               const MCFragment *Fragment,
                                               const MCFixup &Fixup,
                                               MCValue Target,
  ...
  assert(Target.getSpecifier() == Mcasm::S_TLVP && !is64Bit() &&
...
void McasmMachObjectWriter::RecordMcasmRelocation(MachObjectWriter *Writer,
                                              const MCAssembler &Asm,
                                              const MCFragment *Fragment,
                                              const MCFixup &Fixup,
                                              MCValue Target,
  ...
  if (A && Target.getSpecifier() == Mcasm::S_TLVP) {
...
llvm::createMcasmMachObjectWriter(bool Is64Bit, uint32_t CPUType,
                                uint32_t CPUSubtype) {
  return std::make_unique<McasmMachObjectWriter>(Is64Bit, CPUType, CPUSubtype);
}
