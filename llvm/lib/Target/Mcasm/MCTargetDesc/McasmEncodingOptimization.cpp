//===-- McasmEncodingOptimization.cpp - Mcasm Encoding optimization -*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the implementation of the Mcasm encoding optimization
//
//===----------------------------------------------------------------------===//

#include "McasmEncodingOptimization.h"
...
bool Mcasm::optimizeInstFromVEX3ToVEX2(MCInst &MI, const MCInstrDesc &Desc) {
...
#define TO_REV(FROM) FROM_TO(FROM, FROM##_REV, 0, 1)
  ...
  default: {
    ...
    if (!Desc.isCommutable() || (TSFlags & McasmII::EncodingMask) != McasmII::VEX ||
        (TSFlags & McasmII::OpMapMask) != McasmII::TB ||
        (TSFlags & McasmII::FormMask) != McasmII::MRMSrcReg ||
        (TSFlags & McasmII::REX_W) || !(TSFlags & McasmII::VEX_4V) ||
        MI.getNumOperands() != 3)
      return false;
    // These two are not truly commutable.
    if (Opcode == Mcasm::VMOVHLPSrr || Opcode == Mcasm::VUNPCKHPDrr)
      return false;
    OpIdx1 = 1;
    OpIdx2 = 2;
    break;
...
#undef FROM_TO
  ...
  if (McasmII::isMcasm_64ExtendedReg(MI.getOperand(OpIdx1).getReg()) ||
      !McasmII::isMcasm_64ExtendedReg(MI.getOperand(OpIdx2).getReg()))
    return false;
  if (NewOpc)
    MI.setOpcode(NewOpc);
  else
...
bool Mcasm::optimizeShiftRotateWithImmediateOne(MCInst &MI) {
  unsigned NewOpc;
#define TO_IMM1(FROM)                                                          \
  case Mcasm::FROM##i:                                                           \
    NewOpc = Mcasm::FROM##1;                                                     \
    break;                                                                     \
  case Mcasm::FROM##i_EVEX:                                                      \
    NewOpc = Mcasm::FROM##1_EVEX;                                                \
    break;                                                                     \
  case Mcasm::FROM##i_ND:                                                        \
    NewOpc = Mcasm::FROM##1_ND;                                                  \
    break;
  switch (MI.getOpcode()) {
  default:
    return false;
...
#define TO_IMM1(FROM)                                                          \
  ...
  case Mcasm::FROM##i_EVEX:                                                      \
    NewOpc = Mcasm::FROM##1_EVEX;                                                \
    break;                                                                     \
  case Mcasm::FROM##i_NF:                                                        \
    NewOpc = Mcasm::FROM##1_NF;                                                  \
    break;                                                                     \
  case Mcasm::FROM##i_ND:                                                        \
    NewOpc = Mcasm::FROM##1_ND;                                                  \
    break;                                                                     \
  case Mcasm::FROM##i_NF_ND:                                                     \
    NewOpc = Mcasm::FROM##1_NF_ND;                                               \
    break;
    TO_IMM1(ROR8r)
    TO_IMM1(ROR16r)
    TO_IMM1(ROR32r)
...
bool Mcasm::optimizeVPCMPWithImmediateOneOrSix(MCInst &MI) {
  unsigned Opc1;
  unsigned Opc2;
...
bool Mcasm::optimizeMOVSX(MCInst &MI) {
  unsigned NewOpc;
#define FROM_TO(FROM, TO, R0, R1)                                              \
  case Mcasm::FROM:                                                              \
    if (MI.getOperand(0).getReg() != Mcasm::R0 ||                                \
        MI.getOperand(1).getReg() != Mcasm::R1)                                  \
      return false;                                                            \
    NewOpc = Mcasm::TO;                                                          \
    break;
  switch (MI.getOpcode()) {
...
bool Mcasm::optimizeINCDEC(MCInst &MI, bool In64BitMode) {
  if (In64BitMode)
    return false;
  unsigned NewOpc;
...
static bool isARegister(MCRegister Reg) {
  return Reg == Mcasm::AL || Reg == Mcasm::AX || Reg == Mcasm::EAX || Reg == Mcasm::RAX;
}

/// Simplify things like MOV32rm to MOV32o32a.
bool Mcasm::optimizeMOV(MCInst &MI, bool In64BitMode) {
  ...
  if (MI.getOperand(AddrOp).isExpr()) {
    ...
    if (const MCSymbolRefExpr *SRE = dyn_cast<MCSymbolRefExpr>(MCE))
      if (SRE->getSpecifier() == Mcasm::S_TLVP)
        Absolute = false;
  }
  if (Absolute && (MI.getOperand(AddrBase + Mcasm::AddrBaseReg).getReg() ||
                   MI.getOperand(AddrBase + Mcasm::AddrScaleAmt).getImm() != 1 ||
                   MI.getOperand(AddrBase + Mcasm::AddrIndexReg).getReg()))
    return false;
  // If so, rewrite the instruction.
  MCOperand Saved = MI.getOperand(AddrOp);
  MCOperand Seg = MI.getOperand(AddrBase + Mcasm::AddrSegmentReg);
  MI.clear();
  MI.setOpcode(NewOpc);
  MI.addOperand(Saved);
  MI.addOperand(Seg);
...
unsigned Mcasm::getOpcodeForShortImmediateForm(unsigned Opcode) {
#define ENTRY(LONG, SHORT)                                                     \
  ...
  default:
    return Opcode;
#include "McasmEncodingOptimizationForImmediate.def"
  }
}

unsigned Mcasm::getOpcodeForLongImmediateForm(unsigned Opcode) {
#define ENTRY(LONG, SHORT)                                                     \
  case Mcasm::SHORT:                                                             \
    return Mcasm::LONG;
  switch (Opcode) {
...
#include "McasmEncodingOptimizationForImmediate.def"
  ...
  unsigned SkipOperands = Mcasm::isCCMPCC(MI.getOpcode()) ? 2 : 0;
  ...
  if (LastOp.isExpr()) {
    ...
    if (!SRE || SRE->getSpecifier() != Mcasm::S_ABS8)
      return false;
...
bool Mcasm::optimizeToFixedRegisterOrShortImmediateForm(MCInst &MI) {
