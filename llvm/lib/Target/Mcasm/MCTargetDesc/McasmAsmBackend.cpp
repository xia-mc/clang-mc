...
namespace {
/// A wrapper for holding a mask of the values from Mcasm::AlignBranchBoundaryKind
class McasmAlignBranchKind {
private:
...
public:
  void operator=(const std::string &Val) {
    ...
    for (auto BranchType : BranchTypes) {
      if (BranchType == "fused")
        addKind(Mcasm::AlignBranchFused);
      else if (BranchType == "jcc")
        addKind(Mcasm::AlignBranchJcc);
      else if (BranchType == "jmp")
        addKind(Mcasm::AlignBranchJmp);
      else if (BranchType == "call")
        addKind(Mcasm::AlignBranchCall);
      else if (BranchType == "ret")
        addKind(Mcasm::AlignBranchRet);
      else if (BranchType == "indirect")
        addKind(Mcasm::AlignBranchIndirect);
      else {
  ...
  void addKind(Mcasm::AlignBranchBoundaryKind Value) { AlignBranchKind |= Value; }
};

McasmAlignBranchKind McasmAlignBranchKindLoc;

cl::opt<unsigned> McasmAlignBranchBoundary(
...
cl::opt<McasmAlignBranchKind, true, cl::parser<std::string>> McasmAlignBranch(
    "x86-align-branch",
    cl::desc(
    ...
    cl::location(McasmAlignBranchKindLoc));

cl::opt<bool> McasmAlignBranchWithin32BBoundaries(
...
class McasmAsmBackend : public MCAsmBackend {
  ...
  McasmAlignBranchKind AlignBranchType;
  Align AlignBoundary;
...
public:
  McasmAsmBackend(const Target &T, const MCSubtargetInfo &STI)
    ...
    if (McasmAlignBranchWithin32BBoundaries) {
      ...
      AlignBranchType.addKind(Mcasm::AlignBranchFused);
      AlignBranchType.addKind(Mcasm::AlignBranchJcc);
      AlignBranchType.addKind(Mcasm::AlignBranchJmp);
    }
    // Allow overriding defaults set by main flag
    if (McasmAlignBranchBoundary.getNumOccurrences())
      AlignBoundary = assumeAligned(McasmAlignBranchBoundary);
    if (McasmAlignBranch.getNumOccurrences())
      AlignBranchType = McasmAlignBranchKindLoc;
    if (McasmPadMaxPrefixSize.getNumOccurrences())
      TargetPrefixMax = McasmPadMaxPrefixSize;

    AllowAutoPadding =
        AlignBoundary != Align(1) && AlignBranchType != Mcasm::AlignBranchNone;
    AllowEnhancedRelaxation =
        AllowAutoPadding && TargetPrefixMax != 0 && McasmPadForBranchAlign;
  }
...
static bool isRelaxableBranch(unsigned Opcode) {
  return Opcode == Mcasm::JCC_1 || Opcode == Mcasm::JMP_1;
}
...
static unsigned getRelaxedOpcodeBranch(unsigned Opcode,
  ...
  case Mcasm::JCC_1:
    return (Is16BitMode) ? Mcasm::JCC_2 : Mcasm::JCC_4;
  case Mcasm::JMP_1:
    return (Is16BitMode) ? Mcasm::JMP_2 : Mcasm::JMP_4;
  }
}
...
static unsigned getRelaxedOpcode(const MCInst &MI, bool Is16BitMode) {
  ...
  return isRelaxableBranch(Opcode) ? getRelaxedOpcodeBranch(Opcode, Is16BitMode)
                                   : Mcasm::getOpcodeForLongImmediateForm(Opcode);
}

static Mcasm::CondCode getCondFromBranch(const MCInst &MI,
  ...
  case Mcasm::JCC_1: {
    ...
    return static_cast<Mcasm::CondCode>(
        MI.getOperand(Desc.getNumOperands() - 1).getImm());
  }
  }
}

static Mcasm::SecondMacroFusionInstKind
classifySecondInstInMacroFusion(const MCInst &MI, const MCInstrInfo &MCII) {
  Mcasm::CondCode CC = getCondFromBranch(MI, MCII);
...
static bool isRIPRelative(const MCInst &MI, const MCInstrInfo &MCII) {
  ...
  if (MemoryOperand < 0)
    return false;
  unsigned BaseRegNum = MemoryOperand + CurOp + Mcasm::AddrBaseReg;
  MCRegister BaseReg = MI.getOperand(BaseRegNum).getReg();
  return (BaseReg == Mcasm::RIP);
}

/// Check if the instruction is a prefix.
static bool isPrefix(unsigned Opcode, const MCInstrInfo &MCII) {
  return McasmII::isPrefix(MCII.get(Opcode).TSFlags);
}
...
static bool isFirstMacroFusibleInst(const MCInst &Inst,
  ...
  if (isRIPRelative(Inst, MCII))
    return false;
  Mcasm::FirstMacroFusionInstKind FIK =
      Mcasm::classifyFirstOpcodeInMacroFusion(Inst.getOpcode());
  return FIK != Mcasm::FirstMacroFusionInstKind::Invalid;
}

/// Mcasm can reduce the bytes of NOP by padding instructions with prefixes to
...
///   - Otherwise use DS.
uint8_t McasmAsmBackend::determinePaddingPrefix(const MCInst &Inst) const {
  assert((STI.hasFeature(Mcasm::Is32Bit) || STI.hasFeature(Mcasm::Is64Bit)) &&
  ...
  if (MemoryOperand >= 0) {
    ...
    SegmentReg = Inst.getOperand(MemoryOperand + Mcasm::AddrSegmentReg).getReg();
  }
  ...
  case McasmII::RawFrmDstSrc: {
    ...
    if (Inst.getOperand(2).getReg() != Mcasm::DS)
      SegmentReg = Inst.getOperand(2).getReg();
    break;
  }
  case McasmII::RawFrmSrc: {
    // Check segment override opcode prefix as needed (not for %ds).
    if (Inst.getOperand(1).getReg() != Mcasm::DS)
  ...
  if (SegmentReg)
    return Mcasm::getSegmentOverridePrefixForReg(SegmentReg);

  if (STI.hasFeature(Mcasm::Is64Bit))
    return Mcasm::CS_Encoding;

  if (MemoryOperand >= 0) {
    unsigned BaseRegNum = MemoryOperand + Mcasm::AddrBaseReg;
    MCRegister BaseReg = Inst.getOperand(BaseRegNum).getReg();
    if (BaseReg == Mcasm::ESP || BaseReg == Mcasm::EBP)
      return Mcasm::SS_Encoding;
  }
  return Mcasm::DS_Encoding;
}

/// Check if the two instructions will be macro-fused on the target cpu.
bool McasmAsmBackend::isMacroFused(const MCInst &Cmp, const MCInst &Jcc) const {
  const MCInstrDesc &InstDesc = MCII->get(Jcc.getOpcode());
  if (!InstDesc.isConditionalBranch())
    return false;
  if (!isFirstMacroFusibleInst(Cmp, *MCII))
    return false;
  const Mcasm::FirstMacroFusionInstKind CmpKind =
      Mcasm::classifyFirstOpcodeInMacroFusion(Cmp.getOpcode());
  const Mcasm::SecondMacroFusionInstKind BranchKind =
      classifySecondInstInMacroFusion(Jcc, *MCII);
  return Mcasm::isMacroFused(CmpKind, BranchKind);
}
...
/// Mcasm has certain instructions which enable interrupts exactly one
...
bool McasmAsmBackend::canPadInst(const MCInst &Inst, MCObjectStreamer &OS) const {
...
bool McasmAsmBackend::canPadBranches(MCObjectStreamer &OS) const {
  ...
  if (!(STI.hasFeature(Mcasm::Is64Bit) || STI.hasFeature(Mcasm::Is32Bit)))
    return false;
...
bool McasmAsmBackend::needAlign(const MCInst &Inst) const {
  const MCInstrDesc &Desc = MCII->get(Inst.getOpcode());
  return (Desc.isConditionalBranch() &&
          (AlignBranchType & Mcasm::AlignBranchJcc)) ||
         (Desc.isUnconditionalBranch() &&
          (AlignBranchType & Mcasm::AlignBranchJmp)) ||
         (Desc.isCall() && (AlignBranchType & Mcasm::AlignBranchCall)) ||
         (Desc.isReturn() && (AlignBranchType & Mcasm::AlignBranchRet)) ||
         (Desc.isIndirectBranch() &&
          (AlignBranchType & Mcasm::AlignBranchIndirect));
}

void Mcasm_MC::emitInstruction(MCObjectStreamer &S, const MCInst &Inst,
                             const MCSubtargetInfo &STI) {
  bool AutoPadding = S.getAllowAutoPadding();
  if (LLVM_LIKELY(!AutoPadding && !McasmPadForAlign)) {
    S.MCObjectStreamer::emitInstruction(Inst, STI);
    return;
  }

  auto &Backend = static_cast<McasmAsmBackend &>(S.getAssembler().getBackend());
...
void McasmAsmBackend::emitInstructionBegin(MCObjectStreamer &OS,
  ...
  if (needAlign(Inst) || ((AlignBranchType & Mcasm::AlignBranchFused) &&
...
void McasmAsmBackend::emitInstructionEnd(MCObjectStreamer &OS,
                                       const MCInst &Inst) {
...
std::optional<MCFixupKind> McasmAsmBackend::getFixupKind(StringRef Name) const {
  if (STI.getTargetTriple().isOSBinFormatELF()) {
    unsigned Type;
    if (STI.getTargetTriple().isMcasm_64()) {
      Type = llvm::StringSwitch<unsigned>(Name)
#define ELF_RELOC(X, Y) .Case(#X, Y)
#include "llvm/BinaryFormat/ELFRelocs/x86_64.def"
#undef ELF_RELOC
                 .Case("BFD_RELOC_NONE", ELF::R_Mcasm_64_NONE)
                 .Case("BFD_RELOC_8", ELF::R_Mcasm_64_8)
                 .Case("BFD_RELOC_16", ELF::R_Mcasm_64_16)
                 .Case("BFD_RELOC_32", ELF::R_Mcasm_64_32)
                 .Case("BFD_RELOC_64", ELF::R_Mcasm_64_64)
                 .Default(-1u);
    } else {
      Type = llvm::StringSwitch<unsigned>(Name)
#define ELF_RELOC(X, Y) .Case(#X, Y)
...
MCFixupKindInfo McasmAsmBackend::getFixupKindInfo(MCFixupKind Kind) const {
  const static MCFixupKindInfo Infos[Mcasm::NumTargetFixupKinds] = {
      // clang-format off
  ...
  // Fixup kinds from .reloc directive are like R_386_NONE/R_Mcasm_64_NONE. They
  ...
  assert(unsigned(Kind - FirstTargetFixupKind) < Mcasm::NumTargetFixupKinds &&
         "Invalid kind!");
...
// next instruction.
std::optional<bool> McasmAsmBackend::evaluateFixup(const MCFragment &,
                                                 MCFixup &Fixup,
    ...
    default: {
      ...
      if (Add && Add->getName() == GotSymName)
        Fixup = MCFixup::create(Fixup.getOffset(), Fixup.getValue(),
                                Mcasm::reloc_global_offset_table);
    } break;
    }
  }
...
void McasmAsmBackend::applyFixup(const MCFragment &F, const MCFixup &Fixup,
  ...
  // treats more as signed (e.g. unresolved R_Mcasm_64_32S).
...
bool McasmAsmBackend::mayNeedRelaxation(unsigned Opcode,
                                      ArrayRef<MCOperand> Operands,
                                      const MCSubtargetInfo &STI) const {
  unsigned SkipOperands = Mcasm::isCCMPCC(Opcode) ? 2 : 0;
  return isRelaxableBranch(Opcode) ||
         (Mcasm::getOpcodeForLongImmediateForm(Opcode) != Opcode &&
          Operands[Operands.size() - 1 - SkipOperands].isExpr());
}

bool McasmAsmBackend::fixupNeedsRelaxationAdvanced(const MCFragment &,
                                                 const MCFixup &Fixup,
  ...
  if (Fixup.getKind() == FK_Data_1 && Target.getAddSym() &&
      Target.getSpecifier() == Mcasm::S_ABS8)
    return false;
  return true;
}
...
// we can relax?
void McasmAsmBackend::relaxInstruction(MCInst &Inst,
                                     const MCSubtargetInfo &STI) const {
  // The only relaxations Mcasm does is from a 1byte pcrel to a 4byte pcrel.
...
bool McasmAsmBackend::padInstructionViaPrefix(MCFragment &RF,
  ...
  const unsigned RemainingPrefixSize = [&]() -> unsigned {
    SmallString<15> Code;
    Mcasm_MC::emitPrefix(Emitter, RF.getInst(), Code, STI);
...
bool McasmAsmBackend::padInstructionViaRelaxation(MCFragment &RF,
                                                MCCodeEmitter &Emitter,
                                                unsigned &RemainingSize) const {
  if (!mayNeedRelaxation(RF.getOpcode(), RF.getOperands(),
                         *RF.getSubtargetInfo()))
...
bool McasmAsmBackend::padInstructionEncoding(MCFragment &RF,
                                           MCCodeEmitter &Emitter,
                                           unsigned &RemainingSize) const {
  bool Changed = false;
  if (RemainingSize != 0)
...
bool McasmAsmBackend::finishLayout() const {
  // See if we can further relax some instructions to cut down on the number of
  // nop bytes required for code alignment.  The actual win is in reducing
  // instruction count, not number of bytes.  Modern Mcasm-64 can easily end up
  ...
  if (!McasmPadForAlign && !McasmPadForBranchAlign)
    return false;

  // The processed regions are delimitered by LabeledFragments. -g may have more
  // MCSymbols and therefore different relaxation results. McasmPadForAlign is
    ...
    for (MCSection::iterator I = Sec.begin(), IE = Sec.end(); I != IE; ++I) {
      ...
      auto canHandle = [](MCFragment &F) -> bool {
        ...
        case MCFragment::FT_Align:
          return McasmPadForAlign;
        case MCFragment::FT_BoundaryAlign:
          return McasmPadForBranchAlign;
        }
      };
...
unsigned McasmAsmBackend::getMaximumNopSize(const MCSubtargetInfo &STI) const {
  if (STI.hasFeature(Mcasm::Is16Bit))
    return 4;
  if (!STI.hasFeature(Mcasm::FeatureNOPL) && !STI.hasFeature(Mcasm::Is64Bit))
    return 1;
  if (STI.hasFeature(Mcasm::TuningFast7ByteNOP))
    return 7;
  if (STI.hasFeature(Mcasm::TuningFast15ByteNOP))
    return 15;
  if (STI.hasFeature(Mcasm::TuningFast11ByteNOP))
    return 11;
...
bool McasmAsmBackend::writeNopData(raw_ostream &OS, uint64_t Count,
                                 const MCSubtargetInfo *STI) const {
  ...
  const char(*Nops)[11] =
      STI->hasFeature(Mcasm::Is16Bit) ? Nops16Bit : Nops32Bit;
...
class ELFMcasmAsmBackend : public McasmAsmBackend {
public:
  uint8_t OSABI;
  ELFMcasmAsmBackend(const Target &T, uint8_t OSABI, const MCSubtargetInfo &STI)
      : McasmAsmBackend(T, STI), OSABI(OSABI) {}
};

class ELFMcasm_32AsmBackend : public ELFMcasmAsmBackend {
public:
  ELFMcasm_32AsmBackend(const Target &T, uint8_t OSABI,
                      const MCSubtargetInfo &STI)
    : ELFMcasmAsmBackend(T, OSABI, STI) {}

  std::unique_ptr<MCObjectTargetWriter>
  createObjectTargetWriter() const override {
    return createMcasmELFObjectWriter(/*IsELF64*/ false, OSABI, ELF::EM_386);
  }
};

class ELFMcasm_X32AsmBackend : public ELFMcasmAsmBackend {
public:
  ELFMcasm_X32AsmBackend(const Target &T, uint8_t OSABI,
                       const MCSubtargetInfo &STI)
      : ELFMcasmAsmBackend(T, OSABI, STI) {}

  std::unique_ptr<MCObjectTargetWriter>
  createObjectTargetWriter() const override {
    return createMcasmELFObjectWriter(/*IsELF64*/ false, OSABI,
                                    ELF::EM_Mcasm_64);
  }
};

class ELFMcasm_IAMCUAsmBackend : public ELFMcasmAsmBackend {
public:
  ELFMcasm_IAMCUAsmBackend(const Target &T, uint8_t OSABI,
                         const MCSubtargetInfo &STI)
      : ELFMcasmAsmBackend(T, OSABI, STI) {}
...
class ELFMcasm_64AsmBackend : public ELFMcasmAsmBackend {
public:
  ELFMcasm_64AsmBackend(const Target &T, uint8_t OSABI,
                      const MCSubtargetInfo &STI)
    : ELFMcasmAsmBackend(T, OSABI, STI) {}

  std::unique_ptr<MCObjectTargetWriter>
  createObjectTargetWriter() const override {
    return createMcasmELFObjectWriter(/*IsELF64*/ true, OSABI, ELF::EM_Mcasm_64);
  }
};

class WindowsMcasmAsmBackend : public McasmAsmBackend {
  bool Is64Bit;

public:
  WindowsMcasmAsmBackend(const Target &T, bool is64Bit,
                       const MCSubtargetInfo &STI)
    : McasmAsmBackend(T, STI)
    , Is64Bit(is64Bit) {
  }
  ...
  std::unique_ptr<MCObjectTargetWriter>
  createObjectTargetWriter() const override {
    return createMcasmWinCOFFObjectWriter(Is64Bit);
  }
};
...
class DarwinMcasmAsmBackend : public McasmAsmBackend {
...
private:
  ...
  int getCompactUnwindRegNum(unsigned Reg) const {
    static const MCPhysReg CU32BitRegs[7] = {
      Mcasm::EBX, Mcasm::ECX, Mcasm::EDX, Mcasm::EDI, Mcasm::ESI, Mcasm::EBP, 0
    };
    static const MCPhysReg CU64BitRegs[] = {
      Mcasm::RBX, Mcasm::R12, Mcasm::R13, Mcasm::R14, Mcasm::R15, Mcasm::RBP, 0
    };
...
public:
  DarwinMcasmAsmBackend(const Target &T, const MCRegisterInfo &MRI,
                      const MCSubtargetInfo &STI)
      : McasmAsmBackend(T, STI), MRI(MRI), TT(STI.getTargetTriple()),
  ...
  createObjectTargetWriter() const override {
    ...
    return createMcasmMachObjectWriter(Is64Bit, CPUType, CPUSubType);
  }
...
MCAsmBackend *llvm::createMcasm_32AsmBackend(const Target &T,
  ...
  if (TheTriple.isOSBinFormatMachO())
    return new DarwinMcasmAsmBackend(T, MRI, STI);

  if (TheTriple.isOSWindows() && TheTriple.isOSBinFormatCOFF())
    return new WindowsMcasmAsmBackend(T, false, STI);

  uint8_t OSABI = MCELFObjectTargetWriter::getOSABI(TheTriple.getOS());

  if (TheTriple.isOSIAMCU())
    return new ELFMcasm_IAMCUAsmBackend(T, OSABI, STI);

  return new ELFMcasm_32AsmBackend(T, OSABI, STI);
}
...
MCAsmBackend *llvm::createMcasm_64AsmBackend(const Target &T,
  ...
  if (TheTriple.isOSWindows() && TheTriple.isOSBinFormatCOFF())
    return new WindowsMcasmAsmBackend(T, true, STI);
  ...
  if (TheTriple.isX32())
    return new ELFMcasm_X32AsmBackend(T, OSABI, STI);
  return new ELFMcasm_64AsmBackend(T, OSABI, STI);
}

namespace {
class McasmELFStreamer : public MCELFStreamer {
public:
  McasmELFStreamer(MCContext &Context, std::unique_ptr<MCAsmBackend> TAB,
...
void McasmELFStreamer::emitInstruction(const MCInst &Inst,
                                     const MCSubtargetInfo &STI) {
  Mcasm_MC::emitInstruction(*this, Inst, STI);
}

MCStreamer *llvm::createMcasmELFStreamer(const Triple &T, MCContext &Context,
                                       std::unique_ptr<MCAsmBackend> &&MAB,
                                       std::unique_ptr<MCObjectWriter> &&MOW,
                                       std::unique_ptr<MCCodeEmitter> &&MCE) {
  return new McasmELFStreamer(Context, std::move(MAB), std::move(MOW),
                            std::move(MCE));
}
