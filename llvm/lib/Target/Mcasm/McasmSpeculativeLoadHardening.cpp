//====- McasmSpeculativeLoadHardening.cpp - A Spectre v1 mitigation ---------===//
//
...
constexpr StringRef McasmSLHPassName = "Mcasm speculative load hardening";

class McasmSpeculativeLoadHardeningLegacy : public MachineFunctionPass {
public:
  McasmSpeculativeLoadHardeningLegacy() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override { return McasmSLHPassName; }
...
class McasmSpeculativeLoadHardeningImpl {
public:
  McasmSpeculativeLoadHardeningImpl() = default;
...
bool McasmSpeculativeLoadHardeningLegacy::runOnMachineFunction(
    MachineFunction &MF) {
  McasmSpeculativeLoadHardeningImpl Impl;
...
char McasmSpeculativeLoadHardeningLegacy::ID = 0;

void McasmSpeculativeLoadHardeningLegacy::getAnalysisUsage(
...
static MachineBasicBlock &splitEdge(MachineBasicBlock &MBB,
  ...
  if (Br) {
    ...
    if (!UncondBr) {
      ...
      auto BrBuilder =
          BuildMI(&MBB, DebugLoc(), TII.get(Mcasm::JMP_1)).addMBB(&OldLayoutSucc);
...
bool McasmSpeculativeLoadHardeningImpl::run(MachineFunction &MF) {
  LLVM_DEBUG(dbgs() << "********** " << McasmSLHPassName << " : " << MF.getName()
                    << " **********\n");
  ...
  PS.emplace(MF, &Mcasm::GR64_NOSPRegClass);
  ...
  BuildMI(Entry, EntryInsertPt, Loc, TII->get(Mcasm::MOV64ri32), PS->PoisonReg)
      .addImm(PoisonVal);
  ++NumInstsInserted;
  ...
  if (HasVulnerableLoad && FenceCallAndRet) {
    ...
    // a constant.
    BuildMI(Entry, EntryInsertPt, Loc, TII->get(Mcasm::LFENCE));
    ++NumInstsInserted;
    ++NumLFENCEsInserted;
  }
  ...
  if (HardenInterprocedurally && !FenceCallAndRet) {
    ...
    Register PredStateSubReg = MRI->createVirtualRegister(&Mcasm::GR32RegClass);
    auto ZeroI = BuildMI(Entry, EntryInsertPt, Loc, TII->get(Mcasm::MOV32r0),
                         PredStateSubReg);
    ++NumInstsInserted;
    MachineOperand *ZeroEFLAGSDefOp =
        ZeroI->findRegisterDefOperand(Mcasm::EFLAGS, /*TRI=*/nullptr);
    assert(ZeroEFLAGSDefOp && ZeroEFLAGSDefOp->isImplicit() &&
           "Must have an implicit def of EFLAGS!");
    ZeroEFLAGSDefOp->setIsDead(true);
    BuildMI(Entry, EntryInsertPt, Loc, TII->get(Mcasm::SUBREG_TO_REG),
            PS->InitialReg)
        .addImm(0)
...
void McasmSpeculativeLoadHardeningImpl::hardenEdgesWithLFENCE(
    MachineFunction &MF) {
  ...
  for (MachineBasicBlock *MBB : Blocks) {
    ...
    BuildMI(*MBB, InsertPt, DebugLoc(), TII->get(Mcasm::LFENCE));
    ++NumInstsInserted;
    ++NumLFENCEsInserted;
  }
}

SmallVector<McasmSpeculativeLoadHardeningImpl::BlockCondInfo, 16>
McasmSpeculativeLoadHardeningImpl::collectBlockCondInfo(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    for (MachineInstr &MI : llvm::reverse(MBB)) {
      ...
      if (Mcasm::getCondFromBranch(MI) == Mcasm::COND_INVALID) {
        Info.CondBrs.clear();
        Info.UncondBr = &MI;
        continue;
      }
...
/// complete.
SmallVector<MachineInstr *, 16>
McasmSpeculativeLoadHardeningImpl::tracePredStateThroughCFG(
  ...
  for (const BlockCondInfo &Info : Infos) {
    ...
    MachineBasicBlock *UncondSucc =
        UncondBr ? (UncondBr->getOpcode() == Mcasm::JMP_1
    ...
    auto BuildCheckingBlockForSuccAndConds =
        [&](MachineBasicBlock &MBB, MachineBasicBlock &Succ, int SuccCount,
          ...
          bool LiveEFLAGS = Succ.isLiveIn(Mcasm::EFLAGS);
          if (!LiveEFLAGS)
            CheckingMBB.addLiveIn(Mcasm::EFLAGS);
          ...
          for (Mcasm::CondCode Cond : Conds) {
            int PredStateSizeInBytes = TRI->getRegSizeInBits(*PS->RC) / 8;
            auto CMovOp = Mcasm::getCMovOpcode(PredStateSizeInBytes);
            ...
            if (!LiveEFLAGS && Cond == Conds.back())
              CMovI->findRegisterUseOperand(Mcasm::EFLAGS, /*TRI=*/nullptr)
                  ->setIsKill(true);
    ...
    std::vector<Mcasm::CondCode> UncondCodeSeq;
    for (auto *CondBr : CondBrs) {
      MachineBasicBlock &Succ = *CondBr->getOperand(0).getMBB();
      int &SuccCount = SuccCounts[&Succ];

      Mcasm::CondCode Cond = Mcasm::getCondFromBranch(*CondBr);
      Mcasm::CondCode InvCond = Mcasm::GetOppositeBranchCondition(Cond);
...
///
/// FIXME: This should probably live in McasmInstrInfo, potentially by adding
/// a way to unfold into a newly created vreg rather than requiring a register
/// input.
static const TargetRegisterClass *
getRegClassForUnfoldedLoad(const McasmInstrInfo &TII, unsigned Opcode) {
  unsigned Index;
...
void McasmSpeculativeLoadHardeningImpl::unfoldCallAndJumpLoads(
    MachineFunction &MF) {
  for (MachineBasicBlock &MBB : MF)
    ...
    for (MachineInstr &MI : llvm::make_early_inc_range(MBB.instrs())) {
      ...
      case Mcasm::FARCALL16m:
      case Mcasm::FARCALL32m:
      case Mcasm::FARCALL64m:
      case Mcasm::FARJMP16m:
      case Mcasm::FARJMP32m:
      case Mcasm::FARJMP64m:
        // We cannot mitigate far jumps or calls, but we also don't expect them
        // to be vulnerable to Spectre v1.2 style attacks.
        continue;

      case Mcasm::CALL16m:
      case Mcasm::CALL16m_NT:
      case Mcasm::CALL32m:
      case Mcasm::CALL32m_NT:
      case Mcasm::CALL64m:
      case Mcasm::CALL64m_NT:
      case Mcasm::JMP16m:
      case Mcasm::JMP16m_NT:
      case Mcasm::JMP32m:
      case Mcasm::JMP32m_NT:
      case Mcasm::JMP64m:
      case Mcasm::JMP64m_NT:
...
SmallVector<MachineInstr *, 16>
McasmSpeculativeLoadHardeningImpl::tracePredStateThroughIndirectBranches(
    MachineFunction &MF) {
  ...
  TargetAddrSSA.Initialize(MRI->createVirtualRegister(&Mcasm::GR64RegClass));
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    // hardening.
    assert(!MBB.isLiveIn(Mcasm::EFLAGS) &&
    ...
    for (MachineBasicBlock *Pred : MBB.predecessors()) {
      ...
      Register TargetReg = MRI->createVirtualRegister(&Mcasm::GR64RegClass);
      if (MF.getTarget().getCodeModel() == CodeModel::Small &&
        ...
        auto AddrI = BuildMI(*Pred, InsertPt, DebugLoc(),
                             TII->get(Mcasm::MOV64ri32), TargetReg)
                         .addMBB(&MBB);
        ++NumInstsInserted;
        (void)AddrI;
        ...
        auto AddrI = BuildMI(*Pred, InsertPt, DebugLoc(), TII->get(Mcasm::LEA64r),
                             TargetReg)
                         .addReg(/*Base*/ Mcasm::RIP)
                         .addImm(/*Scale*/ 1)
                         .addReg(/*Index*/ 0)
                         .addMBB(&MBB)
                         .addReg(/*Segment*/ 0);
    ...
    if (MF.getTarget().getCodeModel() == CodeModel::Small &&
      ...
      auto CheckI = BuildMI(MBB, InsertPt, DebugLoc(), TII->get(Mcasm::CMP64ri32))
      ...
      Register AddrReg = MRI->createVirtualRegister(&Mcasm::GR64RegClass);
      auto AddrI =
          BuildMI(MBB, InsertPt, DebugLoc(), TII->get(Mcasm::LEA64r), AddrReg)
              .addReg(/*Base*/ Mcasm::RIP)
              .addImm(/*Scale*/ 1)
              .addReg(/*Index*/ 0)
              .addMBB(&MBB)
      ...
      auto CheckI = BuildMI(MBB, InsertPt, DebugLoc(), TII->get(Mcasm::CMP64rr))
                        .addReg(TargetReg, RegState::Kill)
...
void McasmSpeculativeLoadHardeningImpl::tracePredStateThroughBlocksAndHarden(
    MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    if (HardenLoads)
      for (MachineInstr &MI : MBB) {
        ...
        const int MemRefBeginIdx = Mcasm::getFirstAddrOperandIdx(MI);
        if (MemRefBeginIdx < 0) {
          LLVM_DEBUG(dbgs()
        ...
        MachineOperand &BaseMO =
            MI.getOperand(MemRefBeginIdx + Mcasm::AddrBaseReg);
        MachineOperand &IndexMO =
            MI.getOperand(MemRefBeginIdx + Mcasm::AddrIndexReg);
        ...
        if (!BaseMO.isFI() && BaseMO.getReg() != Mcasm::RIP &&
        ...
        if (EnablePostLoadHardening && McasmInstrInfo::isDataInvariantLoad(MI) &&
    ...
    for (MachineInstr &MI : MBB) {
      if (HardenLoads) {
        ...
        if (HardenPostLoad.erase(&MI)) {
          ...
          // possible.
          if (McasmInstrInfo::isDataInvariantLoad(MI) && !isEFLAGSDefLive(MI)) {
...
/// reliably lower.
void McasmSpeculativeLoadHardeningImpl::restoreEFLAGS(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator InsertPt,
    const DebugLoc &Loc, Register Reg) {
  BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::COPY), Mcasm::EFLAGS).addReg(Reg);
  ++NumInstsInserted;
}
...
void McasmSpeculativeLoadHardeningImpl::mergePredStateIntoSP(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator InsertPt,
    const DebugLoc &Loc, Register PredStateReg) {
  ...
  auto ShiftI = BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::SHL64ri), TmpReg)
                    .addReg(PredStateReg, RegState::Kill)
                    .addImm(47);
  ShiftI->addRegisterDead(Mcasm::EFLAGS, TRI);
  ++NumInstsInserted;
  auto OrI = BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::OR64rr), Mcasm::RSP)
                 .addReg(Mcasm::RSP)
                 .addReg(TmpReg, RegState::Kill);
  OrI->addRegisterDead(Mcasm::EFLAGS, TRI);
  ++NumInstsInserted;
}

/// Extracts the predicate state stored in the high bits of the stack pointer.
Register McasmSpeculativeLoadHardeningImpl::extractPredStateFromSP(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator InsertPt,
    const DebugLoc &Loc) {
  ...
  BuildMI(MBB, InsertPt, Loc, TII->get(TargetOpcode::COPY), TmpReg)
      .addReg(Mcasm::RSP);
  auto ShiftI =
      BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::SAR64ri), PredStateReg)
          .addReg(TmpReg, RegState::Kill)
...
void McasmSpeculativeLoadHardeningImpl::hardenLoadAddr(
  ...
  if (BaseMO.isFI()) {
    ...
    LLVM_DEBUG(
        ...
        MI.dump(); dbgs() << "\n");
  } else if (BaseMO.getReg() == Mcasm::RSP) {
    ...
    assert(IndexMO.getReg() == Mcasm::NoRegister &&
           "Explicit RSP access with dynamic index!");
    LLVM_DEBUG(
        dbgs() << "  Cannot harden base of explicit RSP offset in a load!");
  } else if (BaseMO.getReg() == Mcasm::RIP ||
             BaseMO.getReg() == Mcasm::NoRegister) {
    ...
    LLVM_DEBUG(
        dbgs() << "  Cannot harden base of "
               << (BaseMO.getReg() == Mcasm::RIP ? "RIP-relative" : "no-base")
               << " address in a load!");
  } else {
    assert(BaseMO.isReg() &&
  ...
  if (IndexMO.getReg() != Mcasm::NoRegister &&
      (HardenOpRegs.empty() ||
  ...
  for (MachineOperand *Op : HardenOpRegs) {
    ...
    // hardening it.
    if (!Subtarget->hasVLX() && (OpRC->hasSuperClassEq(&Mcasm::VR128RegClass) ||
                                 OpRC->hasSuperClassEq(&Mcasm::VR256RegClass))) {
      assert(Subtarget->hasAVX2() && "AVX2-specific register classes!");
      bool Is128Bit = OpRC->hasSuperClassEq(&Mcasm::VR128RegClass);
      ...
      Register VStateReg = MRI->createVirtualRegister(&Mcasm::VR128RegClass);
      auto MovI =
          BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::VMOV64toPQIrr), VStateReg)
              .addReg(StateReg);
      (void)MovI;
      ++NumInstsInserted;
      ...
      auto BroadcastI = BuildMI(MBB, InsertPt, Loc,
                                TII->get(Is128Bit ? Mcasm::VPBROADCASTQrr
                                                  : Mcasm::VPBROADCASTQYrr),
                                VBStateReg)
                            .addReg(VStateReg);
      (void)BroadcastI;
      ++NumInstsInserted;
      ...
      auto OrI =
          BuildMI(MBB, InsertPt, Loc,
                  TII->get(Is128Bit ? Mcasm::VPORrr : Mcasm::VPORYrr), TmpReg)
              .addReg(VBStateReg)
              .addReg(OpReg);
      (void)OrI;
      ++NumInstsInserted;
      LLVM_DEBUG(dbgs() << "  Inserting or: "; OrI->dump(); dbgs() << "\n");
    } else if (OpRC->hasSuperClassEq(&Mcasm::VR128XRegClass) ||
               OpRC->hasSuperClassEq(&Mcasm::VR256XRegClass) ||
               OpRC->hasSuperClassEq(&Mcasm::VR512RegClass)) {
      assert(Subtarget->hasAVX512() && "AVX512-specific register classes!");
      bool Is128Bit = OpRC->hasSuperClassEq(&Mcasm::VR128XRegClass);
      bool Is256Bit = OpRC->hasSuperClassEq(&Mcasm::VR256XRegClass);
      ...
      unsigned BroadcastOp = Is128Bit ? Mcasm::VPBROADCASTQrZ128rr
                                      : Is256Bit ? Mcasm::VPBROADCASTQrZ256rr
                                                 : Mcasm::VPBROADCASTQrZrr;
      auto BroadcastI =
      ...
      // Merge our potential poison state into the value with a vector or.
      unsigned OrOp = Is128Bit ? Mcasm::VPORQZ128rr
                               : Is256Bit ? Mcasm::VPORQZ256rr : Mcasm::VPORQZrr;
      ...
      if (!EFLAGSLive) {
        ...
        auto ShiftI =
            BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::SHRX64rr), TmpReg)
                .addReg(OpReg)
                .addReg(StateReg);
        (void)ShiftI;
        ++NumInstsInserted;
...
MachineInstr *McasmSpeculativeLoadHardeningImpl::sinkPostLoadHardenedInst(
    MachineInstr &InitialMI, SmallPtrSetImpl<MachineInstr *> &HardenedInstrs) {
  assert(McasmInstrInfo::isDataInvariantLoad(InitialMI) &&
  ...
  auto SinkCheckToSingleUse =
    ...
    for (MachineInstr &UseMI : MRI->use_instructions(DefReg)) {
      ...
      if (HardenedInstrs.count(&UseMI)) {
        if (!McasmInstrInfo::isDataInvariantLoad(UseMI) || isEFLAGSDefLive(UseMI)) {
          ...
          // be data-invariant.
          assert(McasmInstrInfo::isDataInvariant(UseMI) &&
        ...
        const int MemRefBeginIdx = Mcasm::getFirstAddrOperandIdx(UseMI);
        assert(MemRefBeginIdx >= 0 &&
               "Should always have mem references here!");

        MachineOperand &BaseMO =
            UseMI.getOperand(MemRefBeginIdx + Mcasm::AddrBaseReg);
        MachineOperand &IndexMO =
            UseMI.getOperand(MemRefBeginIdx + Mcasm::AddrIndexReg);
      ...
      if (!McasmInstrInfo::isDataInvariant(UseMI) || UseMI.getParent() != MI.getParent() ||
...
bool McasmSpeculativeLoadHardeningImpl::canHardenRegister(Register Reg) {
  ...
  const TargetRegisterClass *NOREXRegClasses[] = {
      &Mcasm::GR8_NOREXRegClass, &Mcasm::GR16_NOREXRegClass,
      &Mcasm::GR32_NOREXRegClass, &Mcasm::GR64_NOREXRegClass};
  if (RC == NOREXRegClasses[RegIdx])
    return false;

  const TargetRegisterClass *GPRRegClasses[] = {
      &Mcasm::GR8RegClass, &Mcasm::GR16RegClass, &Mcasm::GR32RegClass,
      &Mcasm::GR64RegClass};
...
Register McasmSpeculativeLoadHardeningImpl::hardenValueInRegister(
  ...
  if (Bytes != 8) {
    unsigned SubRegImms[] = {Mcasm::sub_8bit, Mcasm::sub_16bit, Mcasm::sub_32bit};
  ...
  unsigned OrOpCodes[] = {Mcasm::OR8rr, Mcasm::OR16rr, Mcasm::OR32rr, Mcasm::OR64rr};
...
Register McasmSpeculativeLoadHardeningImpl::hardenPostLoad(MachineInstr &MI) {
  MachineBasicBlock &MBB = *MI.getParent();
  const DebugLoc &Loc = MI.getDebugLoc();
...
void McasmSpeculativeLoadHardeningImpl::hardenReturnInstr(MachineInstr &MI) {
  MachineBasicBlock &MBB = *MI.getParent();
  const DebugLoc &Loc = MI.getDebugLoc();
  auto InsertPt = MI.getIterator();
...
/// state.
void McasmSpeculativeLoadHardeningImpl::tracePredStateThroughCall(
    MachineInstr &MI) {
  MachineBasicBlock &MBB = *MI.getParent();
  MachineFunction &MF = *MBB.getParent();
  auto InsertPt = MI.getIterator();
  ...
  if (FenceCallAndRet) {
    ...
    BuildMI(MBB, std::next(InsertPt), Loc, TII->get(Mcasm::LFENCE));
    ++NumInstsInserted;
    ++NumLFENCEsInserted;
    return;
  }
  ...
  const TargetRegisterClass *AddrRC = &Mcasm::GR64RegClass;
  ...
  if (!Subtarget->getFrameLowering()->has128ByteRedZone(MF) ||
    ...
    if (MF.getTarget().getCodeModel() == CodeModel::Small &&
        !Subtarget->isPositionIndependent()) {
      BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::MOV64ri32), ExpectedRetAddrReg)
          .addSym(RetSymbol);
    } else {
      BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::LEA64r), ExpectedRetAddrReg)
          .addReg(/*Base*/ Mcasm::RIP)
          .addImm(/*Scale*/ 1)
          .addReg(/*Index*/ 0)
          .addSym(RetSymbol)
  ...
  if (!ExpectedRetAddrReg) {
    ExpectedRetAddrReg = MRI->createVirtualRegister(AddrRC);
    BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::MOV64rm), ExpectedRetAddrReg)
        .addReg(/*Base*/ Mcasm::RSP)
        .addImm(/*Scale*/ 1)
        .addReg(/*Index*/ 0)
  ...
  if (MF.getTarget().getCodeModel() == CodeModel::Small &&
    ...
    // management.
    BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::CMP64ri32))
        .addReg(ExpectedRetAddrReg, RegState::Kill)
        .addSym(RetSymbol);
  } else {
    Register ActualRetAddrReg = MRI->createVirtualRegister(AddrRC);
    BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::LEA64r), ActualRetAddrReg)
        .addReg(/*Base*/ Mcasm::RIP)
        .addImm(/*Scale*/ 1)
        .addReg(/*Index*/ 0)
        .addSym(RetSymbol)
        .addReg(/*Segment*/ 0);
    BuildMI(MBB, InsertPt, Loc, TII->get(Mcasm::CMP64rr))
        .addReg(ExpectedRetAddrReg, RegState::Kill)
  ...
  auto CMovI = BuildMI(MBB, InsertPt, Loc, TII->get(CMovOp), UpdatedStateReg)
                   ...
                   .addReg(PS->PoisonReg)
                   .addImm(Mcasm::COND_NE);
  CMovI->findRegisterUseOperand(Mcasm::EFLAGS, /*TRI=*/nullptr)->setIsKill(true);
  ++NumInstsInserted;
  LLVM_DEBUG(dbgs() << "  Inserting cmov: "; CMovI->dump(); dbgs() << "\n");
...
void McasmSpeculativeLoadHardeningImpl::hardenIndirectCallOrJumpInstr(
    MachineInstr &MI,
    SmallDenseMap<Register, Register, 32> &AddrRegToHardenedReg) {
  switch (MI.getOpcode()) {
  case Mcasm::FARCALL16m:
...
PreservedAnalyses
McasmSpeculativeLoadHardeningPass::run(MachineFunction &MF,
...
INITIALIZE_PASS_BEGIN(McasmSpeculativeLoadHardeningLegacy, PASS_KEY,
                      "Mcasm speculative load hardener", false, false)
INITIALIZE_PASS_END(McasmSpeculativeLoadHardeningLegacy, PASS_KEY,
                    "Mcasm speculative load hardener", false, false)

FunctionPass *llvm::createMcasmSpeculativeLoadHardeningLegacyPass() {
  return new McasmSpeculativeLoadHardeningLegacy();
}
