//===-- McasmFixupBWInsts.cpp - Fixup Byte or Word instructions -----------===//
//
...
#define FIXUPBW_DESC "Mcasm Byte/Word Instruction Fixup"
...
namespace {
class McasmFixupBWInstImpl {
public:
  McasmFixupBWInstImpl(ProfileSummaryInfo *PSI, MachineBlockFrequencyInfo *MBFI)
      : PSI(PSI), MBFI(MBFI) {}
...
class McasmFixupBWInstLegacy : public MachineFunctionPass {
public:
  static char ID;

  StringRef getPassName() const override { return FIXUPBW_DESC; }

  McasmFixupBWInstLegacy() : MachineFunctionPass(ID) {}
...
char McasmFixupBWInstLegacy::ID = 0;

} // namespace

INITIALIZE_PASS(McasmFixupBWInstLegacy, FIXUPBW_NAME, FIXUPBW_DESC, false, false)

FunctionPass *llvm::createMcasmFixupBWInstsLegacyPass() {
  return new McasmFixupBWInstLegacy();
}

bool McasmFixupBWInstImpl::runOnMachineFunction(MachineFunction &MF) {
  if (!FixupBWInsts)
    return false;
  ...
  LLVM_DEBUG(dbgs() << "Start McasmFixupBWInsts\n";);

  // Process all basic blocks.
  for (auto &MBB : MF)
    processBasicBlock(MF, MBB);

  LLVM_DEBUG(dbgs() << "End McasmFixupBWInsts\n";);
...
Register McasmFixupBWInstImpl::getSuperRegDestIfDead(MachineInstr *OrigMI) const {
  const McasmRegisterInfo *TRI = &TII->getRegisterInfo();
  Register OrigDestReg = OrigMI->getOperand(0).getReg();
  Register SuperDestReg = getMcasmSubSuperRegister(OrigDestReg, 32);
  ...
  if (SubRegIdx == Mcasm::sub_8bit_hi)
    return Register();
  ...
  //
  // The Mcasm backend does not have subregister liveness tracking enabled,
  ...
  // same properties.
  if (Opc != Mcasm::MOV8rm && Opc != Mcasm::MOV16rm && Opc != Mcasm::MOV8rr &&
      Opc != Mcasm::MOV16rr)
    return Register();
...
MachineInstr *McasmFixupBWInstImpl::tryReplaceLoad(unsigned New32BitOpcode,
                                                 MachineInstr *MI) const {
...
MachineInstr *McasmFixupBWInstImpl::tryReplaceCopy(MachineInstr *MI) const {
  ...
  Register NewSrcReg = getMcasmSubSuperRegister(OldSrc.getReg(), 32);
  ...
  MachineInstrBuilder MIB =
      BuildMI(*MF, MIMetadata(*MI), TII->get(Mcasm::MOV32rr), NewDestReg)
...
MachineInstr *McasmFixupBWInstImpl::tryReplaceExtend(unsigned New32BitOpcode,
                                                   MachineInstr *MI) const {
  Register NewDestReg = getSuperRegDestIfDead(MI);
  if (!NewDestReg)
    return nullptr;
  ...
  if (MI->getOpcode() == Mcasm::MOVSX16rr8 &&
      MI->getOperand(0).getReg() == Mcasm::AX &&
      MI->getOperand(1).getReg() == Mcasm::AL)
    return nullptr;
...
MachineInstr *
McasmFixupBWInstImpl::tryReplaceInstr(MachineInstr *MI,
  ...
  case Mcasm::MOV8rm:
    ...
    if (!OptForSize)
      return tryReplaceLoad(Mcasm::MOVZX32rm8, MI);
    break;
  ...
  case Mcasm::MOV16rm:
    ...
    // the register.
    return tryReplaceLoad(Mcasm::MOVZX32rm16, MI);
  ...
  case Mcasm::MOVSX16rr8:
    return tryReplaceExtend(Mcasm::MOVSX32rr8, MI);
  case Mcasm::MOVSX16rm8:
    return tryReplaceExtend(Mcasm::MOVSX32rm8, MI);
  case Mcasm::MOVZX16rr8:
    return tryReplaceExtend(Mcasm::MOVZX32rr8, MI);
  case Mcasm::MOVZX16rm8:
    return tryReplaceExtend(Mcasm::MOVZX32rm8, MI);
...
void McasmFixupBWInstImpl::processBasicBlock(MachineFunction &MF,
...
bool McasmFixupBWInstLegacy::runOnMachineFunction(MachineFunction &MF) {
  ...
  MachineBlockFrequencyInfo *MBFI =
      (PSI && PSI->hasProfileSummary())
          ...
          : nullptr;
  McasmFixupBWInstImpl Impl(PSI, MBFI);
  return Impl.runOnMachineFunction(MF);
}

PreservedAnalyses
McasmFixupBWInstsPass::run(MachineFunction &MF,
