//===-- McasmAsmPrinter.h - Mcasm implementation of AsmPrinter ------*- C++ -*-===//
//
...
// Implemented in McasmMCInstLower.cpp
namespace {
...
private:
  ...
  // It is used by the McasmAsmPrinter to ensure that the stackmap shadow
  ...
  // All instructions emitted by the McasmAsmPrinter should use this helper
  // method.
  //
  ...
  void LowerPATCHPOINT(const MachineInstr &MI, McasmMCInstLower &MCIL);
  void LowerSTATEPOINT(const MachineInstr &MI, McasmMCInstLower &MCIL);
  void LowerFAULTING_OP(const MachineInstr &MI, McasmMCInstLower &MCIL);
  void LowerPATCHABLE_OP(const MachineInstr &MI, McasmMCInstLower &MCIL);

  void LowerTlsAddr(McasmMCInstLower &MCInstLowering, const MachineInstr &MI);

  // XRay-specific lowering for Mcasm.
  void LowerPATCHABLE_FUNCTION_ENTER(const MachineInstr &MI,
                                     McasmMCInstLower &MCIL);
  void LowerPATCHABLE_RET(const MachineInstr &MI, McasmMCInstLower &MCIL);
  void LowerPATCHABLE_TAIL_CALL(const MachineInstr &MI, McasmMCInstLower &MCIL);
  void LowerPATCHABLE_EVENT_CALL(const MachineInstr &MI, McasmMCInstLower &MCIL);
  void LowerPATCHABLE_TYPED_EVENT_CALL(const MachineInstr &MI,
                                       McasmMCInstLower &MCIL);

  void LowerFENTRY_CALL(const MachineInstr &MI, McasmMCInstLower &MCIL);

  // KCFI specific lowering for Mcasm.
  uint32_t MaskKCFIType(uint32_t Value);
  void EmitKCFITypePadding(const MachineFunction &MF, bool HasType = true);
  void LowerKCFI_CHECK(const MachineInstr &MI);

  // Address sanitizer specific lowering for Mcasm.
...
public:
  McasmAsmPrinter(TargetMachine &TM, std::unique_ptr<MCStreamer> Streamer);

  StringRef getPassName() const override {
    return "Mcasm Assembly Printer";
  }
