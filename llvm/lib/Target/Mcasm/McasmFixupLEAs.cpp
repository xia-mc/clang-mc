//===-- McasmFixupLEAs.cpp - use or replace LEA instructions -----------===//
//
...
#define FIXUPLEA_DESC "Mcasm LEA Fixup"
  ...
  case Mcasm::MOV64rr: {
    ...
    MachineInstr *NewMI =
        BuildMI(MBB, MBBI, MI.getDebugLoc(),
                TII->get(MI.getOpcode() == Mcasm::MOV32rr ? Mcasm::LEA32r
                                                        : Mcasm::LEA64r))
            .add(Dest)
            .add(Src)
            .addImm(1)
...
FunctionPass *llvm::createMcasmFixupLEAsLegacyPass() {
  return new FixupLEAsLegacy();
}

static bool isLEA(unsigned Opcode) {
  return Opcode == Mcasm::LEA32r || Opcode == Mcasm::LEA64r ||
...
bool FixupLEAsImpl::runOnMachineFunction(MachineFunction &MF) {
  ...
  LLVM_DEBUG(dbgs() << "Start McasmFixupLEAs\n";);
  ...
  LLVM_DEBUG(dbgs() << "End McasmFixupLEAs\n";);
...
static inline bool isInefficientLEAReg(Register Reg) {
  return Reg == Mcasm::EBP || Reg == Mcasm::RBP ||
         Reg == Mcasm::R13D || Reg == Mcasm::R13;
}
...
static inline unsigned getADDrrFromLEA(unsigned LEAOpcode) {
  ...
  case Mcasm::LEA64_32r:
    return Mcasm::ADD32rr;
  case Mcasm::LEA64r:
    return Mcasm::ADD64rr;
  }
}
...
static inline unsigned getSUBrrFromLEA(unsigned LEAOpcode) {
  ...
  case Mcasm::LEA32r:
  case Mcasm::LEA64_32r:
    return Mcasm::SUB32rr;
  case Mcasm::LEA64r:
    return Mcasm::SUB64rr;
  }
}
...
static inline unsigned getADDriFromLEA(unsigned LEAOpcode,
  ...
  case Mcasm::LEA32r:
  case Mcasm::LEA64_32r:
    return Mcasm::ADD32ri;
  case Mcasm::LEA64r:
    return Mcasm::ADD64ri32;
  }
}
...
static inline unsigned getINCDECFromLEA(unsigned LEAOpcode, bool IsINC) {
  ...
  case Mcasm::LEA32r:
  case Mcasm::LEA64_32r:
    return IsINC ? Mcasm::INC32r : Mcasm::DEC32r;
  case Mcasm::LEA64r:
    return IsINC ? Mcasm::INC64r : Mcasm::DEC64r;
  }
}
    ...
    for (unsigned I = 0, E = CurInst->getNumOperands(); I != E; ++I) {
      ...
      if (Opnd.isReg()) {
        if (Opnd.getReg() == DestReg) {
          ...
          if (!CurInst->registerDefIsDead(Mcasm::EFLAGS, TRI))
            return MachineBasicBlock::iterator();
...
void FixupLEAsImpl::checkRegUsage(MachineBasicBlock::iterator &LeaI,
  ...
  *KilledBase = *KilledIndex = nullptr;
  Register BaseReg = LeaI->getOperand(1 + Mcasm::AddrBaseReg).getReg();
  Register IndexReg = LeaI->getOperand(1 + Mcasm::AddrIndexReg).getReg();
...
bool FixupLEAsImpl::optLEAALU(MachineBasicBlock::iterator &I,
  ...
  Register AluDestReg = AluI->getOperand(0).getReg();
  Register BaseReg = I->getOperand(1 + Mcasm::AddrBaseReg).getReg();
  Register IndexReg = I->getOperand(1 + Mcasm::AddrIndexReg).getReg();
  if (I->getOpcode() == Mcasm::LEA64_32r) {
    BaseReg = TRI->getSubReg(BaseReg, Mcasm::sub_32bit);
    IndexReg = TRI->getSubReg(IndexReg, Mcasm::sub_32bit);
  }
...
bool FixupLEAsImpl::optTwoAddrLEA(MachineBasicBlock::iterator &I,
  ...
  const MachineOperand &Base =    MI.getOperand(1 + Mcasm::AddrBaseReg);
  const MachineOperand &Scale =   MI.getOperand(1 + Mcasm::AddrScaleAmt);
  const MachineOperand &Index =   MI.getOperand(1 + Mcasm::AddrIndexReg);
  ...
  if (Segment.getReg().isValid() || !Disp.isImm() || Scale.getImm() > 1 ||
      MBB.computeRegisterLiveness(TRI, Mcasm::EFLAGS, I) !=
  ...
  if (UseLEAForSP && (DestReg == Mcasm::ESP || DestReg == Mcasm::RSP))
    return false;

  // LEA64_32 has 64-bit operands but 32-bit result.
  if (MI.getOpcode() == Mcasm::LEA64_32r) {
    if (BaseReg)
      BaseReg = TRI->getSubReg(BaseReg, Mcasm::sub_32bit);
    if (IndexReg)
      IndexReg = TRI->getSubReg(IndexReg, Mcasm::sub_32bit);
...
void FixupLEAsImpl::processInstruction(MachineBasicBlock::iterator &I,
  ...
  int AddrOffset = McasmII::getMemoryOperandNo(Desc.TSFlags);
  if (AddrOffset >= 0) {
    AddrOffset += McasmII::getOperandBias(Desc);
    MachineOperand &p = MI.getOperand(AddrOffset + Mcasm::AddrBaseReg);
    if (p.isReg() && p.getReg() != Mcasm::ESP) {
      seekLEAFixup(p, I, MBB);
    }
    MachineOperand &q = MI.getOperand(AddrOffset + Mcasm::AddrIndexReg);
    if (q.isReg() && q.getReg() != Mcasm::ESP) {
...
void FixupLEAsImpl::processInstructionForSlowLEA(MachineBasicBlock::iterator &I,
  ...
  const MachineOperand &Scale =   MI.getOperand(1 + Mcasm::AddrScaleAmt);
  const MachineOperand &Index =   MI.getOperand(1 + Mcasm::AddrIndexReg);
  const MachineOperand &Offset =  MI.getOperand(1 + Mcasm::AddrDisp);
  const MachineOperand &Segment = MI.getOperand(1 + Mcasm::AddrSegmentReg);

  if (Segment.getReg().isValid() || !Offset.isImm() ||
      MBB.computeRegisterLiveness(TRI, Mcasm::EFLAGS, I, 4) !=
          MachineBasicBlock::LQR_Dead)
    return;
...
PreservedAnalyses McasmFixupLEAsPass::run(MachineFunction &MF,
