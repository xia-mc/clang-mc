//===- McasmISelDAGToDAG.cpp - A DAG pattern matching inst selector for Mcasm -===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines a DAG pattern matching instruction selector for Mcasm,
// converting from a legalized dag to a Mcasm dag.
//
//===----------------------------------------------------------------------===//
...
namespace {
  /// This corresponds to McasmAddressMode, but uses SDValue's instead of register
  /// numbers for the leaves of the matched tree.
  struct McasmISelAddressMode {
    enum {
      RegBase,
      FrameIndexBase
    } BaseType = RegBase;
    ...
    unsigned char SymbolFlags = McasmII::MO_NO_FLAG;  // McasmII::MO_*
    bool NegateIndex = false;

    McasmISelAddressMode() = default;
    ...
    bool isRIPRelative() const {
      ...
      if (RegisterSDNode *RegNode =
        ...
        return RegNode->getReg() == Mcasm::RIP;
      return false;
    }
    ...
    void dump(SelectionDAG *DAG = nullptr) {
      dbgs() << "McasmISelAddressMode " << this << '\n';
      dbgs() << "Base_Reg ";
      if (Base_Reg.getNode())
...
namespace {
  //===--------------------------------------------------------------------===//
  /// ISel - Mcasm-specific code to select Mcasm machine instructions for
  /// SelectionDAG operations.
  ///
  class McasmDAGToDAGISel final : public SelectionDAGISel {
    /// Keep a pointer to the McasmSubtarget around so that we can
...
#include "McasmGenDAGISel.inc"
  ...
  private:
    ...
    bool foldOffsetIntoAddress(uint64_t Offset, McasmISelAddressMode &AM);
    bool matchLoadInAddress(LoadSDNode *N, McasmISelAddressMode &AM,
                            bool AllowSegmentRegForX32 = false);
    bool matchWrapper(SDValue N, McasmISelAddressMode &AM);
    bool matchAddress(SDValue N, McasmISelAddressMode &AM);
    bool matchVectorAddress(SDValue N, McasmISelAddressMode &AM);
    bool matchAdd(SDValue &N, McasmISelAddressMode &AM, unsigned Depth);
    SDValue matchIndexRecursively(SDValue N, McasmISelAddressMode &AM,
                                  unsigned Depth);
    bool matchAddressRecursively(SDValue N, McasmISelAddressMode &AM,
                                 unsigned Depth);
    bool matchVectorAddressRecursively(SDValue N, McasmISelAddressMode &AM,
                                       unsigned Depth);
    bool matchAddressBase(SDValue N, McasmISelAddressMode &AM);
    ...
    inline void getAddressOperands(McasmISelAddressMode &AM, const SDLoc &DL,
                                   MVT VT, SDValue &Base, SDValue &Scale,
                                   SDValue &Index, SDValue &Disp,
                                   SDValue &Segment) {
      if (AM.BaseType == McasmISelAddressMode::FrameIndexBase)
...
#define GET_ND_IF_ENABLED(OPC) (Subtarget->hasNDD() ? OPC##_ND : OPC)
      ...
      if (AM.NegateIndex) {
        ...
        case MVT::i64:
          NegOpc = GET_ND_IF_ENABLED(Mcasm::NEG64r);
          break;
        case MVT::i32:
          NegOpc = GET_ND_IF_ENABLED(Mcasm::NEG32r);
          break;
        case MVT::i16:
          NegOpc = GET_ND_IF_ENABLED(Mcasm::NEG16r);
          break;
        case MVT::i8:
          NegOpc = GET_ND_IF_ENABLED(Mcasm::NEG8r);
          break;
        }
    ...
    bool shouldAvoidImmediateInstFormsForSize(SDNode *N) const {
      ...
      // See the comment in McasmInstrInfo.td for more info.
      ...
      for (const SDNode *User : N->users()) {
        ...
        if (User->getOpcode() == McasmISD::ADD ||
            User->getOpcode() == ISD::ADD    ||
            User->getOpcode() == McasmISD::SUB ||
          ...
          if (OtherOp->getOpcode() == ISD::CopyFromReg &&
            ...
            if ((RegNode->getReg() == Mcasm::ESP) ||
                (RegNode->getReg() == Mcasm::RSP))
              continue;
        }
    ...
    SDValue getSBBZero(SDNode *N) {
      ...
      SDValue Zero =
          SDValue(CurDAG->getMachineNode(Mcasm::MOV32r0, dl, VTs, {}), 0);
      if (VT == MVT::i64) {
        Zero = SDValue(
            CurDAG->getMachineNode(
                TargetOpcode::SUBREG_TO_REG, dl, MVT::i64,
                CurDAG->getTargetConstant(0, dl, MVT::i64), Zero,
                CurDAG->getTargetConstant(Mcasm::sub_32bit, dl, MVT::i32)),
            0);
      }

      // Copy flags to the EFLAGS register and glue it to next node.
      unsigned Opcode = N->getOpcode();
      assert((Opcode == McasmISD::SBB || Opcode == McasmISD::SETCC_CARRY) &&
             "Unexpected opcode for SBB materialization");
      unsigned FlagOpIndex = Opcode == McasmISD::SBB ? 2 : 1;
      SDValue EFLAGS =
          CurDAG->getCopyToReg(CurDAG->getEntryNode(), dl, Mcasm::EFLAGS,
                               N->getOperand(FlagOpIndex), SDValue());

      // Create a 64-bit instruction if the result is 64-bits otherwise use the
      // 32-bit version.
      unsigned Opc = VT == MVT::i64 ? Mcasm::SBB64rr : Mcasm::SBB32rr;
    ...
    Mcasm::CondCode getCondFromNode(SDNode *N) const;
...
// type.
static bool isLegalMaskCompare(SDNode *N, const McasmSubtarget *Subtarget) {
  unsigned Opcode = N->getOpcode();
  if (Opcode == McasmISD::CMPM || Opcode == McasmISD::CMPMM ||
      Opcode == McasmISD::STRICT_CMPM || Opcode == ISD::SETCC ||
      Opcode == McasmISD::CMPMM_SAE || Opcode == McasmISD::VFPCLASS) {
    // We can get 256-bit 8 element types here without VLX being enabled. When
    // this happens we will use 512-bit operations and the mask will not be
    // zero extended.
    EVT OpVT = N->getOperand(0).getValueType();
    // The first operand of McasmISD::STRICT_CMPM is chain, so we need to get the
    // second operand.
    if (Opcode == McasmISD::STRICT_CMPM)
  ...
  if (Opcode == McasmISD::VFPCLASSS || Opcode == McasmISD::FSETCCM ||
      Opcode == McasmISD::FSETCCM_SAE)
    return true;
...
// for us.
bool McasmDAGToDAGISel::isMaskZeroExtended(SDNode *N) const {
...
bool
McasmDAGToDAGISel::IsProfitableToFold(SDValue N, SDNode *U, SDNode *Root) const {
  ...
  if (U == Root) {
    ...
    case ISD::XOR: {
      ...
      if (auto *Imm = dyn_cast<ConstantSDNode>(Op1)) {
        ...
        if ((U->getOpcode() == McasmISD::ADD || U->getOpcode() == McasmISD::SUB) &&
            (-Imm->getAPIntValue()).isSignedIntN(8) &&
      ...
      if (Op1.getOpcode() == McasmISD::Wrapper) {
...
bool McasmDAGToDAGISel::isProfitableToFormMaskedOp(SDNode *N) const {
  assert(
      (N->getOpcode() == ISD::VSELECT || N->getOpcode() == McasmISD::SELECTS) &&
      "Unexpected opcode!");
...
void McasmDAGToDAGISel::PreprocessISelDAG() {
  ...
  for (SelectionDAG::allnodes_iterator I = CurDAG->allnodes_begin(),
    ...
    if (N->getOpcode() == McasmISD::AND && !N->hasAnyUseOfValue(1)) {
    ...
    auto mayPreventLoadFold = [&]() {
      return Mcasm::mayFoldLoad(N->getOperand(0), *Subtarget) &&
    ...
    if ((N->getOpcode() == ISD::ADD || N->getOpcode() == ISD::SUB) &&
      ...
      if (!ISD::isBuildVectorOfConstantSDNodes(
          ...
          Mcasm::isConstantSplat(N->getOperand(1), SplatVal) &&
          SplatVal.isOne()) {
        SDLoc DL(N);
    ...
    case McasmISD::VBROADCAST: {
      ...
      if (!Subtarget->hasBWI() && needBWI(VT)) {
        ...
        SDLoc dl(N);
        SDValue NarrowBCast =
            CurDAG->getNode(McasmISD::VBROADCAST, dl, NarrowVT, N->getOperand(0));
        SDValue Res =
    ...
    case McasmISD::VBROADCAST_LOAD: {
      ...
      if (!Subtarget->hasBWI() && needBWI(VT)) {
        ...
        SDValue NarrowBCast = CurDAG->getMemIntrinsicNode(
            McasmISD::VBROADCAST_LOAD, dl, VTs, Ops, MemNode->getMemoryVT(),
    ...
    case ISD::VSELECT: {
      ...
      if (Subtarget->hasVLX() && CurDAG->ComputeNumSignBits(N->getOperand(0)) ==
                                     EleVT.getSizeInBits()) {
        R = CurDAG->getNode(McasmISD::VPTERNLOG, SDLoc(N), N->getValueType(0),
                            N->getOperand(0), N->getOperand(1), N->getOperand(2),
                            CurDAG->getTargetConstant(0xCA, SDLoc(N), MVT::i8));
      } else {
        R = CurDAG->getNode(McasmISD::BLENDV, SDLoc(N), N->getValueType(0),
                            N->getOperand(0), N->getOperand(1),
                            N->getOperand(2));
      }
      --I;
    ...
    case ISD::STRICT_FP_TO_UINT: {
      // Replace vector fp_to_s/uint with their Mcasm specific equivalent so we
      ...
      case ISD::FP_ROUND:          NewOpc = McasmISD::VFPROUND;        break;
      case ISD::STRICT_FP_ROUND:   NewOpc = McasmISD::STRICT_VFPROUND; break;
      case ISD::STRICT_FP_TO_SINT: NewOpc = McasmISD::STRICT_CVTTP2SI; break;
      case ISD::FP_TO_SINT:        NewOpc = McasmISD::CVTTP2SI;        break;
      case ISD::STRICT_FP_TO_UINT: NewOpc = McasmISD::STRICT_CVTTP2UI; break;
      case ISD::FP_TO_UINT:        NewOpc = McasmISD::CVTTP2UI;        break;
      }
      SDValue Res;
    ...
    case ISD::SRA:
    case ISD::SRL: {
      // Replace vector shifts with their Mcasm specific equivalent so we don't
      ...
      switch (N->getOpcode()) {
      default: llvm_unreachable("Unexpected opcode!");
      case ISD::SHL: NewOpc = McasmISD::VSHLV; break;
      case ISD::SRA: NewOpc = McasmISD::VSRAV; break;
      case ISD::SRL: NewOpc = McasmISD::VSRLV; break;
      }
    ...
    case ISD::STRICT_FRINT: {
      // Replace fp rounding with their Mcasm specific equivalent so we don't
      // need 2 sets of patterns.
      unsigned Imm;
      switch (N->getOpcode()) {
      default: llvm_unreachable("Unexpected opcode!");
      ...
      SDValue Res;
      if (IsStrict)
        Res = CurDAG->getNode(McasmISD::STRICT_VRNDSCALE, dl,
                              {N->getValueType(0), MVT::Other},
                              {N->getOperand(0), N->getOperand(1),
                               CurDAG->getTargetConstant(Imm, dl, MVT::i32)});
      else
        Res = CurDAG->getNode(McasmISD::VRNDSCALE, dl, N->getValueType(0),
                              N->getOperand(0),
                              CurDAG->getTargetConstant(Imm, dl, MVT::i32));
      --I;
      CurDAG->ReplaceAllUsesWith(N, Res.getNode());
    ...
    case McasmISD::FXOR: {
      ...
      if (Subtarget->hasSSE2()) {
        ...
        switch (N->getOpcode()) {
        default: llvm_unreachable("Unexpected opcode!");
        case McasmISD::FANDN: Opc = McasmISD::ANDNP; break;
        case McasmISD::FAND:  Opc = ISD::AND;      break;
        case McasmISD::FOR:   Opc = ISD::OR;       break;
        case McasmISD::FXOR:  Opc = ISD::XOR;      break;
        }
    ...
    if (OptLevel != CodeGenOptLevel::None &&
        ...
        ((N->getOpcode() == McasmISD::CALL && !Subtarget->slowTwoMemOps()) ||
         (N->getOpcode() == McasmISD::TC_RETURN &&
          (Subtarget->is64Bit() ||
      ...
      ///       \      /
      ///       [CALL]
      bool HasCallSeq = N->getOpcode() == McasmISD::CALL;
    ...
    // late" legalization of these inline with the Mcasm isel pass.
    ...
    case ISD::FP_EXTEND:
    {
      ...
      const McasmTargetLowering *McasmLowering =
          static_cast<const McasmTargetLowering *>(TLI);
      bool SrcIsSSE = McasmLowering->isScalarFPTypeInSSEReg(SrcVT);
      bool DstIsSSE = McasmLowering->isScalarFPTypeInSSEReg(DstVT);
    ...
    case ISD::STRICT_FP_EXTEND:
    {
      ...
      if (!SrcIsSSE) {
        ...
        Store = CurDAG->getMemIntrinsicNode(McasmISD::FST, dl, VTs, Ops, MemVT,
      ...
      if (!DstIsSSE) {
        ...
        Result = CurDAG->getMemIntrinsicNode(
            McasmISD::FLD, dl, VTs, Ops, MemVT, MPI,
...
bool McasmDAGToDAGISel::tryOptimizeRem8Extend(SDNode *N) {
  unsigned Opc = N->getMachineOpcode();
  if (Opc != Mcasm::MOVZX32rr8 && Opc != Mcasm::MOVSX32rr8 &&
      Opc != Mcasm::MOVSX64rr8)
    return false;
  ...
  if (!N0.isMachineOpcode() ||
      ...
      N0.getConstantOperandVal(1) != Mcasm::sub_8bit)
    return false;

  // We're looking for either a movsx or movzx to match the original opcode.
  unsigned ExpectedOpc = Opc == Mcasm::MOVZX32rr8 ? Mcasm::MOVZX32rr8_NOREX
                                                : Mcasm::MOVSX32rr8_NOREX;
  SDValue N00 = N0.getOperand(0);
  if (!N00.isMachineOpcode() || N00.getMachineOpcode() != ExpectedOpc)
    return false;

  if (Opc == Mcasm::MOVSX64rr8) {
    // If we had a sign extend from 8 to 64 bits. We still need to go from 32
    // to 64.
    MachineSDNode *Extend = CurDAG->getMachineNode(Mcasm::MOVSX64rr32, SDLoc(N),
                                                   MVT::i64, N00);
...
void McasmDAGToDAGISel::PostprocessISelDAG() {
  ...
  while (Position != CurDAG->allnodes_begin()) {
    ...
    case Mcasm::CTEST8rr:
    case Mcasm::CTEST16rr:
    case Mcasm::CTEST32rr:
    case Mcasm::CTEST64rr: {
      ...
      default:
        ...
        CASE_ND(AND64rm) {
          if (And->hasAnyUseOfValue(1))
            continue;
          unsigned NewOpc;
          bool IsCTESTCC = Mcasm::isCTESTCC(Opc);
#define FROM_TO(A, B)                                                          \
  CASE_ND(A) NewOpc = IsCTESTCC ? Mcasm::C##B : Mcasm::B;                          \
  break;
          switch (And.getMachineOpcode()) {
...
#undef CASE_ND
    ...
    // register.
    case Mcasm::KORTESTBkk:
    case Mcasm::KORTESTWkk:
    case Mcasm::KORTESTDkk:
    case Mcasm::KORTESTQkk: {
...
#define FROM_TO(A, B)                                                          \
  case Mcasm::A:                                                                 \
    ...
    break;
      ...
      if (NewOpc == Mcasm::KTESTWkk && !Subtarget->hasDQI())
        continue;
#undef FROM_TO
    ...
    case TargetOpcode::SUBREG_TO_REG: {
      ...
      if (SubRegIdx != Mcasm::sub_xmm && SubRegIdx != Mcasm::sub_ymm)
        continue;
...
#undef CASE
    ...
    if ((TSFlags & McasmII::EncodingMask) != McasmII::VEX &&
        (TSFlags & McasmII::EncodingMask) != McasmII::EVEX &&
        (TSFlags & McasmII::EncodingMask) != McasmII::XOP)
      continue;
...
void McasmDAGToDAGISel::emitSpecialCodeForMain() {
...
void McasmDAGToDAGISel::emitFunctionEntryCode() {
...
bool McasmDAGToDAGISel::foldOffsetIntoAddress(uint64_t Offset,
                                            McasmISelAddressMode &AM) {
  ...
  if (Subtarget->is64Bit()) {
    if (Val != 0 &&
        !Mcasm::isOffsetSuitableForCodeModel(Val, M,
    ...
    // For 32-bit Mcasm, make sure the displacement still isn't close to the
    // expressible limit.
    return true;
  AM.Disp = Val;
  return false;
...
bool McasmDAGToDAGISel::matchLoadInAddress(LoadSDNode *N, McasmISelAddressMode &AM,
  ...
  // gs:0 (or fs:0 on Mcasm-64) contains its own address. However, for Mcasm-64 mode
  ...
  if (isNullConstant(Address) && AM.Segment.getNode() == nullptr &&
    ...
    case McasmAS::GS:
      AM.Segment = CurDAG->getRegister(Mcasm::GS, MVT::i16);
      return false;
    case McasmAS::FS:
      AM.Segment = CurDAG->getRegister(Mcasm::FS, MVT::i16);
      return false;
      // Address space McasmAS::SS is not handled here, because it is not used to
      // address TLS areas.
    }
  }
...
/// Try to match McasmISD::Wrapper and McasmISD::WrapperRIP nodes into an addressing
...
bool McasmDAGToDAGISel::matchWrapper(SDValue N, McasmISelAddressMode &AM) {
  ...
  bool IsRIPRel = N.getOpcode() == McasmISD::WrapperRIP;
  if (IsRIPRel) {
  ...
  McasmISelAddressMode Backup = AM;
  ...
  if (IsRIPRel)
    AM.setBaseReg(CurDAG->getRegister(Mcasm::RIP, MVT::i64));
...
bool McasmDAGToDAGISel::matchAddress(SDValue N, McasmISelAddressMode &AM) {
  ...
  if (Subtarget->isTarget64BitILP32() &&
      AM.BaseType == McasmISelAddressMode::RegBase &&
  ...
  if (TM.getCodeModel() != CodeModel::Large &&
      ...
      AM.Scale == 1 && AM.BaseType == McasmISelAddressMode::RegBase &&
      AM.Base_Reg.getNode() == nullptr && AM.IndexReg.getNode() == nullptr &&
      AM.SymbolFlags == McasmII::MO_NO_FLAG && AM.hasSymbolicDisplacement()) {
    ...
    AM.Base_Reg = CurDAG->getRegister(Mcasm::RIP, MVT::i64);
  }

  return false;
}

bool McasmDAGToDAGISel::matchAdd(SDValue &N, McasmISelAddressMode &AM,
                               unsigned Depth) {
  ...
  // the add.
  if (AM.BaseType == McasmISelAddressMode::RegBase &&
      !AM.Base_Reg.getNode() &&
      !AM.IndexReg.getNode()) {
    N = Handle.getValue();
...
SDValue McasmDAGToDAGISel::matchIndexRecursively(SDValue N,
                                               McasmISelAddressMode &AM,
                                               unsigned Depth) {
...
bool McasmDAGToDAGISel::matchAddressRecursively(SDValue N, McasmISelAddressMode &AM,
                                              unsigned Depth) {
  LLVM_DEBUG({
    dbgs() << "MatchAddress: ";
    AM.dump(CurDAG);
  ...
  case ISD::FrameIndex:
    if (AM.BaseType == McasmISelAddressMode::RegBase &&
      ...
      AM.BaseType = McasmISelAddressMode::FrameIndexBase;
  ...
  case ISD::SUB: {
    ...
    if ((AM.BaseType == McasmISelAddressMode::RegBase && AM.Base_Reg.getNode() &&
...
bool McasmDAGToDAGISel::matchAddressBase(SDValue N, McasmISelAddressMode &AM) {
...
bool McasmDAGToDAGISel::matchVectorAddressRecursively(SDValue N,
                                                    McasmISelAddressMode &AM,
                                                    unsigned Depth) {
  LLVM_DEBUG({
...
/// been handled.
bool McasmDAGToDAGISel::matchVectorAddress(SDValue N, McasmISelAddressMode &AM) {
  return matchVectorAddressRecursively(N, AM, 0);
}

bool McasmDAGToDAGISel::selectVectorAddr(MemSDNode *Parent, SDValue BasePtr,
  ...
  if (AddrSpace == McasmAS::GS)
    AM.Segment = CurDAG->getRegister(Mcasm::GS, MVT::i16);
  if (AddrSpace == McasmAS::FS)
    AM.Segment = CurDAG->getRegister(Mcasm::FS, MVT::i16);
  if (AddrSpace == McasmAS::SS)
    AM.Segment = CurDAG->getRegister(Mcasm::SS, MVT::i16);
...
bool McasmDAGToDAGISel::selectAddr(SDNode *Parent, SDValue N, SDValue &Base,
  ...
  if (Parent &&
      ...
      Parent->getOpcode() != McasmISD::TLSCALL && // Fixme
      Parent->getOpcode() != McasmISD::ENQCMD && // Fixme
      Parent->getOpcode() != McasmISD::ENQCMDS && // Fixme
      Parent->getOpcode() != McasmISD::EH_SJLJ_SETJMP && // setjmp
      Parent->getOpcode() != McasmISD::EH_SJLJ_LONGJMP) { // longjmp
    unsigned AddrSpace =
...
bool McasmDAGToDAGISel::selectMOV64Imm32(SDValue N, SDValue &Imm) {
  ...
  if (N->getOpcode() != McasmISD::Wrapper)
    return false;
...
bool McasmDAGToDAGISel::selectLEA64_Addr(SDValue N, SDValue &Base, SDValue &Scale,
                                       SDValue &Index, SDValue &Disp,
                                       SDValue &Segment) {
  ...
  unsigned SubReg;
  if (BaseType == MVT::i8)
    SubReg = Mcasm::sub_8bit;
  else if (BaseType == MVT::i16)
    SubReg = Mcasm::sub_16bit;
  else
    SubReg = Mcasm::sub_32bit;
  ...
  if (RN && RN->getReg() == 0)
    ...
    SDValue ImplDef = SDValue(CurDAG->getMachineNode(Mcasm::IMPLICIT_DEF, DL,
                                                     MVT::i64), 0);
...
bool McasmDAGToDAGISel::selectLEAAddr(SDValue N,
                                    SDValue &Base, SDValue &Scale,
                                    SDValue &Index, SDValue &Disp,
                                    SDValue &Segment) {
  McasmISelAddressMode AM;
  ...
  if (AM.BaseType == McasmISelAddressMode::RegBase && AM.Base_Reg.getNode())
    Complexity = 1;
  else if (AM.BaseType == McasmISelAddressMode::FrameIndexBase)
    Complexity = 4;
  ...
  if (AM.hasSymbolicDisplacement()) {
    // For Mcasm-64, always use LEA to materialize RIP-relative addresses.
    if (Subtarget->is64Bit())
      Complexity = 4;
    else
      Complexity += 2;
...
bool McasmDAGToDAGISel::selectTLSADDRAddr(SDValue N, SDValue &Base,
                                        SDValue &Scale, SDValue &Index,
                                        SDValue &Disp, SDValue &Segment) {
  ...
  if (Subtarget->is32Bit()) {
    AM.Scale = 1;
    AM.IndexReg = CurDAG->getRegister(Mcasm::EBX, MVT::i32);
  }
...
bool McasmDAGToDAGISel::selectRelocImm(SDValue N, SDValue &Op) {
...
bool McasmDAGToDAGISel::tryFoldLoad(SDNode *Root, SDNode *P, SDValue N,
                                  SDValue &Base, SDValue &Scale,
                                  SDValue &Index, SDValue &Disp,
                                  SDValue &Segment) {
...
bool McasmDAGToDAGISel::tryFoldBroadcast(SDNode *Root, SDNode *P, SDValue N,
                                       SDValue &Base, SDValue &Scale,
                                       SDValue &Index, SDValue &Disp,
                                       SDValue &Segment) {
  assert(Root && P && "Unknown root/parent nodes");
  if (N->getOpcode() != McasmISD::VBROADCAST_LOAD ||
      !IsProfitableToFold(N, P, Root) ||
      !IsLegalToFold(N, P, Root, OptLevel))
    return false;
...
/// Output instructions required to initialize the global base register,
/// if necessary.
SDNode *McasmDAGToDAGISel::getGlobalBaseReg() {
  Register GlobalBaseReg = getInstrInfo()->getGlobalBaseReg(MF);
  auto &DL = MF->getDataLayout();
  return CurDAG->getRegister(GlobalBaseReg, TLI->getPointerTy(DL)).getNode();
}

bool McasmDAGToDAGISel::isSExtAbsoluteSymbolRef(unsigned Width, SDNode *N) const {
  if (N->getOpcode() == ISD::TRUNCATE)
...
Mcasm::CondCode McasmDAGToDAGISel::getCondFromNode(SDNode *N) const {
  assert(N->isMachineOpcode() && "Unexpected node");
  unsigned Opc = N->getMachineOpcode();
  const MCInstrDesc &MCID = getInstrInfo()->get(Opc);
  int CondNo = Mcasm::getCondSrcNoFromDesc(MCID);
  if (CondNo < 0)
    return Mcasm::COND_INVALID;

  return static_cast<Mcasm::CondCode>(N->getConstantOperandVal(CondNo));
}

/// Test whether the given McasmISD::CMP node has any users that use a flag
/// other than ZF.
bool McasmDAGToDAGISel::onlyUsesZeroFlag(SDValue Flags) const {
  ...
  for (SDUse &Use : Flags->uses()) {
    ...
    if (User->getOpcode() != ISD::CopyToReg ||
        cast<RegisterSDNode>(User->getOperand(1))->getReg() != Mcasm::EFLAGS)
      return false;
    ...
    for (SDUse &FlagUse : User->uses()) {
      ...
      Mcasm::CondCode CC = getCondFromNode(FlagUse.getUser());

      switch (CC) {
      // Comparisons which only use the zero flag.
      case Mcasm::COND_E: case Mcasm::COND_NE:
        continue;
...
/// Test whether the given McasmISD::CMP node has any uses which require the SF
/// flag to be accurate.
bool McasmDAGToDAGISel::hasNoSignFlagUses(SDValue Flags) const {
  // Examine each user of the node.
  for (SDUse &Use : Flags->uses()) {
    // Only check things that use the flags.
    if (Use.getResNo() != Flags.getResNo())
    ...
    for (SDUse &FlagUse : User->uses()) {
      ...
      case Mcasm::COND_A: case Mcasm::COND_AE:
      case Mcasm::COND_B: case Mcasm::COND_BE:
      case Mcasm::COND_E: case Mcasm::COND_NE:
      case Mcasm::COND_O: case Mcasm::COND_NO:
      case Mcasm::COND_P: case Mcasm::COND_NP:
        continue;
      // Anything else: assume conservatively.
      default:
        return false;
...
static bool mayUseCarryFlag(Mcasm::CondCode CC) {
  switch (CC) {
  // Comparisons which don't examine the CF flag.
  case Mcasm::COND_O: case Mcasm::COND_NO:
  case Mcasm::COND_E: case Mcasm::COND_NE:
  case Mcasm::COND_S: case Mcasm::COND_NS:
  case Mcasm::COND_P: case Mcasm::COND_NP:
  case Mcasm::COND_L: case Mcasm::COND_GE:
  case Mcasm::COND_G: case Mcasm::COND_LE:
    return false;
  // Anything else: assume conservatively.
  default:
    return true;
...
/// CF flag to be accurate.
 bool McasmDAGToDAGISel::hasNoCarryFlagUses(SDValue Flags) const {
  // Examine each user of the node.
  for (SDUse &Use : Flags->uses()) {
    // Only check things that use the flags.
    if (Use.getResNo() != Flags.getResNo())
    ...
    if (UserOpc == ISD::CopyToReg) {
      // Only examine CopyToReg uses that copy to EFLAGS.
      if (cast<RegisterSDNode>(User->getOperand(1))->getReg() != Mcasm::EFLAGS)
        return false;
      // Examine each user of the CopyToReg use.
      for (SDUse &FlagUse : User->uses()) {
        // Only examine the Flag result.
    ...
    default:
      ...
      return false;
    case McasmISD::SETCC:       CCOpNo = 0; break;
    case McasmISD::SETCC_CARRY: CCOpNo = 0; break;
    case McasmISD::CMOV:        CCOpNo = 2; break;
    case McasmISD::BRCOND:      CCOpNo = 2; break;
    }

    Mcasm::CondCode CC = (Mcasm::CondCode)User->getConstantOperandVal(CCOpNo);
    if (mayUseCarryFlag(CC))
      return false;
  }
  return true;
...
// def DEC64m : RI<0xFF, MRM1m, (outs), (ins i64mem:$dst), "dec{q}\t$dst",
//  [(store (Mcasmadd_flag (loadi64 addr:$dst), -1), addr:$dst),
//   (transferrable EFLAGS)]>;
//
// Until then, we manually fold these and instruction select the operation
// here.
bool McasmDAGToDAGISel::foldLoadStoreIntoMemOperand(SDNode *Node) {
  ...
  case McasmISD::SUB:
    // Handle negate.
    if (IsNegate) {
      unsigned NewOpc = SelectOpcode(Mcasm::NEG64m, Mcasm::NEG32m, Mcasm::NEG16m,
                                     Mcasm::NEG8m);
  ...
  case McasmISD::ADD:
    ...
    if (!Subtarget->slowIncDec() || CurDAG->shouldOptForSize()) {
      ...
      if ((IsOne || IsNegOne) && hasNoCarryFlagUses(StoredVal.getValue(1))) {
        unsigned NewOpc =
          ((Opc == McasmISD::ADD) == IsOne)
              ? SelectOpcode(Mcasm::INC64m, Mcasm::INC32m, Mcasm::INC16m, Mcasm::INC8m)
              : SelectOpcode(Mcasm::DEC64m, Mcasm::DEC32m, Mcasm::DEC16m, Mcasm::DEC8m);
        const SDValue Ops[] = {Base, Scale, Index, Disp, Segment, InputChain};
        Result = CurDAG->getMachineNode(NewOpc, SDLoc(Node), MVT::i32,
                                        MVT::Other, Ops);
        break;
  ...
  case McasmISD::XOR: {
    auto SelectRegOpcode = [SelectOpcode](unsigned Opc) {
      switch (Opc) {
      case McasmISD::ADD:
        return SelectOpcode(Mcasm::ADD64mr, Mcasm::ADD32mr, Mcasm::ADD16mr,
                            Mcasm::ADD8mr);
      case McasmISD::ADC:
        return SelectOpcode(Mcasm::ADC64mr, Mcasm::ADC32mr, Mcasm::ADC16mr,
                            Mcasm::ADC8mr);
      case McasmISD::SUB:
        return SelectOpcode(Mcasm::SUB64mr, Mcasm::SUB32mr, Mcasm::SUB16mr,
                            Mcasm::SUB8mr);
      case McasmISD::SBB:
        return SelectOpcode(Mcasm::SBB64mr, Mcasm::SBB32mr, Mcasm::SBB16mr,
                            Mcasm::SBB8mr);
      case McasmISD::AND:
        return SelectOpcode(Mcasm::AND64mr, Mcasm::AND32mr, Mcasm::AND16mr,
                            Mcasm::AND8mr);
      case McasmISD::OR:
        return SelectOpcode(Mcasm::OR64mr, Mcasm::OR32mr, Mcasm::OR16mr, Mcasm::OR8mr);
      case McasmISD::XOR:
        return SelectOpcode(Mcasm::XOR64mr, Mcasm::XOR32mr, Mcasm::XOR16mr,
                            Mcasm::XOR8mr);
      default:
    ...
    auto SelectImmOpcode = [SelectOpcode](unsigned Opc) {
      switch (Opc) {
      case McasmISD::ADD:
        return SelectOpcode(Mcasm::ADD64mi32, Mcasm::ADD32mi, Mcasm::ADD16mi,
                            Mcasm::ADD8mi);
      case McasmISD::ADC:
        return SelectOpcode(Mcasm::ADC64mi32, Mcasm::ADC32mi, Mcasm::ADC16mi,
                            Mcasm::ADC8mi);
      case McasmISD::SUB:
        return SelectOpcode(Mcasm::SUB64mi32, Mcasm::SUB32mi, Mcasm::SUB16mi,
                            Mcasm::SUB8mi);
      case McasmISD::SBB:
        return SelectOpcode(Mcasm::SBB64mi32, Mcasm::SBB32mi, Mcasm::SBB16mi,
                            Mcasm::SBB8mi);
      case McasmISD::AND:
        return SelectOpcode(Mcasm::AND64mi32, Mcasm::AND32mi, Mcasm::AND16mi,
                            Mcasm::AND8mi);
      case McasmISD::OR:
        return SelectOpcode(Mcasm::OR64mi32, Mcasm::OR32mi, Mcasm::OR16mi,
                            Mcasm::OR8mi);
      case McasmISD::XOR:
        return SelectOpcode(Mcasm::XOR64mi32, Mcasm::XOR32mi, Mcasm::XOR16mi,
                            Mcasm::XOR8mi);
      default:
        llvm_unreachable("Invalid opcode!");
      }
    ...
    if (Opc == McasmISD::ADC || Opc == McasmISD::SBB) {
      SDValue CopyTo =
          CurDAG->getCopyToReg(InputChain, SDLoc(Node), Mcasm::EFLAGS,
...
//   d) x << (32 - y) >> (32 - y)
//   e) (1 << nbits) - 1
bool McasmDAGToDAGISel::matchBitExtract(SDNode *Node) {
  assert(
  ...
  SDValue SRIdxVal = CurDAG->getTargetConstant(Mcasm::sub_8bit, DL, MVT::i32);
  ...
  if (Subtarget->hasBMI2()) {
    ...
    SDValue Extract = CurDAG->getNode(McasmISD::BZHI, DL, NVT, X, NBits);
  ...
  SDValue Extract = CurDAG->getNode(McasmISD::BEXTR, DL, XVT, X, Control);
...
MachineSDNode *McasmDAGToDAGISel::matchBEXTRFromAndImm(SDNode *Node) {
  MVT NVT = Node->getSimpleValueType(0);
  SDLoc dl(Node);
...
#define GET_EGPR_IF_ENABLED(OPC) (Subtarget->hasEGPR() ? OPC##_EVEX : OPC)
  if (!PreferBEXTR) {
    ...
    ROpc = NVT == MVT::i64 ? GET_EGPR_IF_ENABLED(Mcasm::BZHI64rr)
                           : GET_EGPR_IF_ENABLED(Mcasm::BZHI32rr);
    MOpc = NVT == MVT::i64 ? GET_EGPR_IF_ENABLED(Mcasm::BZHI64rm)
                           : GET_EGPR_IF_ENABLED(Mcasm::BZHI32rm);
    unsigned NewOpc = NVT == MVT::i64 ? Mcasm::MOV32ri64 : Mcasm::MOV32ri;
    ...
    if (Subtarget->hasTBM()) {
      ROpc = NVT == MVT::i64 ? Mcasm::BEXTRI64ri : Mcasm::BEXTRI32ri;
      MOpc = NVT == MVT::i64 ? Mcasm::BEXTRI64mi : Mcasm::BEXTRI32mi;
    } else {
      assert(Subtarget->hasBMI() && "We must have BMI1's BEXTR then.");
      // BMI requires the immediate to placed in a register.
      ROpc = NVT == MVT::i64 ? GET_EGPR_IF_ENABLED(Mcasm::BEXTR64rr)
                             : GET_EGPR_IF_ENABLED(Mcasm::BEXTR32rr);
      MOpc = NVT == MVT::i64 ? GET_EGPR_IF_ENABLED(Mcasm::BEXTR64rm)
                             : GET_EGPR_IF_ENABLED(Mcasm::BEXTR32rm);
      unsigned NewOpc = NVT == MVT::i64 ? Mcasm::MOV32ri64 : Mcasm::MOV32ri;
      Control = SDValue(CurDAG->getMachineNode(NewOpc, dl, NVT, Control), 0);
    }
  }
  ...
  if (!PreferBEXTR) {
    ...
    unsigned NewOpc = NVT == MVT::i64 ? GET_ND_IF_ENABLED(Mcasm::SHR64ri)
                                      : GET_ND_IF_ENABLED(Mcasm::SHR32ri);
    NewNode =
...
MachineSDNode *McasmDAGToDAGISel::emitPCMPISTR(unsigned ROpc, unsigned MOpc,
...
MachineSDNode *McasmDAGToDAGISel::emitPCMPESTR(unsigned ROpc, unsigned MOpc,
                                             bool MayFoldLoad, const SDLoc &dl,
                                             MVT VT, SDNode *Node,
                                             SDValue &InGlue) {
  SDValue N0 = Node->getOperand(0);
...
bool McasmDAGToDAGISel::tryShiftAmountMod(SDNode *N) {
  ...
  // This function is called after McasmDAGToDAGISel::matchBitExtract(),
...
bool McasmDAGToDAGISel::tryShrinkShlLogicImm(SDNode *N) {
...
bool McasmDAGToDAGISel::matchVPTERNLOG(SDNode *Root, SDNode *ParentA,
  ...
  auto tryFoldLoadOrBCast =
      [this](SDNode *Root, SDNode *P, SDValue &L, SDValue &Base, SDValue &Scale,
        ...
        if (L.getOpcode() != McasmISD::VBROADCAST_LOAD)
          return false;
  ...
  if (FoldedLoad) {
    ...
    unsigned Opc;
    if (C.getOpcode() == McasmISD::VBROADCAST_LOAD) {
      ...
      if (NVT.is128BitVector())
        Opc = UseD ? Mcasm::VPTERNLOGDZ128rmbi : Mcasm::VPTERNLOGQZ128rmbi;
      else if (NVT.is256BitVector())
        Opc = UseD ? Mcasm::VPTERNLOGDZ256rmbi : Mcasm::VPTERNLOGQZ256rmbi;
      else if (NVT.is512BitVector())
        Opc = UseD ? Mcasm::VPTERNLOGDZrmbi : Mcasm::VPTERNLOGQZrmbi;
      else
        llvm_unreachable("Unexpected vector size!");
    } else {
      bool UseD = NVT.getVectorElementType() == MVT::i32;
      if (NVT.is128BitVector())
        Opc = UseD ? Mcasm::VPTERNLOGDZ128rmi : Mcasm::VPTERNLOGQZ128rmi;
      else if (NVT.is256BitVector())
        Opc = UseD ? Mcasm::VPTERNLOGDZ256rmi : Mcasm::VPTERNLOGQZ256rmi;
      else if (NVT.is512BitVector())
        Opc = UseD ? Mcasm::VPTERNLOGDZrmi : Mcasm::VPTERNLOGQZrmi;
      else
        llvm_unreachable("Unexpected vector size!");
    }
    ...
    unsigned Opc;
    if (NVT.is128BitVector())
      Opc = UseD ? Mcasm::VPTERNLOGDZ128rri : Mcasm::VPTERNLOGQZ128rri;
    else if (NVT.is256BitVector())
      Opc = UseD ? Mcasm::VPTERNLOGDZ256rri : Mcasm::VPTERNLOGQZ256rri;
    else if (NVT.is512BitVector())
      Opc = UseD ? Mcasm::VPTERNLOGDZrri : Mcasm::VPTERNLOGQZrri;
    else
      llvm_unreachable("Unexpected vector size!");
...
bool McasmDAGToDAGISel::tryVPTERNLOG(SDNode *N) {
  MVT NVT = N->getSimpleValueType(0);
  ...
  case McasmISD::ANDNP: Imm = ~(TernlogMagicB) & TernlogMagicC; break;
  }
...
bool McasmDAGToDAGISel::shrinkAndImmediate(SDNode *And) {
...
case MVT::VT: \
  if (Masked) \
    return IsTestN ? Mcasm::VPTESTNM##SUFFIX##k: Mcasm::VPTESTM##SUFFIX##k; \
  return IsTestN ? Mcasm::VPTESTNM##SUFFIX : Mcasm::VPTESTM##SUFFIX;
...
bool McasmDAGToDAGISel::tryVPTESTM(SDNode *Root, SDValue Setcc,
                                 SDValue InMask) {
  ...
  bool FoldedBCast = FoldedLoad && Src1.getOpcode() == McasmISD::VBROADCAST_LOAD;
  ...
  if (Widen) {
    ...
    unsigned SubReg = CmpVT.is128BitVector() ? Mcasm::sub_xmm : Mcasm::sub_ymm;
...
// into vpternlog.
bool McasmDAGToDAGISel::tryMatchBitSelect(SDNode *N) {
  ...
  SDValue Ternlog = CurDAG->getNode(McasmISD::VPTERNLOG, dl, NVT, A, B, C, Imm);
...
void McasmDAGToDAGISel::Select(SDNode *Node) {
  MVT NVT = Node->getSimpleValueType(0);
  ...
  case ISD::INTRINSIC_W_CHAIN: {
    ...
    case Intrinsic::x86_encodekey256: {
      ...
      case Intrinsic::x86_encodekey128:
        Opcode = Mcasm::ENCODEKEY128;
        break;
      case Intrinsic::x86_encodekey256:
        Opcode = Mcasm::ENCODEKEY256;
        break;
      }

      SDValue Chain = Node->getOperand(0);
      Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::XMM0, Node->getOperand(3),
                                   SDValue());
      if (Opcode == Mcasm::ENCODEKEY256)
        Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::XMM1, Node->getOperand(4),
                                     Chain.getValue(1));
    ...
    case Intrinsic::x86_tileloaddt164_internal: {
      if (!Subtarget->hasAMXTILE())
        break;
      auto *MFI =
          CurDAG->getMachineFunction().getInfo<McasmMachineFunctionInfo>();
      ...
      case Intrinsic::x86_tileloaddrs64_internal:
        Opc = Mcasm::PTILELOADDRSV;
        break;
      case Intrinsic::x86_tileloaddrst164_internal:
        Opc = Mcasm::PTILELOADDRST1V;
        break;
      case Intrinsic::x86_tileloadd64_internal:
        Opc = Mcasm::PTILELOADDV;
        break;
      case Intrinsic::x86_tileloaddt164_internal:
        Opc = Mcasm::PTILELOADDT1V;
        break;
      }
  ...
  case ISD::INTRINSIC_VOID: {
    ...
    case Intrinsic::x86_clzero: {
      ...
      case Intrinsic::x86_sse3_monitor:
        if (!Subtarget->hasSSE3())
          break;
        Opc = Use64BitPtr ? Mcasm::MONITOR64rrr : Mcasm::MONITOR32rrr;
        break;
      case Intrinsic::x86_monitorx:
        if (!Subtarget->hasMWAITX())
          break;
        Opc = Use64BitPtr ? Mcasm::MONITORX64rrr : Mcasm::MONITORX32rrr;
        break;
      case Intrinsic::x86_clzero:
        if (!Subtarget->hasCLZERO())
          break;
        Opc = Use64BitPtr ? Mcasm::CLZERO64r : Mcasm::CLZERO32r;
        break;
      }

      if (Opc) {
        unsigned PtrReg = Use64BitPtr ? Mcasm::RAX : Mcasm::EAX;
        ...
        if (IntNo == Intrinsic::x86_sse3_monitor ||
          ...
          Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::ECX, Node->getOperand(3),
                                       InGlue);
          InGlue = Chain.getValue(1);
          Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::EDX, Node->getOperand(4),
                                       InGlue);
          InGlue = Chain.getValue(1);
        }
    ...
    case Intrinsic::x86_tilestored64_internal: {
      auto *MFI =
          CurDAG->getMachineFunction().getInfo<McasmMachineFunctionInfo>();
      MFI->setAMXProgModel(AMXProgModelEnum::ManagedRA);
      unsigned Opc = Mcasm::PTILESTOREDV;
    ...
    case Intrinsic::x86_tilestored64: {
      ...
      switch (IntNo) {
      default: llvm_unreachable("Unexpected intrinsic!");
      case Intrinsic::x86_tileloadd64:   Opc = Mcasm::PTILELOADD; break;
      case Intrinsic::x86_tileloaddrs64:
        Opc = Mcasm::PTILELOADDRS;
        break;
      case Intrinsic::x86_tileloaddt164: Opc = Mcasm::PTILELOADDT1; break;
      case Intrinsic::x86_tileloaddrst164:
        Opc = Mcasm::PTILELOADDRST1;
        break;
      case Intrinsic::x86_tilestored64:  Opc = Mcasm::PTILESTORED; break;
      }
  ...
  case ISD::SUB: {
    ...
    case MVT::i8:
      ...
      default: llvm_unreachable("Unexpected opcode!");
      case ISD::ADD:
        ROpc = GET_ND_IF_ENABLED(Mcasm::ADD8rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::ADD8rm);
        break;
      case ISD::SUB:
        ROpc = GET_ND_IF_ENABLED(Mcasm::SUB8rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::SUB8rm);
        break;
      case ISD::AND:
        ROpc = GET_ND_IF_ENABLED(Mcasm::AND8rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::AND8rm);
        break;
      case ISD::OR:
        ROpc = GET_ND_IF_ENABLED(Mcasm::OR8rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::OR8rm);
        break;
      case ISD::XOR:
        ROpc = GET_ND_IF_ENABLED(Mcasm::XOR8rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::XOR8rm);
        break;
      }
      break;
    case MVT::i16:
      ...
      default: llvm_unreachable("Unexpected opcode!");
      case ISD::ADD:
        ROpc = GET_ND_IF_ENABLED(Mcasm::ADD16rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::ADD16rm);
        break;
      case ISD::SUB:
        ROpc = GET_ND_IF_ENABLED(Mcasm::SUB16rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::SUB16rm);
        break;
      case ISD::AND:
        ROpc = GET_ND_IF_ENABLED(Mcasm::AND16rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::AND16rm);
        break;
      case ISD::OR:
        ROpc = GET_ND_IF_ENABLED(Mcasm::OR16rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::OR16rm);
        break;
      case ISD::XOR:
        ROpc = GET_ND_IF_ENABLED(Mcasm::XOR16rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::XOR16rm);
        break;
      }
      break;
    case MVT::i32:
      ...
      default: llvm_unreachable("Unexpected opcode!");
      case ISD::ADD:
        ROpc = GET_ND_IF_ENABLED(Mcasm::ADD32rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::ADD32rm);
        break;
      case ISD::SUB:
        ROpc = GET_ND_IF_ENABLED(Mcasm::SUB32rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::SUB32rm);
        break;
      case ISD::AND:
        ROpc = GET_ND_IF_ENABLED(Mcasm::AND32rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::AND32rm);
        break;
      case ISD::OR:
        ROpc = GET_ND_IF_ENABLED(Mcasm::OR32rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::OR32rm);
        break;
      case ISD::XOR:
        ROpc = GET_ND_IF_ENABLED(Mcasm::XOR32rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::XOR32rm);
        break;
      }
      break;
    case MVT::i64:
      ...
      default: llvm_unreachable("Unexpected opcode!");
      case ISD::ADD:
        ROpc = GET_ND_IF_ENABLED(Mcasm::ADD64rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::ADD64rm);
        break;
      case ISD::SUB:
        ROpc = GET_ND_IF_ENABLED(Mcasm::SUB64rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::SUB64rm);
        break;
      case ISD::AND:
        ROpc = GET_ND_IF_ENABLED(Mcasm::AND64rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::AND64rm);
        break;
      case ISD::OR:
        ROpc = GET_ND_IF_ENABLED(Mcasm::OR64rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::OR64rm);
        break;
      case ISD::XOR:
        ROpc = GET_ND_IF_ENABLED(Mcasm::XOR64rr);
        MOpc = GET_ND_IF_ENABLED(Mcasm::XOR64rm);
        break;
      }
      break;
    }
  ...
  case McasmISD::UMUL: {
    ...
    case MVT::i8:
      LoReg = Mcasm::AL;
      ROpc = Opcode == McasmISD::SMUL ? Mcasm::IMUL8r : Mcasm::MUL8r;
      MOpc = Opcode == McasmISD::SMUL ? Mcasm::IMUL8m : Mcasm::MUL8m;
      break;
    case MVT::i16:
      LoReg = Mcasm::AX;
      ROpc = Mcasm::MUL16r;
  ...
  case ISD::UMUL_LOHI: {
    ...
    default: llvm_unreachable("Unsupported VT!");
    case MVT::i32:
      Opc = UseMULXHi  ? Mcasm::MULX32Hrr
            : UseMULX  ? GET_EGPR_IF_ENABLED(Mcasm::MULX32rr)
            : IsSigned ? Mcasm::IMUL32r
                       : Mcasm::MUL32r;
      MOpc = UseMULXHi  ? Mcasm::MULX32Hrm
             : UseMULX  ? GET_EGPR_IF_ENABLED(Mcasm::MULX32rm)
             : IsSigned ? Mcasm::IMUL32m
                        : Mcasm::MUL32m;
      LoReg = UseMULX ? Mcasm::EDX : Mcasm::EAX;
      HiReg = Mcasm::EDX;
      break;
    case MVT::i64:
      Opc = UseMULXHi  ? Mcasm::MULX64Hrr
            : UseMULX  ? GET_EGPR_IF_ENABLED(Mcasm::MULX64rr)
            : IsSigned ? Mcasm::IMUL64r
                       : Mcasm::MUL64r;
      MOpc = UseMULXHi  ? Mcasm::MULX64Hrm
             : UseMULX  ? GET_EGPR_IF_ENABLED(Mcasm::MULX64rm)
             : IsSigned ? Mcasm::IMUL64m
                        : Mcasm::MUL64m;
      LoReg = UseMULX ? Mcasm::RDX : Mcasm::RAX;
      HiReg = Mcasm::RDX;
      break;
    }
  ...
  case ISD::UDIVREM: {
    ...
    if (!isSigned) {
      switch (NVT.SimpleTy) {
      default: llvm_unreachable("Unsupported VT!");
      case MVT::i8:  ROpc = Mcasm::DIV8r;  MOpc = Mcasm::DIV8m;  break;
      case MVT::i16: ROpc = Mcasm::DIV16r; MOpc = Mcasm::DIV16m; break;
      case MVT::i32: ROpc = Mcasm::DIV32r; MOpc = Mcasm::DIV32m; break;
      case MVT::i64: ROpc = Mcasm::DIV64r; MOpc = Mcasm::DIV64m; break;
      }
    } else {
      switch (NVT.SimpleTy) {
      default: llvm_unreachable("Unsupported VT!");
      case MVT::i8:  ROpc = Mcasm::IDIV8r;  MOpc = Mcasm::IDIV8m;  break;
      case MVT::i16: ROpc = Mcasm::IDIV16r; MOpc = Mcasm::IDIV16m; break;
      case MVT::i32: ROpc = Mcasm::IDIV32r; MOpc = Mcasm::IDIV32m; break;
      case MVT::i64: ROpc = Mcasm::IDIV64r; MOpc = Mcasm::IDIV64m; break;
      }
    }
    ...
    default: llvm_unreachable("Unsupported VT!");
    case MVT::i8:
      LoReg = Mcasm::AL;  ClrReg = HiReg = Mcasm::AH;
      SExtOpcode = 0; // Not used.
      break;
    case MVT::i16:
      LoReg = Mcasm::AX;  HiReg = Mcasm::DX;
      ClrReg = Mcasm::DX;
      SExtOpcode = Mcasm::CWD;
      break;
    case MVT::i32:
      LoReg = Mcasm::EAX; ClrReg = HiReg = Mcasm::EDX;
      SExtOpcode = Mcasm::CDQ;
      break;
    case MVT::i64:
      LoReg = Mcasm::RAX; ClrReg = HiReg = Mcasm::RDX;
      SExtOpcode = Mcasm::CQO;
      break;
    }
    ...
    if (NVT == MVT::i8) {
      ...
      if (tryFoldLoad(Node, N0, Tmp0, Tmp1, Tmp2, Tmp3, Tmp4)) {
        ...
        unsigned Opc = (isSigned && !signBitIsZero) ? Mcasm::MOVSX16rm8
                                                    : Mcasm::MOVZX16rm8;
        ...
        unsigned Opc = (isSigned && !signBitIsZero) ? Mcasm::MOVSX16rr8
                                                    : Mcasm::MOVZX16rr8;
        Move = CurDAG->getMachineNode(Opc, dl, MVT::i16, N0);
        Chain = CurDAG->getEntryNode();
      }
      Chain  = CurDAG->getCopyToReg(Chain, dl, Mcasm::AX, SDValue(Move, 0),
                                    SDValue());
      InGlue = Chain.getValue(1);
    } else {
      InGlue =
        ...
        case MVT::i16:
          ClrNode =
              SDValue(CurDAG->getMachineNode(
                          ...
                          CurDAG->getTargetConstant(Mcasm::sub_16bit, dl,
                                                    MVT::i32)),
                      0);
          break;
        case MVT::i32:
        ...
        case MVT::i64:
          ClrNode =
              SDValue(CurDAG->getMachineNode(
                          ...
                          CurDAG->getTargetConstant(Mcasm::sub_32bit, dl,
                                                    MVT::i32)),
                      0);
          break;
        default:
    ...
    if (HiReg == Mcasm::AH && !SDValue(Node, 1).use_empty()) {
      SDValue AHCopy = CurDAG->getRegister(Mcasm::AH, MVT::i8);
      unsigned AHExtOpcode =
          isSigned ? Mcasm::MOVSX32rr8_NOREX : Mcasm::MOVZX32rr8_NOREX;
      ...
      Result =
          CurDAG->getTargetExtractSubreg(Mcasm::sub_8bit, dl, MVT::i8, Result);
  ...
  case McasmISD::FCMP:
  case McasmISD::STRICT_FCMP:
  case McasmISD::STRICT_FCMPS: {
    bool IsStrictCmp = Node->getOpcode() == McasmISD::STRICT_FCMP ||
                       Node->getOpcode() == McasmISD::STRICT_FCMPS;
    ...
    bool IsSignaling = Node->getOpcode() == McasmISD::STRICT_FCMPS;

    unsigned Opc;
    switch (CmpVT.SimpleTy) {
    default: llvm_unreachable("Unexpected type!");
    case MVT::f32:
      Opc = IsSignaling ? Mcasm::COM_Fpr32 : Mcasm::UCOM_Fpr32;
      break;
    case MVT::f64:
      Opc = IsSignaling ? Mcasm::COM_Fpr64 : Mcasm::UCOM_Fpr64;
      break;
    case MVT::f80:
      Opc = IsSignaling ? Mcasm::COM_Fpr80 : Mcasm::UCOM_Fpr80;
      break;
    }
    ...
    // Move FPSW to AX.
    SDValue FNSTSW =
        SDValue(CurDAG->getMachineNode(Mcasm::FNSTSW16r, dl, MVT::i16, Glue), 0);

    // Extract upper 8-bits of AX.
    SDValue Extract =
        CurDAG->getTargetExtractSubreg(Mcasm::sub_8bit_hi, dl, MVT::i8, FNSTSW);

    // Move AH into flags.
    // Some 64-bit targets lack SAHF support, but they do support FCOMI.
    assert(Subtarget->canUseLAHFSAHF() &&
           "Target doesn't support SAHF or FCOMI?");
    SDValue AH = CurDAG->getCopyToReg(Chain, dl, Mcasm::AH, Extract, SDValue());
    Chain = AH;
    SDValue SAHF = SDValue(
        CurDAG->getMachineNode(Mcasm::SAHF, dl, MVT::i32, AH.getValue(1)), 0);
  ...
  case McasmISD::CMP: {
    ...
    if (N0.getOpcode() == ISD::AND && N0.hasOneUse()) {
      if (MachineSDNode *NewNode = matchBEXTRFromAndImm(N0.getNode())) {
        unsigned TestOpc = CmpVT == MVT::i64 ? Mcasm::TEST64rr
                                             : Mcasm::TEST32rr;
    ...
    // Look for (Mcasmcmp (and $op, $imm), 0) and see if we can convert it to
    ...
    if (N0.getOpcode() == ISD::AND && N0.getNode()->hasOneUse() &&
      ...
      if (CmpVT == MVT::i64 && !isInt<8>(Mask) && isShiftedMask_64(Mask) &&
        ...
        if (LeadingZeros == 0 && SavesBytes) {
          ...
          ShiftOpcode = GET_ND_IF_ENABLED(Mcasm::SHR64ri);
          ShiftAmt = TrailingZeros;
          SubRegIdx = 0;
          TestOpcode = Mcasm::TEST64rr;
          ...
          // This emits a redundant TEST which is subsequently eliminated.
          ShiftOpcode = GET_ND_IF_ENABLED(Mcasm::SHL64ri);
          ...
          if (PopCount == 8) {
            ShiftOpcode = GET_ND_IF_ENABLED(Mcasm::SHR64ri);
            ShiftAmt = TrailingZeros;
            SubRegIdx = Mcasm::sub_8bit;
            SubRegVT = MVT::i8;
            TestOpcode = Mcasm::TEST8rr;
          } else if (PopCount == 16) {
            ShiftOpcode = GET_ND_IF_ENABLED(Mcasm::SHR64ri);
            ShiftAmt = TrailingZeros;
            SubRegIdx = Mcasm::sub_16bit;
            SubRegVT = MVT::i16;
            TestOpcode = Mcasm::TEST16rr;
          } else if (PopCount == 32) {
            ShiftOpcode = GET_ND_IF_ENABLED(Mcasm::SHR64ri);
            ShiftAmt = TrailingZeros;
            SubRegIdx = Mcasm::sub_32bit;
            SubRegVT = MVT::i32;
            TestOpcode = Mcasm::TEST32rr;
          }
        }
      ...
      if (tryFoldLoad(Node, N0.getNode(), Reg, Tmp0, Tmp1, Tmp2, Tmp3, Tmp4)) {
        if (auto *LoadN = dyn_cast<LoadSDNode>(N0.getOperand(0).getNode())) {
          if (!LoadN->isSimple()) {
            ...
            if ((MOpc == Mcasm::TEST8mi && NumVolBits != 8) ||
                (MOpc == Mcasm::TEST16mi && NumVolBits != 16) ||
                (MOpc == Mcasm::TEST32mi && NumVolBits != 32))
              break;
          }
        }
  ...
  case McasmISD::PCMPISTR: {
    ...
    if (NeedMask) {
      unsigned ROpc =
          Subtarget->hasAVX() ? Mcasm::VPCMPISTRMrri : Mcasm::PCMPISTRMrri;
      unsigned MOpc =
          Subtarget->hasAVX() ? Mcasm::VPCMPISTRMrmi : Mcasm::PCMPISTRMrmi;
    ...
    if (NeedIndex || !NeedMask) {
      unsigned ROpc =
          Subtarget->hasAVX() ? Mcasm::VPCMPISTRIrri : Mcasm::PCMPISTRIrri;
      unsigned MOpc =
          Subtarget->hasAVX() ? Mcasm::VPCMPISTRIrmi : Mcasm::PCMPISTRIrmi;
  ...
  case McasmISD::PCMPESTR: {
    if (!Subtarget->hasSSE42())
      break;

    // Copy the two implicit register inputs.
    SDValue InGlue = CurDAG->getCopyToReg(CurDAG->getEntryNode(), dl, Mcasm::EAX,
                                          Node->getOperand(1),
                                          SDValue()).getValue(1);
    InGlue = CurDAG->getCopyToReg(CurDAG->getEntryNode(), dl, Mcasm::EDX,
    ...
    if (NeedMask) {
      unsigned ROpc =
          Subtarget->hasAVX() ? Mcasm::VPCMPESTRMrri : Mcasm::PCMPESTRMrri;
      unsigned MOpc =
          Subtarget->hasAVX() ? Mcasm::VPCMPESTRMrmi : Mcasm::PCMPESTRMrmi;
      CNode =
    ...
    if (NeedIndex || !NeedMask) {
      unsigned ROpc =
          Subtarget->hasAVX() ? Mcasm::VPCMPESTRIrri : Mcasm::PCMPESTRIrri;
      unsigned MOpc =
          Subtarget->hasAVX() ? Mcasm::VPCMPESTRIrmi : Mcasm::PCMPESTRIrmi;
  ...
  case McasmISD::SETCC_CARRY: {
    ...
    if (Subtarget->hasSBBDepBreaking()) {
      ...
      // Create a 64-bit instruction if the result is 64-bits otherwise use the
      // 32-bit version.
      unsigned Opc = VT == MVT::i64 ? Mcasm::SETB_C64r : Mcasm::SETB_C32r;
    ...
    if (VT == MVT::i8 || VT == MVT::i16) {
      int SubIndex = VT == MVT::i16 ? Mcasm::sub_16bit : Mcasm::sub_8bit;
  ...
  case McasmISD::MGATHER: {
    auto *Mgt = cast<McasmMaskedGatherSDNode>(Node);
    ...
    if (AVX512Gather) {
      if (IndexVT == MVT::v4i32 && NumElts == 4 && EltSize == 32)
        Opc = IsFP ? Mcasm::VGATHERDPSZ128rm : Mcasm::VPGATHERDDZ128rm;
      else if (IndexVT == MVT::v8i32 && NumElts == 8 && EltSize == 32)
        Opc = IsFP ? Mcasm::VGATHERDPSZ256rm : Mcasm::VPGATHERDDZ256rm;
      else if (IndexVT == MVT::v16i32 && NumElts == 16 && EltSize == 32)
        Opc = IsFP ? Mcasm::VGATHERDPSZrm : Mcasm::VPGATHERDDZrm;
      else if (IndexVT == MVT::v4i32 && NumElts == 2 && EltSize == 64)
        Opc = IsFP ? Mcasm::VGATHERDPDZ128rm : Mcasm::VPGATHERDQZ128rm;
      else if (IndexVT == MVT::v4i32 && NumElts == 4 && EltSize == 64)
        Opc = IsFP ? Mcasm::VGATHERDPDZ256rm : Mcasm::VPGATHERDQZ256rm;
      else if (IndexVT == MVT::v8i32 && NumElts == 8 && EltSize == 64)
        Opc = IsFP ? Mcasm::VGATHERDPDZrm : Mcasm::VPGATHERDQZrm;
      else if (IndexVT == MVT::v2i64 && NumElts == 4 && EltSize == 32)
        Opc = IsFP ? Mcasm::VGATHERQPSZ128rm : Mcasm::VPGATHERQDZ128rm;
      else if (IndexVT == MVT::v4i64 && NumElts == 4 && EltSize == 32)
        Opc = IsFP ? Mcasm::VGATHERQPSZ256rm : Mcasm::VPGATHERQDZ256rm;
      else if (IndexVT == MVT::v8i64 && NumElts == 8 && EltSize == 32)
        Opc = IsFP ? Mcasm::VGATHERQPSZrm : Mcasm::VPGATHERQDZrm;
      else if (IndexVT == MVT::v2i64 && NumElts == 2 && EltSize == 64)
        Opc = IsFP ? Mcasm::VGATHERQPDZ128rm : Mcasm::VPGATHERQQZ128rm;
      else if (IndexVT == MVT::v4i64 && NumElts == 4 && EltSize == 64)
        Opc = IsFP ? Mcasm::VGATHERQPDZ256rm : Mcasm::VPGATHERQQZ256rm;
      else if (IndexVT == MVT::v8i64 && NumElts == 8 && EltSize == 64)
        Opc = IsFP ? Mcasm::VGATHERQPDZrm : Mcasm::VPGATHERQQZrm;
    } else {
      assert(EVT(MaskVT) == EVT(ValueVT).changeVectorElementTypeToInteger() &&
             "Unexpected mask VT!");
      if (IndexVT == MVT::v4i32 && NumElts == 4 && EltSize == 32)
        Opc = IsFP ? Mcasm::VGATHERDPSrm : Mcasm::VPGATHERDDrm;
      else if (IndexVT == MVT::v8i32 && NumElts == 8 && EltSize == 32)
        Opc = IsFP ? Mcasm::VGATHERDPSYrm : Mcasm::VPGATHERDDYrm;
      else if (IndexVT == MVT::v4i32 && NumElts == 2 && EltSize == 64)
        Opc = IsFP ? Mcasm::VGATHERDPDrm : Mcasm::VPGATHERDQrm;
      else if (IndexVT == MVT::v4i32 && NumElts == 4 && EltSize == 64)
        Opc = IsFP ? Mcasm::VGATHERDPDYrm : Mcasm::VPGATHERDQYrm;
      else if (IndexVT == MVT::v2i64 && NumElts == 4 && EltSize == 32)
        Opc = IsFP ? Mcasm::VGATHERQPSrm : Mcasm::VPGATHERQDrm;
      else if (IndexVT == MVT::v4i64 && NumElts == 4 && EltSize == 32)
        Opc = IsFP ? Mcasm::VGATHERQPSYrm : Mcasm::VPGATHERQDYrm;
      else if (IndexVT == MVT::v2i64 && NumElts == 2 && EltSize == 64)
        Opc = IsFP ? Mcasm::VGATHERQPDrm : Mcasm::VPGATHERQQrm;
      else if (IndexVT == MVT::v4i64 && NumElts == 4 && EltSize == 64)
        Opc = IsFP ? Mcasm::VGATHERQPDYrm : Mcasm::VPGATHERQQYrm;
    }
  ...
  case McasmISD::MSCATTER: {
    auto *Sc = cast<McasmMaskedScatterSDNode>(Node);
    ...
    unsigned Opc;
    if (IndexVT == MVT::v4i32 && NumElts == 4 && EltSize == 32)
      Opc = IsFP ? Mcasm::VSCATTERDPSZ128mr : Mcasm::VPSCATTERDDZ128mr;
    else if (IndexVT == MVT::v8i32 && NumElts == 8 && EltSize == 32)
      Opc = IsFP ? Mcasm::VSCATTERDPSZ256mr : Mcasm::VPSCATTERDDZ256mr;
    else if (IndexVT == MVT::v16i32 && NumElts == 16 && EltSize == 32)
      Opc = IsFP ? Mcasm::VSCATTERDPSZmr : Mcasm::VPSCATTERDDZmr;
    else if (IndexVT == MVT::v4i32 && NumElts == 2 && EltSize == 64)
      Opc = IsFP ? Mcasm::VSCATTERDPDZ128mr : Mcasm::VPSCATTERDQZ128mr;
    else if (IndexVT == MVT::v4i32 && NumElts == 4 && EltSize == 64)
      Opc = IsFP ? Mcasm::VSCATTERDPDZ256mr : Mcasm::VPSCATTERDQZ256mr;
    else if (IndexVT == MVT::v8i32 && NumElts == 8 && EltSize == 64)
      Opc = IsFP ? Mcasm::VSCATTERDPDZmr : Mcasm::VPSCATTERDQZmr;
    else if (IndexVT == MVT::v2i64 && NumElts == 4 && EltSize == 32)
      Opc = IsFP ? Mcasm::VSCATTERQPSZ128mr : Mcasm::VPSCATTERQDZ128mr;
    else if (IndexVT == MVT::v4i64 && NumElts == 4 && EltSize == 32)
      Opc = IsFP ? Mcasm::VSCATTERQPSZ256mr : Mcasm::VPSCATTERQDZ256mr;
    else if (IndexVT == MVT::v8i64 && NumElts == 8 && EltSize == 32)
      Opc = IsFP ? Mcasm::VSCATTERQPSZmr : Mcasm::VPSCATTERQDZmr;
    else if (IndexVT == MVT::v2i64 && NumElts == 2 && EltSize == 64)
      Opc = IsFP ? Mcasm::VSCATTERQPDZ128mr : Mcasm::VPSCATTERQQZ128mr;
    else if (IndexVT == MVT::v4i64 && NumElts == 4 && EltSize == 64)
      Opc = IsFP ? Mcasm::VSCATTERQPDZ256mr : Mcasm::VPSCATTERQQZ256mr;
    else if (IndexVT == MVT::v8i64 && NumElts == 8 && EltSize == 64)
      Opc = IsFP ? Mcasm::VSCATTERQPDZmr : Mcasm::VPSCATTERQQZmr;
    else
      break;
  ...
  case ISD::PREALLOCATED_SETUP: {
    auto *MFI = CurDAG->getMachineFunction().getInfo<McasmMachineFunctionInfo>();
  ...
  case McasmISD::AESENCWIDE128KL:
  case McasmISD::AESDECWIDE128KL:
  case McasmISD::AESENCWIDE256KL:
  case McasmISD::AESDECWIDE256KL: {
    ...
    case McasmISD::AESENCWIDE128KL:
      Opcode = Mcasm::AESENCWIDE128KL;
      break;
    case McasmISD::AESDECWIDE128KL:
      Opcode = Mcasm::AESDECWIDE128KL;
      break;
    case McasmISD::AESENCWIDE256KL:
      Opcode = Mcasm::AESENCWIDE256KL;
      break;
    case McasmISD::AESDECWIDE256KL:
      Opcode = Mcasm::AESDECWIDE256KL;
      break;
    }
    ...
    Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::XMM0, Node->getOperand(2),
                                 SDValue());
    Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::XMM1, Node->getOperand(3),
                                 Chain.getValue(1));
    Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::XMM2, Node->getOperand(4),
                                 Chain.getValue(1));
    Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::XMM3, Node->getOperand(5),
                                 Chain.getValue(1));
    Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::XMM4, Node->getOperand(6),
                                 Chain.getValue(1));
    Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::XMM5, Node->getOperand(7),
                                 Chain.getValue(1));
    Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::XMM6, Node->getOperand(8),
                                 Chain.getValue(1));
    Chain = CurDAG->getCopyToReg(Chain, dl, Mcasm::XMM7, Node->getOperand(9),
                                 Chain.getValue(1));
  ...
  case McasmISD::POP_FROM_X87_REG: {
    SDValue Chain = Node->getOperand(0);
...
/// This pass converts a legalized DAG into a Mcasm-specific DAG,
