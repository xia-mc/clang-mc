//===- llvm/lib/Target/Mcasm/McasmISelCallLowering.cpp - Call lowering --------===//
//
...
static bool shouldDisableArgRegFromCSR(CallingConv::ID CC) {
  return CC == CallingConv::Mcasm_RegCall;
}
...
handleMaskRegisterForCallingConv(unsigned NumElts, CallingConv::ID CC,
  ...
  if (NumElts == 4)
    return {MVT::v4i32, 1};
  if (NumElts == 8 && CC != CallingConv::Mcasm_RegCall &&
      CC != CallingConv::Intel_OCL_BI)
    return {MVT::v8i16, 1};
  if (NumElts == 16 && CC != CallingConv::Mcasm_RegCall &&
      CC != CallingConv::Intel_OCL_BI)
    return {MVT::v16i8, 1};
  // v32i1 passes in ymm unless we have BWI and the calling convention is
  // regcall.
  if (NumElts == 32 && (!Subtarget.hasBWI() || CC != CallingConv::Mcasm_RegCall))
    return {MVT::v32i8, 1};
  // Split v64i1 vectors if we don't have v64i8 available.
  if (NumElts == 64 && Subtarget.hasBWI() && CC != CallingConv::Mcasm_RegCall) {
...
MVT McasmTargetLowering::getRegisterTypeForCallingConv(LLVMContext &Context,
                                                     CallingConv::ID CC,
                                                     EVT VT) const {
  if (VT.isVector()) {
...
unsigned McasmTargetLowering::getNumRegistersForCallingConv(LLVMContext &Context,
                                                          CallingConv::ID CC,
                                                          EVT VT) const {
  if (VT.isVector()) {
    if (VT.getVectorElementType() == MVT::i1 && Subtarget.hasAVX512()) {
...
unsigned McasmTargetLowering::getVectorTypeBreakdownForCallingConv(
...
EVT McasmTargetLowering::getSetCCResultType(const DataLayout &DL,
                                          LLVMContext& Context,
                                          EVT VT) const {
  if (!VT.isVector())
    return MVT::i8;
...
bool McasmTargetLowering::functionArgumentNeedsConsecutiveRegisters(
...
/// function arguments in the caller parameter area. For Mcasm, aggregates
/// that contain SSE vectors are placed at 16-byte boundaries while the rest
/// are at 4-byte boundaries.
Align McasmTargetLowering::getByValTypeAlignment(Type *Ty,
                                               const DataLayout &DL) const {
...
EVT McasmTargetLowering::getOptimalMemOpType(
...
bool McasmTargetLowering::isSafeMemOpType(MVT VT) const {
  if (VT == MVT::f32)
...
bool McasmTargetLowering::isMemoryAccessFast(EVT VT, Align Alignment) const {
...
bool McasmTargetLowering::allowsMemoryAccess(LLVMContext &Context,
...
unsigned McasmTargetLowering::getJumpTableEncoding() const {
...
bool McasmTargetLowering::useSoftFloat() const {
  return Subtarget.useSoftFloat();
}

void McasmTargetLowering::markLibCallAttributes(MachineFunction *MF, unsigned CC,
                                              ArgListTy &Args) const {

  // Only relabel Mcasm-32 for C / Stdcall CCs.
  if (Subtarget.is64Bit())
    return;
  if (CC != CallingConv::C && CC != CallingConv::Mcasm_StdCall)
    return;
...
const MCExpr *
McasmTargetLowering::LowerCustomJumpTableEntry(const MachineJumpTableInfo *MJTI,
                                             const MachineBasicBlock *MBB,
                                             unsigned uid,MCContext &Ctx) const{
  assert(isPositionIndependent() && Subtarget.isPICStyleGOT());
  // In 32-bit ELF systems, our jump table entries are formed with @GOTOFF
  // entries.
  return MCSymbolRefExpr::create(MBB->getSymbol(), Mcasm::S_GOTOFF, Ctx);
}

/// Returns relocation base for the given PIC jumptable.
SDValue McasmTargetLowering::getPICJumpTableRelocBase(SDValue Table,
                                                    SelectionDAG &DAG) const {
  if (!Subtarget.is64Bit())
    // This doesn't have SDLoc associated with it, but is not really the
    // same as a Register.
    return DAG.getNode(McasmISD::GlobalBaseReg, SDLoc(),
...
const MCExpr *McasmTargetLowering::
getPICJumpTableRelocBaseExpr(const MachineFunction *MF, unsigned JTI,
                             MCContext &Ctx) const {
  // Mcasm-64 uses RIP relative addressing based on the jump table label.
...
McasmTargetLowering::findRepresentativeClass(const TargetRegisterInfo *TRI,
                                           MVT VT) const {
  ...
  case MVT::i8: case MVT::i16: case MVT::i32: case MVT::i64:
    RRC = Subtarget.is64Bit() ? &Mcasm::GR64RegClass : &Mcasm::GR32RegClass;
    break;
  case MVT::x86mmx:
    RRC = &Mcasm::VR64RegClass;
    break;
  ...
  case MVT::v16f32: case MVT::v8f64:
    RRC = &Mcasm::VR128XRegClass;
    break;
  }
  return std::make_pair(RRC, Cost);
}

unsigned McasmTargetLowering::getAddressSpace() const {
  if (Subtarget.is64Bit())
    return (getTargetMachine().getCodeModel() == CodeModel::Kernel) ? McasmAS::GS
                                                                    : McasmAS::FS;
  return McasmAS::GS;
}
...
Value *
McasmTargetLowering::getIRStackGuard(IRBuilderBase &IRB,
  ...
  if (hasStackGuardSlotTLS(Subtarget.getTargetTriple())) {
    ...
    if (GuardReg == "fs")
      AddressSpace = McasmAS::FS;
    else if (GuardReg == "gs")
      AddressSpace = McasmAS::GS;
...
void McasmTargetLowering::insertSSPDeclarations(
  ...
  if (SecurityCheckCookieLibcall != RTLIB::Unsupported &&
    ...
    if (Function *F = dyn_cast<Function>(SecurityCheckCookie.getCallee())) {
      F->setCallingConv(CallingConv::Mcasm_FastCall);
...
Value *McasmTargetLowering::getSafeStackPointerLocation(
...
bool McasmTargetLowering::CanLowerReturn(
    CallingConv::ID CallConv, MachineFunction &MF, bool isVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs, LLVMContext &Context,
    const Type *RetTy) const {
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, isVarArg, MF, RVLocs, Context);
  return CCInfo.CheckReturn(Outs, RetCC_Mcasm);
}

const MCPhysReg *McasmTargetLowering::getScratchRegisters(CallingConv::ID) const {
  static const MCPhysReg ScratchRegs[] = { Mcasm::R11, 0 };
  return ScratchRegs;
}

ArrayRef<MCPhysReg> McasmTargetLowering::getRoundingControlRegisters() const {
  static const MCPhysReg RCRegs[] = {Mcasm::FPCW, Mcasm::MXCSR};
  return RCRegs;
}
...
static void Passv64i1ArgInRegs(
    ...
    CCValAssign &NextVA, const McasmSubtarget &Subtarget) {
...
SDValue
McasmTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool isVarArg,
                               const SmallVectorImpl<ISD::OutputArg> &Outs,
                               const SmallVectorImpl<SDValue> &OutVals,
                               const SDLoc &dl, SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  McasmMachineFunctionInfo *FuncInfo = MF.getInfo<McasmMachineFunctionInfo>();

  // In some cases we need to disable registers from the default CSR list.
  // For example, when they are used as return registers (preserve_* and Mcasm's
  // regcall) or for argument passing (Mcasm's regcall).
  bool ShouldDisableCalleeSavedRegister =
      shouldDisableRetRegFromCSR(CallConv) ||
      MF.getFunction().hasFnAttribute("no_caller_saved_registers");

  if (CallConv == CallingConv::Mcasm_INTR && !Outs.empty())
    report_fatal_error("Mcasm interrupts may not return any value");
  ...
  for (unsigned I = 0, OutsIndex = 0, E = RVLocs.size(); I != E;
    ...
    if (!Subtarget.hasSSE1() && Mcasm::FR32XRegClass.contains(VA.getLocReg())) {
      errorUnsupported(DAG, dl, "SSE register return with SSE disabled");
      VA.convertToReg(Mcasm::FP0); // Set reg to FP0, avoid hitting asserts.
    } else if (!Subtarget.hasSSE2() &&
               Mcasm::FR64XRegClass.contains(VA.getLocReg()) &&
               ValVT == MVT::f64) {
    ...
    if (VA.getLocReg() == Mcasm::FP0 ||
        VA.getLocReg() == Mcasm::FP1) {
    ...
    if (Subtarget.is64Bit()) {
      if (ValVT == MVT::x86mmx) {
        if (VA.getLocReg() == Mcasm::XMM0 || VA.getLocReg() == Mcasm::XMM1) {
  ...
  for (auto &RetVal : RetVals) {
    if (RetVal.first == Mcasm::FP0 || RetVal.first == Mcasm::FP1) {
  ...
  if (I) {
    for (; *I; ++I) {
      if (Mcasm::GR64RegClass.contains(*I))
  ...
  McasmISD::NodeType opcode = McasmISD::RET_GLUE;
  if (CallConv == CallingConv::Mcasm_INTR)
    opcode = McasmISD::IRET;
  return DAG.getNode(opcode, dl, MVT::Other, RetOps);
}

bool McasmTargetLowering::isUsedByReturnOnly(SDNode *N, SDValue &Chain) const {
  ...
  for (const SDNode *U : Copy->users()) {
    if (U->getOpcode() != McasmISD::RET_GLUE)
      return false;
...
EVT McasmTargetLowering::getTypeForExtReturn(LLVMContext &Context, EVT VT,
...
static SDValue getv64i1Argument(CCValAssign &VA, CCValAssign &NextVA,
                                ...
                                const SDLoc &DL, const McasmSubtarget &Subtarget,
...
static SDValue getPopFromX87Reg(SelectionDAG &DAG, SDValue Chain,
  ...
  return DAG.getNode(McasmISD::POP_FROM_X87_REG, dl, VTs,
...
///
SDValue McasmTargetLowering::LowerCallResult(
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), RVLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallResult(Ins, RetCC_Mcasm);
  ...
  for (unsigned I = 0, InsIndex = 0, E = RVLocs.size(); I != E;
    ...
    if (!Subtarget.hasSSE1() && Mcasm::FR32XRegClass.contains(VA.getLocReg())) {
      errorUnsupported(DAG, dl, "SSE register return with SSE disabled");
      if (VA.getLocReg() == Mcasm::XMM1)
        VA.convertToReg(Mcasm::FP1); // Set reg to FP1, avoid hitting asserts.
      else
        VA.convertToReg(Mcasm::FP0); // Set reg to FP0, avoid hitting asserts.
    } else if (!Subtarget.hasSSE2() &&
               Mcasm::FR64XRegClass.contains(VA.getLocReg()) &&
    ...
    bool X87Result = VA.getLocReg() == Mcasm::FP0 || VA.getLocReg() == Mcasm::FP1;
...
static bool hasCalleePopSRet(const SmallVectorImpl<T> &Args,
  ...
  if (!TT.isMcasm_32() || TT.isOSMSVCRT() || TT.isOSIAMCU())
    return false;
...
static bool canGuaranteeTCO(CallingConv::ID CC) {
  return (CC == CallingConv::Fast || CC == CallingConv::GHC ||
          CC == CallingConv::Mcasm_RegCall || CC == CallingConv::HiPE ||
...
bool McasmTargetLowering::mayBeEmittedAsTailCall(const CallInst *CI) const {
  if (!CI->isTailCall())
    return false;
...
SDValue
McasmTargetLowering::LowerMemArgument(SDValue Chain, CallingConv::ID CallConv,
...
static ArrayRef<MCPhysReg> get64BitArgumentGPRs(CallingConv::ID CallConv,
  ...
  if (Subtarget.isCallingConvWin64(CallConv)) {
    static const MCPhysReg GPR64ArgRegsWin64[] = {
      Mcasm::RCX, Mcasm::RDX, Mcasm::R8,  Mcasm::R9
    };
...
public:
  VarArgsLoweringHelper(McasmMachineFunctionInfo *FuncInfo, const SDLoc &Loc,
                        SelectionDAG &DAG, const McasmSubtarget &Subtarget,
...
private:
  ...
  McasmMachineFunctionInfo *FuncInfo;
  const SDLoc &DL;
  SelectionDAG &DAG;
  const McasmSubtarget &Subtarget;
...
void VarArgsLoweringHelper::createVarArgAreaAndStoreRegisters(
  ...
  if (is64Bit() || (CallConv != CallingConv::Mcasm_FastCall &&
                    CallConv != CallingConv::Mcasm_ThisCall)) {
  ...
  if (is64Bit()) {
    ...
    if (isWin64()) {
      ...
      // For Mcasm-64, if there are vararg parameters that are passed via
    ...
    for (MCPhysReg Reg : ArgGPRs.slice(NumIntRegs)) {
      Register GPR = TheMachineFunction.addLiveIn(Reg, &Mcasm::GR64RegClass);
    ...
    if (!AvailableXmms.empty()) {
      Register AL = TheMachineFunction.addLiveIn(Mcasm::AL, &Mcasm::GR8RegClass);
    ...
    if (!LiveXMMRegs.empty()) {
      ...
      MemOps.push_back(DAG.getMemIntrinsicNode(McasmISD::VASTART_SAVE_XMM_REGS,
...
void VarArgsLoweringHelper::forwardMustTailParameters(SDValue &Chain) {
  ...
  if (Subtarget.useAVX512Regs() &&
      (is64Bit() || (CallConv == CallingConv::Mcasm_VectorCall ||
  ...
  CCInfo.analyzeMustTailForwardedRegisters(Forwards, RegParmTypes, CC_Mcasm);

  // Forward AL for SysV x86_64 targets, since it is used for varargs.
  if (is64Bit() && !isWin64() && !CCInfo.isAllocated(Mcasm::AL)) {
    Register ALVReg = TheMachineFunction.addLiveIn(Mcasm::AL, &Mcasm::GR8RegClass);
    Forwards.push_back(ForwardedRegister(ALVReg, Mcasm::AL, MVT::i8));
  }
...
SDValue McasmTargetLowering::LowerFormalArguments(
  ...
  CCInfo.AnalyzeArguments(Ins, CC_Mcasm);

  // In vectorcall calling convention a second pass is required for the HVA
  // types.
  if (CallingConv::Mcasm_VectorCall == CallConv) {
    CCInfo.AnalyzeArgumentsSecondPass(Ins, CC_Mcasm);
  }
    ...
    if (VA.isRegLoc()) {
      ...
      if (VA.needsCustom()) {
        ...
        if (RegVT == MVT::i8)
          ...
          RC = Subtarget.hasAVX512() ? &Mcasm::FR16XRegClass : &Mcasm::FR16RegClass;
        else if (RegVT == MVT::f32)
          RC = Subtarget.hasAVX512() ? &Mcasm::FR32XRegClass : &Mcasm::FR32RegClass;
        else if (RegVT == MVT::f64)
          RC = Subtarget.hasAVX512() ? &Mcasm::FR64XRegClass : &Mcasm::FR64RegClass;
        else if (RegVT == MVT::f80)
          RC = &Mcasm::RFP80RegClass;
        else if (RegVT == MVT::f128)
          RC = &Mcasm::VR128RegClass;
        else if (RegVT.is512BitVector())
          RC = &Mcasm::VR512RegClass;
        else if (RegVT.is256BitVector())
          RC = Subtarget.hasVLX() ? &Mcasm::VR256XRegClass : &Mcasm::VR256RegClass;
        else if (RegVT.is128BitVector())
          RC = Subtarget.hasVLX() ? &Mcasm::VR128XRegClass : &Mcasm::VR128RegClass;
        else if (RegVT == MVT::x86mmx)
          RC = &Mcasm::VR64RegClass;
        else if (RegVT == MVT::v1i1)
          RC = &Mcasm::VK1RegClass;
        else if (RegVT == MVT::v8i1)
          RC = &Mcasm::VK8RegClass;
        else if (RegVT == MVT::v16i1)
          RC = &Mcasm::VK16RegClass;
        else if (RegVT == MVT::v32i1)
          RC = &Mcasm::VK32RegClass;
        else if (RegVT == MVT::v64i1)
          RC = &Mcasm::VK64RegClass;
        else
      ...
      if (VA.isExtInLoc()) {
        ...
        if (RegVT.isVector() && VA.getValVT().getScalarType() != MVT::i1)
          ArgValue = DAG.getNode(McasmISD::MOVDQ2Q, dl, VA.getValVT(), ArgValue);
  ...
  for (unsigned I = 0, E = Ins.size(); I != E; ++I) {
    if (Ins[I].Flags.isSwiftAsync()) {
      ...
      if (Mcasm::isExtendedSwiftAsyncFrameSupported(Subtarget, MF))
        McasmFI->setHasSwiftAsyncContext(true);
      else {
        int PtrSize = Subtarget.is64Bit() ? 8 : 4;
        int FI =
            MF.getFrameInfo().CreateStackObject(PtrSize, Align(PtrSize), false);
        McasmFI->setSwiftAsyncContextFrameIdx(FI);
  ...
  if (Mcasm::isCalleePop(CallConv, Is64Bit, IsVarArg,
                       MF.getTarget().Options.GuaranteedTailCallOpt)) {
    FuncInfo->setBytesToPopOnReturn(StackSize); // Callee pops everything.
  } else if (CallConv == CallingConv::Mcasm_INTR && Ins.size() == 2) {
    // Mcasm interrupts must pop the error code (and the alignment padding) if
    // present.
  ...
  if (!Is64Bit) {
    // RegSaveFrameIndex is Mcasm-64 only.
    FuncInfo->setRegSaveFrameIndex(0xAAAAAAA);
  }
...
SDValue McasmTargetLowering::LowerMemOpCallTo(SDValue Chain, SDValue StackPtr,
...
SDValue McasmTargetLowering::EmitTailCallLoadRetAddr(
...
SDValue McasmTargetLowering::getMOVL(SelectionDAG &DAG, const SDLoc &dl, MVT VT,
...
// caller (NoCopy).
McasmTargetLowering::ByValCopyKind McasmTargetLowering::ByValNeedsCopyForTailCall(
...
SDValue
McasmTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  McasmMachineFunctionInfo *McasmInfo = MF.getInfo<McasmMachineFunctionInfo>();
  ...
  if (CallConv == CallingConv::Mcasm_INTR)
    report_fatal_error("Mcasm interrupts may not be called directly");
  ...
  CCInfo.AnalyzeArguments(Outs, CC_Mcasm);

  // In vectorcall calling convention a second pass is required for the HVA
  // types.
  if (CallingConv::Mcasm_VectorCall == CallConv) {
    CCInfo.AnalyzeArgumentsSecondPass(Outs, CC_Mcasm);
  }
  ...
  if (isTailCall && ShouldGuaranteeTCO && !IsSibcall) {
    ...
    unsigned NumBytesCallerPushed = McasmInfo->getBytesToPopOnReturn();

    FPDiff = NumBytesCallerPushed - NumBytes;

    // Set the delta of movement of the returnaddr stackslot.
    // But only set if delta is greater than previous delta.
    if (FPDiff < McasmInfo->getTCReturnAddrDelta())
      McasmInfo->setTCReturnAddrDelta(FPDiff);
  }
  ...
  if (!Outs.empty() && Outs.back().Flags.isInAlloca()) {
    ...
    auto *MFI = DAG.getMachineFunction().getInfo<McasmMachineFunctionInfo>();
  ...
  for (unsigned I = 0, OutIndex = 0, E = ArgLocs.size(); I != E;
    ...
    if (VA.needsCustom()) {
      ...
      if (isVarArg && IsWin64) {
        ...
        Register ShadowReg;
        switch (VA.getLocReg()) {
        case Mcasm::XMM0: ShadowReg = Mcasm::RCX; break;
        case Mcasm::XMM1: ShadowReg = Mcasm::RDX; break;
        case Mcasm::XMM2: ShadowReg = Mcasm::R8; break;
        case Mcasm::XMM3: ShadowReg = Mcasm::R9; break;
        }
        if (ShadowReg)
  ...
  if (Subtarget.isPICStyleGOT()) {
    ...
    if (!isTailCall) {
      ...
      if (CallConv != CallingConv::Mcasm_RegCall)
        RegsToPass.push_back(std::make_pair(
          Register(Mcasm::EBX), DAG.getNode(McasmISD::GlobalBaseReg, SDLoc(),
                                          getPointerTy(DAG.getDataLayout()))));
    } else {
  ...
  if (Is64Bit && isVarArg && !IsWin64 && !IsMustTail &&
    ...
    RegsToPass.push_back(std::make_pair(Register(Mcasm::AL),
                                        DAG.getConstant(NumXMMRegs, dl,
                                                        MVT::i8)));
  }

  if (isVarArg && IsMustTail) {
    const auto &Forwards = McasmInfo->getForwardedMustTailRegParms();
  ...
  if (isTailCall && !IsSibcall) {
    ...
    for (unsigned I = 0, OutsIndex = 0, E = ArgLocs.size(); I != E;
      ...
      if (VA.isRegLoc()) {
        if (VA.needsCustom()) {
          assert((CallConv == CallingConv::Mcasm_RegCall) &&
  ...
  if (DAG.getTarget().getCodeModel() == CodeModel::Large) {
    ...
    Callee = DAG.getTargetGlobalAddress(
        GA->getGlobal(), dl, GA->getValueType(0), 0, McasmII::MO_NO_FLAG);
  }
  ...
  const uint32_t *Mask = [&]() {
    ...
    // use Mcasm_INTR calling convention because it has the same CSR mask
    // (same preserved registers).
    if (HasNCSR)
      AdaptedCC = (CallingConv::ID)CallingConv::Mcasm_INTR;
  ...
  if (MachineOperand::clobbersPhysReg(Mask, RegInfo->getFramePtr())) {
    McasmInfo->setFPClobberedByCall(true);
    if (CLI.CB && isa<InvokeInst>(CLI.CB))
      McasmInfo->setFPClobberedByInvoke(true);
  }
  if (MachineOperand::clobbersPhysReg(Mask, RegInfo->getBaseRegister())) {
    McasmInfo->setBPClobberedByCall(true);
    if (CLI.CB && isa<InvokeInst>(CLI.CB))
      McasmInfo->setBPClobberedByInvoke(true);
  }
  ...
  if (isTailCall) {
    ...
    auto Opcode =
        IsCFGuardCall ? McasmISD::TC_RETURN_GLOBALADDR : McasmISD::TC_RETURN;
  ...
  if (IsImpCall) {
    Chain = DAG.getNode(McasmISD::IMP_CALL, dl, NodeTys, Ops);
  } else if (IsNoTrackIndirectCall) {
    Chain = DAG.getNode(McasmISD::NT_CALL, dl, NodeTys, Ops);
  } else if (IsCFGuardCall) {
    Chain = DAG.getNode(McasmISD::CALL_GLOBALADDR, dl, NodeTys, Ops);
    ...
    Chain = DAG.getNode(McasmISD::CALL_RVMARKER, dl, NodeTys, Ops);
  } else {
...
//  On Mcasm_64 architecture with GOT-style position independent code only local
...
/// requirement.
unsigned
McasmTargetLowering::GetAlignedArgumentStackSize(const unsigned StackSize,
                                               SelectionDAG &DAG) const {
...
bool MatchingStackOffset(SDValue Arg, unsigned Offset, ISD::ArgFlagsTy Flags,
                         ...
                         const McasmInstrInfo *TII, const CCValAssign &VA) {
...
bool McasmTargetLowering::isEligibleForSiblingCallOpt(
  ...
  if (CallerF.getReturnType()->isMcasm_FP80Ty() && !CLI.RetTy->isMcasm_FP80Ty())
    return false;
  ...
  if (Unused) {
    ...
    RVCCInfo.AnalyzeCallResult(Ins, RetCC_Mcasm);
  ...
  if (!Outs.empty()) {
    ...
    if (!Subtarget.is64Bit() && ((!isa<GlobalAddressSDNode>(Callee) &&
      ...
      for (const auto &VA : ArgLocs) {
        ...
        switch (Reg) {
        default: break;
        case Mcasm::EAX: case Mcasm::EDX: case Mcasm::ECX:
  ...
  bool CalleeWillPop =
      Mcasm::isCalleePop(CalleeCC, Subtarget.is64Bit(), isVarArg,
                       MF.getTarget().Options.GuaranteedTailCallOpt);
...
bool Mcasm::isCalleePop(CallingConv::ID CallingConv,
