//===- McasmFixupSetCC.cpp - fix zero-extension of setcc patterns -----------===//
//
...
// Mcasm setcc instructions are modeled to have no input arguments, and a single
...
namespace {
class McasmFixupSetCCLegacy : public MachineFunctionPass {
public:
  static char ID;

  McasmFixupSetCCLegacy() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override { return "Mcasm Fixup SetCC"; }

  bool runOnMachineFunction(MachineFunction &MF) override;
};
} // end anonymous namespace

char McasmFixupSetCCLegacy::ID = 0;

INITIALIZE_PASS(McasmFixupSetCCLegacy, DEBUG_TYPE, DEBUG_TYPE, false, false)

FunctionPass *llvm::createMcasmFixupSetCCLegacyPass() {
  return new McasmFixupSetCCLegacy();
}
...
static bool fixupSetCC(MachineFunction &MF) {
  ...
  for (auto &MBB : MF) {
    ...
    for (auto &MI : MBB) {
      ...
      if (MI.definesRegister(Mcasm::EFLAGS, /*TRI=*/nullptr))
        FlagsDefMI = &MI;
      ...
      // regardless.
      if (MI.getOpcode() != Mcasm::SETCCr && MI.getOpcode() != Mcasm::SETZUCCr)
        continue;

      MachineInstr *ZExt = nullptr;
      Register Reg0 = MI.getOperand(0).getReg();
      for (auto &Use : MRI->use_instructions(Reg0))
        if (Use.getOpcode() == Mcasm::MOVZX32rr8)
          ZExt = &Use;
      ...
      // invalid.
      if (!ST->hasZU() &&
          FlagsDefMI->readsRegister(Mcasm::EFLAGS, /*TRI=*/nullptr))
        continue;

      // On 32-bit, we need to be careful to force an ABCD register.
      const TargetRegisterClass *RC =
          ST->is64Bit() ? &Mcasm::GR32RegClass : &Mcasm::GR32_ABCDRegClass;
      ...
      // Mcasm setcc/setzucc only takes an output GR8, so fake a GR32 input by
      // inserting the setcc/setzucc result into the low byte of the zeroed
      // register.
      Register ZeroReg = MRI->createVirtualRegister(RC);
      if (ST->hasZU()) {
        if (!ST->preferLegacySetCC())
          assert((MI.getOpcode() == Mcasm::SETZUCCr) &&
                 "Expect setzucc instruction!");
        else
          MI.setDesc(TII->get(Mcasm::SETZUCCr));
        BuildMI(*ZExt->getParent(), ZExt, ZExt->getDebugLoc(),
                TII->get(TargetOpcode::IMPLICIT_DEF), ZeroReg);
      } else {
        // Initialize a register with 0. This must go before the eflags def
        BuildMI(MBB, FlagsDefMI, MI.getDebugLoc(), TII->get(Mcasm::MOV32r0),
                ZeroReg);
      }

      BuildMI(*ZExt->getParent(), ZExt, ZExt->getDebugLoc(),
              TII->get(Mcasm::INSERT_SUBREG), ZExt->getOperand(0).getReg())
          .addReg(ZeroReg)
          .addReg(Reg0)
          .addImm(Mcasm::sub_8bit);
...
bool McasmFixupSetCCLegacy::runOnMachineFunction(MachineFunction &MF) {
  return fixupSetCC(MF);
}

PreservedAnalyses McasmFixupSetCCPass::run(MachineFunction &MF,
