//===---- McasmIndirectBranchTracking.cpp - Enables CET IBT mechanism -------===//
//
...
cl::opt<bool> IndirectBranchTracking(
    ...
    cl::desc("Enable Mcasm indirect branch tracking pass."));

STATISTIC(NumEndBranchAdded, "Number of ENDBR instructions added");

namespace {
class McasmIndirectBranchTrackingPass : public MachineFunctionPass {
public:
  McasmIndirectBranchTrackingPass() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override {
    return "Mcasm Indirect Branch Tracking";
  }
...
char McasmIndirectBranchTrackingPass::ID = 0;

FunctionPass *llvm::createMcasmIndirectBranchTrackingPass() {
  return new McasmIndirectBranchTrackingPass();
}

bool McasmIndirectBranchTrackingPass::addENDBR(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator I) const {
  assert(TII && "Target instruction info was not initialized");
  assert((Mcasm::ENDBR64 == EndbrOpcode || Mcasm::ENDBR32 == EndbrOpcode) &&
...
bool McasmIndirectBranchTrackingPass::runOnMachineFunction(MachineFunction &MF) {
  ...
  EndbrOpcode = SubTarget.is64Bit() ? Mcasm::ENDBR64 : Mcasm::ENDBR32;
