//===-- McasmInstrBuilder.h - Functions to aid building x86 insts -*- C++ -*-===//
//
...
// MachineInstrBuilder.h file to handle Mcasm'isms in a clean way.
//
// The BuildMem function may be used with the BuildMI function to add entire
// memory references in a single, typed, function call.  Mcasm memory references
...
#ifndef LLVM_LIB_TARGET_Mcasm_McasmINSTRBUILDER_H
#define LLVM_LIB_TARGET_Mcasm_McasmINSTRBUILDER_H
...
/// McasmAddressMode - This struct holds a generalized full x86 address mode.
/// The base register can be a frame index, which will eventually be replaced
/// with BP or SP and Disp being offsetted accordingly.  The displacement may
/// also include the offset of a global value.
struct McasmAddressMode {
  ...
  void getFullAddress(SmallVectorImpl<MachineOperand> &MO) {
    ...
    if (BaseType == McasmAddressMode::RegBase)
      MO.push_back(MachineOperand::CreateReg(Base.Reg, false, false, false,
                                             false, false, false, 0, false));
    else {
      assert(BaseType == McasmAddressMode::FrameIndexBase);
...
/// given operand.
static inline McasmAddressMode getAddressFromInstr(const MachineInstr *MI,
                                                 unsigned Operand) {
  McasmAddressMode AM;
  const MachineOperand &Op0 = MI->getOperand(Operand);
  if (Op0.isReg()) {
    AM.BaseType = McasmAddressMode::RegBase;
...
addFullAddress(const MachineInstrBuilder &MIB,
               const McasmAddressMode &AM) {
  assert(AM.Scale == 1 || AM.Scale == 2 || AM.Scale == 4 || AM.Scale == 8);

  if (AM.BaseType == McasmAddressMode::RegBase)
    MIB.addReg(AM.Base.Reg);
  else {
    assert(AM.BaseType == McasmAddressMode::FrameIndexBase);
...
#endif // LLVM_LIB_TARGET_Mcasm_McasmINSTRBUILDER_H
